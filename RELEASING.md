...
# Size-generality arm: folds every structure model at 256/512/640/768 aa and
# fails if the fired/dark perf-lever set or the runtime scaling exponent drifted
# from docs/size_ladder_baseline.json. It also folds protenix-v2 at 506 tokens,
# which must land on an 11x10 grid (the window that hangs on 13x10, issue #9) --
# that leg needs no baseline. A perf lever may not ship default-ON on the
