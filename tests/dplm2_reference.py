...
def make_dplm2_generator(seed: int = 0, num_diffusion_timesteps: int = 500):
    """Reference discrete-diffusion generator: same loop as the ttnn port, with a
    fp32 DPLM2Reference backbone. Used to parity-gate the ttnn generation loop.
    """
    from tt_bio.dplm2_sampler import DPLM2Sampler, DPLM2Tokenizer
    torch.manual_seed(seed)
    ref = make_dplm2_150m(seed).eval()
    tok = DPLM2Tokenizer.from_pretrained("airkingbd/dplm2_150m")

    def backbone_fn(input_ids):
        logits, _ = ref(input_ids)
        return logits

    return DPLM2Sampler(tok, backbone_fn, num_diffusion_timesteps), tok
