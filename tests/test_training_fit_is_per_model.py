...
def test_openfold3_is_refused_at_every_crop_measured_to_refuse(tokens):
    # The second direction. Both were measured to refuse on Blackhole at 1350 MHz sampled
    # during; UNMEASURED here would let a user start a run we have watched run out of memory.
    # 544 and 576 were in this list until 2026-09-25. of3t-cropwall's concat-heads split
    # (e558bfb06, on main since 2026-09-23) removed the CONTIGUITY refusal they measured: 576
    # now COMPLETES its backward twice over, ok=True at 30,230,471,680 B and 30,315,406,336 B
    # (perf/of3t_cropwall/out/split_576_fix2_card3.json, split_576_post.json), and 640 was
    # re-confirmed refusing post-fix in split_640_fix2_card3.json. A refusal is only true of
    # the code it was measured on.
...
def test_a_measured_fit_is_reported_even_when_the_step_time_is_not():
    ...
    p = plan(tokens=576, model="openfold3")
    assert p.verdict == UNMEASURED
    assert "memory fits up to 576 aa" in p.why
    assert p.sources


def test_the_fit_note_does_not_appear_above_the_measured_fit():
    # 640 is the first crop that refuses post-concat-fix, so nothing may claim a fit above it.
    largest, _ = LARGEST_MEASURED_TO_FIT["openfold3"]
    assert largest == 576
    assert "memory fits" not in (plan(tokens=640, model="openfold3").why or "")
