...
class TestPwaBlockArithmetic:
    ...
    @staticmethod
    def _rows(tokens, c_m=64, depth=14189):
        return tt.pwa_depth_block(depth, tokens, c_m, tt.PWA_SINGLE_SHOT_BYTES_BASE)

    @pytest.mark.parametrize("tokens", (800, 896, 1024, 1088))
    def test_the_block_keeps_the_normed_chunk_under_budget(self, tokens):
        rows = self._rows(tokens)
        assert rows % 32 == 0 and rows >= 32
        assert rows * tokens * 64 * 2 <= tt.PWA_DEPTH_BUDGET_BYTES

    @pytest.mark.parametrize("tokens", (640, 768))
    def test_a_size_that_folds_today_is_the_unblocked_path(self, tokens):
        """Whole-tensor back means the untouched sequence of allocations, byte for byte."""
        assert self._rows(tokens) == 14189
    ...
    def test_a_shallow_alignment_is_one_block_and_takes_the_whole_path(self):
        ...
        assert self._rows(1088, depth=256) == 256
