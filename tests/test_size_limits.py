...
def test_a_wormhole_row_walked_past_1536_admits_1536_and_refuses_its_first_failure(model):
    ...
    existed boltz2 and protenix-v1 were never refused and rf3, openfold3 and protenix-v2 stopped at the top of a
...
@pytest.mark.parametrize("model", ["protenix-v2", "openbind"])
def test_a_wormhole_row_past_1536_whose_walk_is_open_refuses_above_what_folded(model):
    """protenix-v2 and openbind folded the rung their old rows failed at once mgx-bigalloc landed,
    and the rung above has not been walked on main yet. Until it is, the cap is the size that folded:
    1536 and the cap are admitted and one residue more is refused, so nothing unmeasured is let in."""
    c = sl.ceiling(model, "wormhole_b0")
