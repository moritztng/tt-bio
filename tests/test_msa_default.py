...
by default via ``_resolve_msa_default``; these tests pin the precedence contract:
  ...
  4. nothing                -> enable the online server + a privacy notice
...
from tt_bio.main import _resolve_msa_default as resolve
...
def test_single_sequence_models_untouched(model, tmp_path, capsys):
    ...
    assert resolve(model, False, None, None, False, str(tmp_path), None) == (False, None)
...
def test_no_source_falls_back_online_with_notice(model, tmp_path, capsys):
    ...
    use_msa_server, msa_db_path = resolve(model, False, None, None, False, str(tmp_path), None)
    assert use_msa_server is True and msa_db_path is None
    assert "api.colabfold.com" in capsys.readouterr().out
...
def test_local_db_used_silently(model, tmp_path, capsys):
    ...
    assert resolve(model, False, None, None, False, str(tmp_path), None) == (False, str(db))
    assert capsys.readouterr().out == ""
...
def test_explicit_sources_pass_through(model, tmp_path, capsys):
    ...
    assert resolve(model, True, None, None, False, str(tmp_path), None) == (True, None)
    assert resolve(model, False, "/db", None, False, str(tmp_path), None) == (False, "/db")
    # msa_endpoint alone counts as an explicit source: stay off both server and DB
    assert resolve(model, False, None, "http://h:8765", False, str(tmp_path), None) == (False, None)
    assert capsys.readouterr().out == ""
...
def test_single_sequence_opt_out_silent(model, tmp_path, capsys):
    ...
    _ready_db(tmp_path)
    assert resolve(model, False, None, None, True, str(tmp_path), None) == (False, None)
    assert capsys.readouterr().out == ""


@pytest.mark.parametrize("kwargs", [
    dict(use_msa_server=True, msa_db_path=None, msa_endpoint=None),
    dict(use_msa_server=False, msa_db_path="/db", msa_endpoint=None),
    dict(use_msa_server=False, msa_db_path=None, msa_endpoint="http://h"),
])
def test_single_sequence_conflicts_error(kwargs, tmp_path):
    ...
    with pytest.raises(click.BadParameter):
        resolve("protenix-v2", single_sequence=True, cache=str(tmp_path), controller=None, **kwargs)
...
def test_controller_skips_local_db(tmp_path, capsys):
    ...
    _ready_db(tmp_path)
    assert resolve("boltz2", False, None, None, False, str(tmp_path), "http://host:8765") == (True, None)
    assert "api.colabfold.com" in capsys.readouterr().out
