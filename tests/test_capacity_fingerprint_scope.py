...
`ceilings_fingerprint` decides whether a recorded capacity cell is stale, and a stale cell is
card time to re-measure. It therefore has to fire on a row that changes what tt-bio ADMITS, and
not on a row that only improves the evidence for the same admitted size. This file pins the
FIELD SET the hash is taken over. `tests/test_capacity_gate.py` pins its SCOPE, which is the one
(model, arch) row the cell was measured against and not the whole table -- hence the explicit
MODEL, ARCH below, where this file used to hash everything and pass no arguments at all.
...
refused -- and marked every cell on that row stale, because `binds` and `mechanism` were in the hash.
...
def row(monkeypatch):
    ...
    def put(**kw):
        ...
        return cg.ceilings_fingerprint(MODEL, ARCH)

    return put


def test_moving_the_admitted_size_invalidates_the_cells_on_that_row(row):
    ...
    before = cg.ceilings_fingerprint(MODEL, ARCH)
