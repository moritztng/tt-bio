...
The HSA pre-fix diagonal matches its all-pairs cross mean on both metrics, so the residual was systematic bf16 arithmetic divergence, not an RNG-wiring defect — the same precision-floor family the affinity legs show. JSON: `docs/implementation-parity-data/protenix-v2-hsa-paired-5x5.json` (historical, pre-fix). Reproduce: `python3 scripts/pharma_parity.py structures --ref-fixtures protenix-v2/hsa/msa-server_200step_5sample_10cycle_bf16 --dev-dirs <dev_hsa_seed0..4>/boltz_results_hsa --paired`.
