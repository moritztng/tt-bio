...
The remaining structure-leg local-metric gap is Boltz-2 ubiquitin CA-lDDT (§).
Its same-seed diagonal (0.155) matches the all-pairs cross mean (0.151), so shared
RNG draws do not collapse the residual — not an RNG-wiring defect. The device's
local structure sits about 0.07 lDDT below the reference's seed-to-seed
self-consistency while global CA-RMSD and TM-score pass. The pinned Boltz 2.2.1
reference uses `bf16-mixed` for Boltz-2 on both CPU and GPU, so this is not an
fp32-reference versus bf16-device mismatch (unlike Protenix-v2 HSA below). The
§ CA-lDDT GAP is on the no-MSA ubiquitin config (not the production default);
the MSA-backed ubiquitin leg (§§§, the production default) passes CA-lDDT
within the tight floor (X/floor 0.97), so the caveat does not apply to the
config a pharma team actually runs. Protenix-v2 HSA no longer belongs in this
set: matching the reference's full fp32 diffusion boundary reduces its CA-RMSD
same-seed diagonal from 1.007 Å to 0.665 Å and puts every scored metric inside the
reference floor. JSON: `docs/implementation-parity-data/protenix-v2-hsa-fp32-device.json`
and `docs/implementation-parity-data/boltz2-ubiquitin-paired-5x5.json`.
...
In the ubiquitin GAP the same-seed diagonal is NOT smaller than the all-pairs cross mean (1-lDDT diagonal 0.155 is above the cross 0.151) — matching the RNG stream does not collapse the residual — so the residual is systematic bf16 arithmetic divergence between the device and reference diffusions, not a port defect in the RNG wiring or the sampler. This is the measured-diagonal rigor the FKBP12 affinity leg established, now applied to the remaining structure-leg GAP: a live pharma reviewer sees identical evidentiary style on every non-PASS entry. JSON: `docs/implementation-parity-data/protenix-v2-hsa-paired-5x5.json` (historical, pre-fix), `docs/implementation-parity-data/boltz2-ubiquitin-paired-5x5.json`. Reproduce: `python3 scripts/pharma_parity.py structures --ref-fixtures protenix-v2/hsa/msa-server_200step_5sample_10cycle_bf16 --dev-dirs <dev_hsa_seed0..4>/boltz_results_hsa --paired` (and the same for `boltz2/ubiquitin/nomsa_200step_1sample_3recycle_bf16`, dev `boltz_results_ubiquitin_no_msa`).
...
```bash
# reference (once, pinned in docs/implementation-parity-data/ref-fixtures/boltz2/ubiquitin/nomsa_200step_1sample_3recycle_bf16/):
...
python3 scripts/pharma_parity.py structures \
  ...
  --ref-seeds 0 1 2 3 4 \
  --dev-dirs dev_hsa_seed{0,1,2,3,4}/boltz_results_hsa \
