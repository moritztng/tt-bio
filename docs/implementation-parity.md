...
| RoseTTAFold3 | ubiquitin, L76, MSA | PASS | vendored torch CPU reference, bf16 both sides, shared draws; CA-RMSD X 0.226 Å inside the 0.418 Å reference noise floor (X/floor 0.54) and all-atom X 0.213 Å inside a 0.467 Å floor (X/floor 0.46). Four of the five seeds reproduce the reference trajectory to 0.087-0.097 Å; seed 4 alone reads 0.763 Å and it is the reference that moved, its own sample sitting 0.86-0.91 Å from its siblings while the device's stays inside the normal device spread (worst device pair 0.291 Å) — a bifurcation, like `boltz2-prot-nomsa` at seed 0. `scripts/rf3_port/accuracy_cell.py`, five seeds, shipped arm (10 recycles, 50 sampling steps, one diffusion sample) |
| RoseTTAFold3 | 7ROA, L117, MSA | PASS | same harness and convention as the row above; CA-RMSD X 0.207 Å inside the 0.361 Å reference noise floor (X/floor 0.57) and all-atom X 0.279 Å inside a 0.619 Å floor (X/floor 0.45). No outlier seed here: all five X land 0.121-0.349 Å, so the ubiquitin leg's seed-4 bifurcation is a per-seed trajectory property and not a length- or target-independent one |
...
| PXDesign | LacZ-C 768aa target + 80-mer binder, 848 tokens, generated coordinates | PASS | design coordinates bit-exact across all six warm repetitions of three independent device legs, `structure_sha16 cd80f8e274306706`, including across the fp32-softmax L1-padded plan arm — the lever changes the shard's extent and not the arithmetic |
...
Net: 37 PASS, 6 PASS-caveated, 2 GAP-evidenced (boltz2-9ncy-nomsa, root-caused below; af2ig-trunk-device, a measured bfloat16 floor the leg gates as such). The three Boltz-2 affinity
