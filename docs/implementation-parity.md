...
| OpenBind-0 | FKBP12 + SB3 (1FKG), L107 + 33 ligand atoms, MSA | PASS-caveated | external CPU reference; all-atom RMSD X 0.604 Å inside the 0.551 Å floor (X/floor 1.10), so `full_parity_gate.py` records it PASS on the gate metric. The residual is the ligand pose, not the fold: split under the same global superposition the protein half is X 0.583 Å against a 0.551 Å floor and the ligand half X 0.980 Å against a 0.630 Å floor. 1-TM (1.21) and 1-lDDT (1.31) sit above their tighter floors. All 864 atoms pair between device and reference, the 33 SB3 atoms included ([per-leg evidence](implementation-parity-details.md#per-leg-evidence)) |
...
| RoseTTAFold3 | ubiquitin, L76, MSA | PASS | vendored torch CPU reference, bf16 both sides, shared draws; CA-RMSD X 0.229 Å inside the 0.418 Å reference noise floor (X/floor 0.55) and all-atom X 0.248 Å inside a 0.467 Å floor (X/floor 0.53). Nine of ten seeds reproduce the reference trajectory to 0.085-0.101 Å; seed 4 alone reads 0.763 Å and it is the reference that moved, its own sample sitting 0.86-0.92 Å from its siblings while the device's stays inside the normal device spread — a bifurcation, like `boltz2-prot-nomsa` at seed 0. `scripts/rf3_port/accuracy_cell.py`, five seeds, shipped arm (10 recycles, 50 sampling steps, one diffusion sample) |
| RoseTTAFold3 | 7ROA, L117, MSA | PASS | same harness and convention as the row above; CA-RMSD X 0.203 Å inside the 0.361 Å reference noise floor (X/floor 0.56) and all-atom X 0.265 Å inside a 0.619 Å floor (X/floor 0.43). No outlier seed here: all five X land 0.128-0.359 Å, so the ubiquitin leg's seed-4 bifurcation is a per-seed trajectory property and not a length- or target-independent one |
