...
The third axis is MSA depth. `MSA_PAD_MULTIPLE` used to pad it to a single 1024, so a 35-row
alignment cost exactly what a 1000-row one did. The published 512 aa fixture carries 35 rows,
29.3x padding, and the three units that read the depth axis move 68.4 % of the MSA block's bytes.
The ladder pads instead to the smallest rung of (64, 128, 256, 512, 1024) that holds the depth, and
...
It moves the structure and that was worth getting right. Poison the padded rows with garbage instead
...
| nesso1 | K2 at every rung, all 768 calls; TriMul F1 and the minimal-matmul guard at every rung; **the SDPA q-chunk overflow set is non-empty at 640 and 768** | K2 is dark because `affinity=True` adds a per-row pair-mask slice, which makes the triangle bias `[S, h, S, S]` instead of `[1, h, S, S]`: the kernel exists to read one batch-broadcast mask per head, so it is inapplicable to this path rather than mis-tuned. Since 29b285fb7 only the affinity stack takes that path: the trunk's mask is the outer product of the token mask and now goes through the 1D path, so this row predates the change and Nesso-1's baseline fragment needs re-recording on a p150a Two more size-conditioned gates appear above 512: the transpose headroom gate answers DRAM for 96 of 576 calls at 768, and the pair projections' L1 destination is refused for 16.6 % of calls from 512 up |
