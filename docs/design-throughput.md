...
**To estimate a size we have not measured, cost grows a little slower than the square of the
token count.** Across seven BoltzGen cells on two targets, wall time fits `tokens^1.7` to
`tokens^1.9` with every point inside 11 %. Going from a 512-residue target to a 1536-residue
one is 2.7x the tokens and 6.4x the wall, not the 7.5x a square would give. Tokens here are
target residues plus binder.
