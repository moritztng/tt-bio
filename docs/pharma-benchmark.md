...
**Measured update (Boltz-2 ubiquitin, 2026-07-21): the on-device fp32-diffusion lever was tried and does NOT close the gap.** The boltz-2 reference runs the whole model in fp32 on both CPU and GPU (`Trainer(precision=32)` hardcoded in `src/boltz/main.py`, no bf16 autocast — the `torch.autocast("cuda", enabled=False)` blocks in `diffusion.py`/`attention.py` are defensive no-ops), so the ubiquitin CA-lDDT gap is a genuine fp32-ref vs bf16-device mismatch and the lever is principled to try. A same-session A/B (3 device seeds 0-2, `BOLTZ2_DIFFUSION_FP32_DEVICE=1` vs the bf16 default, vs the committed fp32 CPU reference fixture, `scripts/pharma_parity.py structures --paired --ref-seeds 0 1 2`) measured the 1-lDDT residual GROW, not shrink: bf16 X 0.148 (X/floor 1.69, NO) -> fp32-device X 0.156 (X/floor 1.77, NO); same-seed diagonal == cross in both modes (systematic). Two compounding reasons: (1) the structure TRUNK is fp32 on the reference but bf16 on the device, so the fp32 diffusion score model consumes a bf16-biased trunk z and faithfully propagates that bias (the same pass-3 failure mode the affinity leg showed); (2) the TT fused SDPA kernel is bf16/bf8-only, so the attention softmax — the most precision-sensitive op for local structure — stays bf16 even on the "fp32" path, while the reference runs fp32 attention. The lever that would actually shrink the gap is fp32-matching the TRUNK (and the attention), not the diffusion score model in isolation — a deeper, out-of-scope lift (the 64-block O(L^3) trunk is ~62% of fold). The on-device fp32-diffusion change was reverted (no non-helping change kept); the leg stays PASS-caveated, now a MEASURED irreducible gap for the diffusion-scoped lever, not an assumed dead-end. Perf (warm, L76, same session): bf16 ~17 s/seed, fp32-device ~17 s/seed — no measurable regression at L76 (SDPA and trunk stay bf16). Detail: `~/.coworker/state/tt-bio-boltz2-ubiquitin-diffusion-fp32-device.md`.
