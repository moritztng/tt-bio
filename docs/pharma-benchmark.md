...
| Boltz-2 | ubiquitin, L76, no MSA (not the production default config) | CA-RMSD | 1.84 Å | 1.55 Å | 1.69 ± 0.39 Å | PASS§ |
...
The two structure-leg GAPs (Protenix-v2 HSA ¶¶, Boltz-2 ubiquitin CA-lDDT §) persist under shared random draws, so neither is an RNG-wiring defect. For Boltz-2 ubiquitin, the device's local structure sits about 0.07 lDDT below the reference's seed-to-seed self-consistency while global CA-RMSD and TM-score pass. The pinned Boltz 2.2.1 reference uses `bf16-mixed` for Boltz-2 on both CPU and GPU, so this is not an fp32-reference versus bf16-device mismatch. The § CA-lDDT GAP is on the no-MSA ubiquitin config (not the production default); the MSA-backed ubiquitin leg (§§§, the production default) passes CA-lDDT within the tight floor (X/floor 0.97), so the caveat does not apply to the config a pharma team actually runs.

**Measured update (Boltz-2 ubiquitin, 2026-07-21): forcing the device diffusion to fp32 does not close the gap.** Boltz 2.2.1 selects `Trainer(precision="bf16-mixed")` for Boltz-2, and its diffusion score model remains inside that autocast on both CPU and GPU. The default TT path therefore already matches the upstream precision recipe. In a three-seed exploratory A/B, forcing the device diffusion to fp32 changed 1-lDDT X from 0.148 (X/floor 1.69) to 0.156 (X/floor 1.77); the change was reverted. Warm L76 latency was about 17 seconds per seed in both modes. The leg remains PASS-caveated pending a same-target GPU-reference check of the residual under the production execution device.

**Measured update (GPU-reference self-floor, 2026-07-21): the reference is NOT hardware-invariant at pharma-realistic lengths.** Moritz's directive (2026-07-21): the GPU reference is the highest-value comparison because that is what pharma companies run in production, so prefer it over the CPU reference wherever feasible. The four highest-priority stochastic legs (the three Boltz-2 affinity targets FKBP12/DHFR/trypsin and the Boltz-2 ubiquitin structure leg — the PASS-caveated legs where a GPU reference could in principle close the caveat) had their REFERENCE regenerated on a rented vast.ai RTX 3090 (on-demand) with the pinned official `boltz 2.2.1` and `--no_kernels`, forcing the torch-einsum triangle path that is the SAME kernel the committed CPU reference uses, so only the execution device differs (GPU vs CPU). Seeds 0-4 both sides, identical settings to the committed CPU fixtures. The GPU-reference SELF-FLOOR (R = reference-vs-reference across the 5 seeds, 10 pairwise distances) measured against the committed CPU-reference R:
...
§§§ The Boltz-2 ubiquitin MSA leg (L76, MSA on via the colabfold server, 5 reference + 5 device seeds, seeds 0-4 both sides, 3 recycle / 200 sampling steps / 1 sample — the production default config, same settings as the Boltz-2 7ROA MSA ††† and Protenix-v2 ubiquitin MSA ¶ legs). The reference was generated fresh on a rented vast.ai RTX 3090 (on-demand) with the pinned official boltz 2.2.1, `--use_msa_server` for the colabfold MSA (api.colabfold.com; uniref 9655 seqs + bfd/mgnify30/metaeuk30/smag30 12014 seqs, ~21669 combined) and `--no_kernels` to force the torch-einsum triangle path — the SAME kernel the committed CPU fixtures use, so only the execution device differs (GPU vs CPU); per-seed fold ~14-18s after the seed-0 CCD/weights download. The device folds the identical MSA (tt-bio `compute_msa` calls the same colabfold server for the same sequence; header-set diff = 0, verified), ran live on qb1 card 1 (p150a), ~16-23s/seed. The boltz-2 worker seed fix (`"seed": seed or 0` in the boltz-2 `worker_cfg` in `tt_bio/main.py`) is present on this branch, so the spawned-worker diffusion is properly seeded and the same-seed diagonal is a valid test. All four metrics pass within the tight MSA-backed GPU-reference floor (floor = max(R, D), within = X ≤ floor + max(R.std, D.std)): CA-RMSD X 1.586 ± 0.243 Å vs R 1.542 Å (std 0.265, 10 ref-seed pairs) / D 1.447 Å (std 0.242, 10 dev-seed pairs), X/floor 1.03, within floor YES; 1-PCC X 0.0096 ± 0.003 vs R 0.0091 / D 0.0080, X/floor 1.05, within floor YES; 1-TM X 0.0210 ± 0.005 vs R 0.0204 / D 0.0184, X/floor 1.03, within floor YES; 1-lDDT X 0.0768 ± 0.016 vs R 0.0706 / D 0.0790, X/floor 0.97, within floor YES — the 1-lDDT metric that GAPped on the no-MSA § config (X/floor 1.76) now passes, because the MSA-backed reference floor is properly measured here and the device lands within it. The same-seed diagonal is not smaller than the all-pairs cross on any metric (CA-RMSD diag 1.720 vs cross 1.586; 1-lDDT diag 0.0807 vs cross 0.0768; n=5), so the residual is systematic bf16 arithmetic divergence, not an RNG-wiring defect — the same precision-floor family as the other stochastic legs, absorbed by the floor here. Confidence agreement is tight (device mean − reference mean: ptm −0.0018, confidence_score −0.0010). Per-seed reference ptm 0.9244/0.9229/0.9214/0.9233/0.9251, confidence_score 0.9338/0.9338/0.9301/0.9327/0.9342 — far tighter than the no-MSA ubiquitin basin (ptm 0.812-0.874), confirming the MSA-backed reference is markedly more self-consistent, as expected for a tight-floor leg. This is the GPU-reference MSA companion Moritz asked for (2026-07-21 directive: the GPU reference is the highest-value comparison because that is what pharma runs in production); the no-MSA § caveat does not apply to this production-default config. Fixture: `docs/pharma-benchmark-data/ref-fixtures/boltz2/ubiquitin/msa-colabfold_200step_1sample_3recycle_bf16_gpu/`; JSON: `docs/pharma-benchmark-data/boltz2-ubiquitin-msa-paired-5x5.json`. Reproduce: `python3 scripts/pharma_parity.py structures --ref-fixtures boltz2/ubiquitin/msa-colabfold_200step_1sample_3recycle_bf16_gpu --dev-dirs <dev_seed0..4>/boltz_results_ubiquitin_msa --paired`.
...
¶¶ The Protenix-v2 HSA leg (L585, MSA, 5 reference + 5 device seeds, seeds 0-4 both sides, n_cycle=10/n_step=200/n_sample=5, bf16, the same production settings as the 7ROA/ubiquitin protenix legs): the device-vs-reference CA-RMSD is 1.03 ± 0.17 Å (n=25 cross pairs), ABOVE the floor max(R, D) = 0.70 Å (R 0.70 Å over 10 ref-seed pairs; D 0.38 Å over 10 dev-seed pairs; X/floor 1.47, NOT within floor on 1-PCC either) -> GAP. HSA (PDB 1AO6, 585 res, 3-domain) is the first L300-800 pharma-realistic target on the protenix leg. The reference was generated on a vast.ai RTX3090 GPU (CPU infeasible at L585) with the pinned protenix 2.0.0 / commit c3bfc365 and torch triangle kernels (the SAME kernel as the qb2 CPU ref for the other protenix legs); per-seed ~5.5 min. The protenix-v2 checkpoint (1.86GB) was streamed from qb2 because the protenix OSS /checkpoint/ path is now AccessDenied (403) for unsigned requests (it worked on qb2 Jul 13; blocked Jul 19); the data cache (components.cif + rdkit_mol.pkl) downloaded fine from /common/. The device ran live on qb1 card 0 (p150a), ~4-5 min/seed. The GAP is a tight-floor effect, not a bad structure: the absolute X (1.03 Å) is a good fold for a 585-res multi-domain target (sub-Å-to-low-Å CA-RMSD is excellent at this length), but the GPU bf16 reference is unusually self-consistent (R 0.70 Å, an order of magnitude tighter than the L76 ubiquitin protenix R 1.92 Å) because the bf16 GPU diffusion collapses to a narrow basin, and the p150a device collapses even tighter (D 0.38 Å); the device and reference land in DIFFERENT tight basins ~1 Å apart, so X exceeds each self-floor. This is the same 'bf16 device collapses to a narrower basin' property documented for the ubiquitin protenix leg, but at L585 the ref floor is so tight that X (1.03 Å) no longer fits inside it (ubiquitin: X 1.73 < floor 1.92 -> PASS; HSA: X 1.03 > floor 0.70 -> GAP). JSON: docs/pharma-benchmark-data/protenix-v2-hsa.json. The CA-RMSD GAP is measured directly as a same-seed diagonal above (X_diag 1.007 vs X_all 1.025, n=5): matching the seed does not collapse the residual, so it is systematic, not a port defect in the RNG wiring or sampler.

**Precision root cause (source-grounded, 2026-07-21):** the residual is NOT "two bf16 diffusions diverging by NVIDIA-vs-Tenstorrent hardware reduction order" as the prior framing implied — reading the pinned protenix 2.0.0 / commit c3bfc365 source shows the GPU reference does NOT run the diffusion in bf16. The reference default config sets `skip_amp.sample_diffusion: True` (protenix/configs/configs_base.py:138), and `protenix/model/protenix.py` wraps the entire diffusion sampling — conditioning cache (pair_z, p_lm/c_l, lines 538/543), atom attention encoder, token DiT, atom decoder (line 806) — in `autocasting_disable_decorator(...)`, i.e. autocast DISABLED = fp32, on top of the outer bf16 inference autocast. The diffusion score model additionally upcasts its inputs unconditionally (`protenix/model/modules/diffusion.py:454,466-472`). So the reference diffusion is fp32; only the trunk pairformer runs in bf16 autocast (with triangle-ops/softmax/distogram forced fp32 internally via `autocast("cuda", enabled=False)` in `protenix/model/triangular/layers.py` and `primitives.py`). The Tenstorrent port runs the entire diffusion in bf16 on device (`tt_bio/protenix.py`, `device_dit=True` hardcoded at line 408; atom encoder/transformer/decoder + token DiT all on-device bf16 HiFi4; only the EDM preconditioning math and the hoisted conditioning input upcasts are host fp32). The GAP is therefore a matchable selective-fp32 boundary mismatch — the SAME family as the Boltz-2 affinity/ubiquitin legs (CPU fp32 reference vs bf16 device) — not an irreducible hardware-divergence artifact with no reference-side fp32 boundary to match. The positive control is the Boltz-2 HSA GPU leg above (§§), which PASSES at the same target length: boltz's GPU reference does NOT disable autocast on the diffusion score model (`boltz/model/modules/diffusion.py:522,648` wrap ONLY the rigid-align step in `autocast("cuda", enabled=False)`, leaving the score model in bf16 autocast), so the boltz GPU ref diffusion is bf16-ish and matches the device bf16 — a bf16-ref-diffusion leg passes the tight-floor local metric while a fp32-ref-diffusion leg (protenix HSA) GAPs. Closing the protenix HSA GAP cleanly would need the device diffusion run in fp32 (a full host-fp32 diffusion fallback, gated default OFF by analogy to `BOLTZ2_AFFINITY_DIFFUSION_FP32_HOST`); this is a substantial code lift and its A/B at L585 is infeasible in a single pass (CPU fp32 diffusion at L585 is multi-hour/seed, the reason the reference itself used a GPU), so it is recorded as the identified next lever rather than applied blind — the Boltz-2 affinity pass-3 caution (fp32 host diffusion GROWS X when the trunk z feeding it is bf16-biased) means an unverified apply is not principled. The protenix case may be cleaner than the boltz affinity case because the protenix trunk runs bf16 on BOTH sides (matched), so the fp32-diffusion lever is expected to help once wired, but that is a hypothesis for the next pass, not a measured result. Precision map and per-component mismatch table: `~/.coworker/state/tt-bio-pharma-benchmark-selective-fp32-investigation.md`.
