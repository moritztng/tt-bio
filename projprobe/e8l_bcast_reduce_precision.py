#!/usr/bin/env python3
"""E8k -- is unary_bcast exact in fp32, i.e. does a precision-safe broadcast exist at all?
...
def main():
    ...
    try:
        ...
        cases = (
            ...
            ("compute_e8_sfpuprec.cpp", 3, "BCAST unary_bcast ROW", a[0:1, :].expand(32, 32)),
            ("compute_e8_sfpuprec.cpp", 4, "BCAST unary_bcast COL", a[:, 0:1].expand(32, 32)),
            ("compute_e8_sfpuprec.cpp", 5, "REDUCE row, fp32-enforced", None),
            ("compute_e8_sfpuprec.cpp", 6, "REDUCE row, not enforced", None),
        )
        for kern, op, name, exact in cases:
            if exact is None:                       # reduce: the answer lands in column 0
                exact = torch.zeros(32, 32)
                exact[:, 0] = a.sum(dim=1)
    ...
    (HERE / "e8l_bcast_reduce_precision.json").write_text(json.dumps(res, indent=1))
