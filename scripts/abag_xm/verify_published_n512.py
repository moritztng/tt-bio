...
# Exit non-zero when a check fails. This printed 58 FAIL lines and returned 0, which is the
# one behaviour a gate must never have: a caller that checks the exit code -- every watchdog
# and every runbook line in this campaign does -- logs it green. Found while re-verifying the
# published N=512 numbers after the 164-target backfill (workstream
# abag-xm-panel-complete-164).
import sys as _sys
