...
#
# The pass-260 ZERO_MIN third leg ("0-byte log = pre-banner spin hang") was REMOVED
# (2026-08-07, workstream abag-xm-panel-to-100pct): tt-bio predict writes nothing to a
# redirected stdout until the run finishes (every successful p29 protenix log is one
# final frame), so a 0-byte log carries no information and the leg killed every fold
# slower than its threshold, healthy or not -- 9d73 c0/c1/c2 were made permanent that
# way. The two-leg no-progress rule (CPU delta < MIN_CPU_S AND no log growth for
# STALL_MIN) is the valid hang detector: a computing fold never trips it. The engine
# fix (plain timestamped progress lines on a non-TTY) makes logs grow again.
set -u
H=$HOME/mthuening
SRC=$H/deepn_src
PREV=$H/p28
...
guarded_fold() { # <logfile> <chip> <cmd...> -- setsid launch + stall/cap group kills + quarantine
  ...
  # CAP_S default 57600 (16h): the slowest legitimate fold on record (od 9gvn c2)
  # exceeded 21600 three times while its siblings took ~2280s, i.e. >9.5x; 16h gives
  # ~25x headroom over the sibling median and is still a real bound.
...
attempt_cap_hit() { # <model> <target> <rung> <seed> <chunk> -- true after 3 failed attempts
...
try:
    for line in open(path):
        ...
        if (r.get("model"), r.get("target"), str(r.get("rung")), str(r.get("seed")),
                str(r.get("chunk"))) == (m, t, rung, seed, c) and r.get("rc") not in (0, "0"):
            n += 1
...
slot() {
  ...
  for ((idx=1; idx<=n; idx++)); do
    ...
    # Attempt cap: 3 failed attempts -> terminal rc=125 marker, so a genuinely
    # bad input cannot loop forever.
    if attempt_cap_hit "$model" "$t" "$rung" "$seed" "$c"; then
      record "$model" "$t" "$rung" "$seed" "$c" "$k" na "$chip" 125 0 0 0 0
      continue
    fi
