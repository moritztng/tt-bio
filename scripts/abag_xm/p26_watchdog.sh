#!/bin/bash
# Restore watchdog for the p25/p25b/p26/p26b galaxy windows (deep-N saturation).
# Pattern copied from p25_watchdog.sh (proven): restore when ALL fleets finish, or at the
# deadline regardless. Run pattern HARDCODED with bracket-glob so pgrep never matches this
# script's own command line.
RUNPAT='[p]2[56].*_fleet'
MIN_MIN=330
DEADLINE_MIN=580
LOG=/home/cust-team/mthuening/p26_watchdog.log
