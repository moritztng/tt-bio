#!/bin/bash
# Restore watchdog for the p28 galaxy window (deep-N saturation, window 4).
...
# ~1300 card-h ~ 41h on 32 chips).
RUNPAT='[p]28_fleet'
MIN_MIN=720
DEADLINE_MIN=2700
LOG=/home/cust-team/mthuening/p28_watchdog.log
...
"); do kill -TERM "$p" 2>/dev/null; done
  ...
  for p in $(pgrep -f 'mthuening/p28'); do
    [ "$p" = "$$" ] && continue
