#!/bin/bash
# p28 window watchdog: on fleet completion (or safe deadline) restore the prod fold
...
# Deadline bounds the worst case (link gate tripped -> all 2592 chunks fold fresh,
# ~1300 card-h ~ 41h on 32 chips; the hardened runner caps any single hang at ~47min).
B=$HOME/mthuening/p28
DEADLINE=$(( $(date +%s) + 162000 ))   # 45h
while true; do
  sleep 600
  if grep -q P28_DONE $B/results.jsonl 2>/dev/null; then
...
echo "$(date -Is) prod worker respawned pid=$!" >> $HOME/mthuening/p28_restore.log
