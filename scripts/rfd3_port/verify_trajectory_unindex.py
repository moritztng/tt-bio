"""Multi-step trajectory parity on a REAL from-PDB input WITH an unindexed
motif (F6 unindex field, p14): same real IAI protein + contig as
verify_trajectory_from_pdb.py, PLUS `unindex="A100-101,A120"` (a tied
2-residue island + a separate singleton island, appended at the end of the
token list — value-parity-verified bit-exact vs a real reference capture in
scripts/rfd3_port/parity_artifacts/parity_unindex.py).

This checks the on-device RFD3DiffusionModule numerically agrees with the
vendored torch reference on an input that actually exercises
`unindexing_pair_mask` through the RPE (blocks.py: d_token/d_residue clamped
to the max bin wherever the mask is True) — not just the featurizer output.

Usage:
  TT_VISIBLE_DEVICES=0 python3 scripts/rfd3_port/verify_trajectory_unindex.py [num_timesteps]
"""
import os, sys, torch
...
def main(n_ts):
    ...
    print(f"[setup] real IAI-protein + unindex f: I={I} L={L} "
          f"({n_unindexed} unindexed tokens, {int(is_motif.sum())} motif atoms)")
    ...
    print(f"[motif] final motif-atom coords == seeded motif_pos (incl. unindexed): ref={motif_seed_ok_ref} device={motif_seed_ok_tt}")
    if worst < 0.97 or pf < 0.97:
        raise AssertionError(f"trajectory parity failed on the unindex input: "
                              f"worst-step PCC={worst:.6f} final PCC={pf:.6f}")
    if not (motif_seed_ok_ref and motif_seed_ok_tt):
        raise AssertionError("motif atoms (incl. unindexed) moved away from their seeded ground-truth position")
    print("TRAJECTORY PARITY OK on the unindex input (per-step PCC >= 0.97, "
          "motif+unindexed atoms verified fixed at their true position on both backends)")
