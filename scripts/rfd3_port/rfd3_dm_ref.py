...
# relying on the network to keep it symmetric. FIXED_ENTITY_ID=-1 marks atoms outside
# any symmetric group: real motif/ligand atoms replicated per-subunit but held fixed
# (never re-symmetrized every step, since their per-replica coordinate was already
# baked in once at feature-build time) or excluded from symmetrization entirely via
# `is_unsym_motif` -- reachable since p19's F5+motif combination (see
# tt_bio.rfd3_featurize's F5+motif grounding), verified device-vs-reference on real
# `unsym_C3_6t8h`/`unindexed_C2_1j79` fixtures.
_FIXED_ENTITY_ID = -1
