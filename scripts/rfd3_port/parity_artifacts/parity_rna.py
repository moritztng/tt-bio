...
RNA-binder case (p22, symmetric to parity_dna.py's DNA case, owed since p15):
1q75.pdb (single-strand RNA, chain A, residues 1-15, real A/C/G/U bases),
contig 'A1-15,/0,5' (RNA target fixed in space + a short designed protein
binder — same structure as the real 'RNA_basic' reference example, with a
deterministic designed length instead of a range to avoid RNG ambiguity).
...
Reproduce the reference capture (CPU, no ckpt — same method as parity_dna.py):
    ...
    /tmp/fndry_venv/bin/python scripts/rfd3_port/capture_ref_f.py \
        --pdb scripts/rfd3_port/parity_artifacts/dsdna_basic/1q75.pdb \
        --contig "A1-15,/0,5" \
        --out_dir /tmp/ref_rna_capture

Known, documented gap (NOT a mismatch): `ref_pos` for RNA atoms is real
reference-conformer 3D geometry from RDKit/CCD-template embedding, which this
port does not vendor — left at 0 (same shape of gap as parity_dna.py's DNA
case). Reported separately, not counted in MISMATCHES.
"""
import os, sys
import torch

PDB = os.path.join(os.path.dirname(__file__), "dsdna_basic", "1q75.pdb")
CONTIG = "A1-15,/0,5"
REF_PT = "/tmp/ref_rna_capture/ref_f.pt"
