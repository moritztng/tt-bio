#!/usr/bin/env python3
"""p73 -- the L2 fold gate and A/B: `RFD3_DENSE_BIAS_FUSED` on the whole design.

Same harness as p68. One process, one lease, one benchlock hold, one discarded warmup fold, arms
interleaved so card warmth cannot be mistaken for a lever. The three landed levers stay ON in both
arms -- this scores L2 against the 102.662 s/design baseline p68 established, not against 116.

Run it twice. First at 3 timesteps: that is the digest gate, and it exists because
`tt-bio-ttnn-slice-not-a-view-and-allocation-order-sensitivity` records a chunkwise `torch.equal`
passing while the fold digest moved. p72 already has torch.equal at four shapes; this is the
end-to-end check that the wiring feeds the kernel the tensors it was gated on. Then at 200: that
is the A/B, and the A/A control is the two consecutive off arms it opens with.
    ...
    ~/.coworker/scripts/benchlock.sh rfd3-b8-to-4x-p2 -- env TT_VISIBLE_DEVICES=1 \
      ...
      /home/ttuser/tt-bio-dev/env/bin/python3 -u scripts/rfd3_port/p73_dense_fold_ab.py \
          perf/p73/smoke3.json 3 off,on,off,on
"""
import hashlib
import json
import os
...
OUT = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else "perf/p73/dense_fold_ab.json")
...
SEED = 42
PREDICTED_DELTA_S = -2.200      # p72: 0.5093 -> 0.2038 ms/call x 36 x 200 steps
...
def main():
    ...
    print("[p73] steps=%d card=%d arms=%s  predicted delta %+.3f s/design"
          % (STEPS, CARD, ",".join(ARMS), PREDICTED_DELTA_S * (STEPS / 200.0)), flush=True)

    rfd3_bias.set_dense_enabled(False)
    s, dig, n = fold(specs, "/tmp/rfd3_p73_warm")
    print("[p73] warmup fold %.3f s (%s), discarded" % (s, dig[:20]), flush=True)
    warm = round(s, 3)

    rows = []
    for i, arm in enumerate(ARMS):
        rfd3_bias.set_dense_enabled(arm == "on")
        before = rfd3_bias.DSTATS[0]
        s, dig, n = fold(specs, "/tmp/rfd3_p73_%d" % i)
        served = rfd3_bias.DSTATS[0] - before
        rows.append({"arm": arm, "rep": i, "sampler_s": round(s, 3),
                     "cif_sha256_16": dig, "n_cifs": n, "kernel_calls": served})
        print("[p73] rep%d %-3s %9.3f s  %d cif  %s  kernel calls %d"
    ...
    print("[p73] arm provenance: on served %s, off served %s -> %s"
    ...
    OUT.write_text(json.dumps({
        ...
        "flag": "RFD3_DENSE_BIAS_FUSED", "arms_distinct": arms_real,
