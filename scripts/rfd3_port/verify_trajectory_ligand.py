"""Multi-step trajectory parity on a REAL from-PDB F3 small-molecule-binder
input (p16): IAI.pdb (RosettaCommons/foundry's own `sm_binder_design.md`
"buried" example, real ligand IAI) + a pure designed-length (180) protein
binder — value-parity-verified 42/43 keys bit-exact vs a real reference
capture in scripts/rfd3_port/parity_artifacts/parity_ligand.py (the lone gap,
`ref_pos`'s stochastic RDKit reference-conformer draw, does not affect this
trajectory check since it feeds the model as ligand-identity conditioning,
not a ground-truth position -- the real reference example itself leaves the
ligand's COORDINATE unfixed too, per its own `select_fixed_atoms: {"IAI":
""}`).

This checks the on-device RFD3DiffusionModule numerically agrees with the
vendored torch reference on a real ligand-bearing input (per-atom atomized
ligand tokens flowing through the SAME TokenInitializer + DiffusionModule as
protein/NA), not just the featurizer output.

Usage:
  TT_VISIBLE_DEVICES=0 python3 scripts/rfd3_port/verify_trajectory_ligand.py [num_timesteps]
"""
import json
import os, sys, torch
...
PDB = os.path.join(DIR, "parity_artifacts", "ligand_iai", "IAI.pdb")
SPEC_JSON = os.path.join(DIR, "parity_artifacts", "ligand_iai", "spec_buried.json")
...
def main(n_ts):
    ...
    print(f"[setup] real ligand-binder f: I={I} L={L} ({n_lig} ligand atom-tokens, "
          f"{int(is_motif.sum())} fixed-coord atoms — this example's own "
          f"select_fixed_atoms leaves the ligand coordinate diffused, per the "
          f"real reference's own documented 'buried' example)")
    ...
    if int(is_motif.sum()) > 0:
        ...
        print("[motif] no fixed-coord atoms in this spec (matches the real reference's own "
              "'buried' example — the ligand's position, not just its identity, is diffused)")
    if worst < 0.97 or pf < 0.97:
        raise AssertionError(f"trajectory parity failed on the ligand-binder input: "
                              f"worst-step PCC={worst:.6f} final PCC={pf:.6f}")
    print("TRAJECTORY PARITY OK on the small-molecule-binder input (per-step PCC >= 0.97, "
          "atomized ligand tokens flow correctly through TokenInitializer + DiffusionModule "
          "on both backends)")
