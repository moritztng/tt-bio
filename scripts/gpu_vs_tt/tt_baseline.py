...
def measure(model: str, repeat: int, msa_dir: Path, out_path: Path,
    ...
    cold_cif = sha_structures(Path(meta["struct_dir"]))
    ...
    for _ in range(repeat):
        t, m = one_fold()
        times.append(t)
        # Per-fold plDDT and CIF digest, not just the cold fold's. Recording only
        # cold_metrics is what left a 0.30 plDDT gap between two hosts unexplainable.
        warm_folds.append(dict(fold_s=round(t, 3), plddt=m.get("plddt"),
                               ptm=m.get("ptm"), n_tokens=m.get("n_tokens"),
    ...
    result = dict(
        ...
        **_card_info(),
        ...
        cold_metrics=cold_metrics, cold_cif_sha256=cold_cif,
