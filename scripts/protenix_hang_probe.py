    ...
    for _ in range(6):
        ...
        for pid in holders:
            try:
                ...
                pass
    time.sleep(5)
    left = device_holders(card)
    # A hung fold leaves the CARD dirty, not just the process table. Killing the
    # holders is not enough: the next fold then stalls in _open_and_init_device /
    # _assert_local_dispatch and the probe scores it as another hang, which is the
    # same non-independent-sample trap release_gate.py hit with its trials 4-5.
    # Reset the card so every trial after a hang starts from a clean device.
    reset = sh(f"~/.local/bin/tt-smi -r {card} 2>&1", timeout=300)
    ok = ("Reset" in reset or "reset" in reset) and "Traceback" not in reset
    time.sleep(10)
    return {"killed_holders": killed, "holders_after": left,
            "card_reset_ok": ok, "card_reset_tail": reset[-300:]}
