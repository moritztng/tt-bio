#!/usr/bin/env bash
# abag_xm_opendde_qb2.sh — run the opendde-abag Tier-A leg on qb2 (4 cards) in
# parallel with qb1's protenix-v2+boltz2 fanout. opendde-only so the two hosts
# never overlap (qb1 does protenix+boltz2; qb2 does opendde). Writes to qb2's
# host-local ~/abag_xm/tier_a/ (no shared mount to qb1). Merge later: rsync
# qb2:~/abag_xm/tier_a/opendde_abag/ -> qb1:~/abag_xm/tier_a/opendde_abag/ and
# append qb2's ok opendde records to qb1's progress.jsonl (result_dir paths
# match after rsync since both use ~/abag_xm/tier_a/opendde_abag/).
#
# Uses the fixed (unpaired-only) OpenDDE path — paired_a3ms=None, commit ac0e339a.
# MSA cache (354 a3m) + UNIREF30_READY marker present on qb2; cache hits skip the
# DB so no 1.3 TB DB needed. --msa_db_path passed defensively (a miss would raise
# loudly, never go online).
set +u
...
launch_card(){ local card="$1"; local targets="${SUBSETS[$card]}"
  TT_VISIBLE_DEVICES=$card TT_BIO_LEASE_HOLDER=$LEASE PYTHONPATH=$WT \
    ...
    --timeout "$TIMEOUT" --models opendde-abag \
    >> /tmp/abag_opendde_qb2_card$card.log 2>&1 < /dev/null &
  echo "$!"; }
kill_all(){ for c in $(pgrep -f "python3 -m tt_bio.main predict" 2>/dev/null); do kill -INT "$c" 2>/dev/null; done; sleep 6
  ...
  sleep 2; rm -f ~/.coworker/state/leases/tt-quietbox2-card*.json; }
...
if [ "$nrun" -gt 0 ]; then
  log "ABORT: $nrun generate.py already running on qb2"; exit 1; fi
