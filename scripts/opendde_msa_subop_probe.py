...
def stage9(args):
    ...
    if args.sub in ("triatt", "both", "bothfix"):
        T._tri_att_sdpa = host_core
    # `opmfix` / `bothfix` are NOT oracle substitutions: they are the real one-line fix,
    # `OuterProductMean.scale_bias=True`, which is the reference's own form (divide the
    # whole linear_out output by the pair norm, bias included). Everything stays on device.
    if args.sub in ("opmfix", "bothfix"):
        for b in range(N_MSA_BLOCKS):
...
def stage4(args):
    ...
    def wrapped(self, s, z, *a, **kw):
        if self is p.trunk.PF and "z_in" not in taps:
            ...
            # STEP B: the shipped z3 as the device holds it, tile padding and all.
            # `p._to_host` returns the LOGICAL values only, so re-uploading it zero-fills
            # the 24 invented columns triangle attention reads. A device-side clone does not.
            taps["z_in_dev"] = ttnn.clone(z)
            full = z.to_torch_with_padded_shape().float()
            n = int(z.shape[-2])
            pad = full[..., n:, :].reshape(-1)
            print(f"  shipped z3 logical {tuple(z.shape)} padded {tuple(z.padded_shape)}: "
                  f"tile-padding rows {n}..{full.shape[-2]} rms="
                  f"{float(pad.pow(2).mean().sqrt()):.4e} maxabs={float(pad.abs().max()):.4e} "
                  f"nonzero={int((pad != 0).sum())}/{pad.numel()}", flush=True)
    ...
    up = p.trunk._up(z_in.reshape(1, n, n, -1))
    full = up.to_torch_with_padded_shape().float()
    pad = full[..., n:, :].reshape(-1)
    print(f"  host round-trip upload: tile-padding rms="
          f"{float(pad.pow(2).mean().sqrt()):.4e} nonzero={int((pad != 0).sum())}/{pad.numel()}",
          flush=True)
    ttnn.deallocate(up)

    for arm in args.arms.split(","):
        # L0 is the STEP B arm: drive the harness chain from the shipped path's OWN device
        # tensor. Same logical values as L1, same 48 sub-op sequence; the only difference
        # left is what sits in the tile padding.
        zt = (ttnn.clone(taps["z_in_dev"]) if arm == "L0"
              else p.trunk._up(z_in.reshape(1, n, n, -1)))
...
def stage11(args):
    """STEP A: is OuterProductMean's error a token-axis padding leak, or the proj_o bias?

    Stage 10 ruled out the row axis (flat 2.3-2.6% norm inflation from 32 to 76 rows, and
    ttnn does not pad that dim at all). The token axis is the one it padded at every rung,
    136 -> 160, so it was held fixed and never tested. Same discriminator as stage 7: a
    ladder that crosses the tile boundary. A leak dies at a crop that is already a multiple
    of 32, a scale error does not.

    Second arm in the same run, because the source read says the answer is a scale error and
    names it: OpenDDE's reference (`opendde/model/triangular/layers.py` OuterProductMean)
    divides the WHOLE `linear_out` output by the pair norm, bias included, while
    `OuterProductMean.scale_bias` defaults to False and adds `proj_o.bias` at full strength.
    At 76 MSA rows that is the bias 76x too large. `scale_bias=True` is the reference's own
    form, so flipping it on the same op at the same input prices the hypothesis directly.
    """
    import ttnn
    cache = Path(args.cache)
    m0 = torch.load(cache / "ref_m0.pt").float()
    model, p, state_dict = _dev_setup()
    reference, _ = _load_reference(state_dict)
    ref_blk = reference.msa_module.blocks[args.src_block]
    dev_opm = p.trunk.MSA[args.src_block][0]
    rows = m0.shape[0]
    o_bias = ttnn.to_torch(dev_opm.o_bias).float().reshape(-1)
    print(f"m0 {tuple(m0.shape)} rows={rows} block={args.src_block}", flush=True)
    print(f"proj_o.bias rms={float(o_bias.pow(2).mean().sqrt()):.4e} "
          f"unscaled-vs-scaled excess factor=(1 - 1/{rows})={1 - 1 / rows:.4f}", flush=True)
    for crop in [int(x) for x in args.crops.split(",")]:
        mh = m0[:, :crop, :].contiguous()
        ref_u = ref_blk.outer_product_mean_msa(mh, inplace_safe=False, chunk_size=None)
        print(f"\n--- crop {crop} (tile-padded to {-(-crop // 32) * 32}) ---", flush=True)
        got = {}
        for flag in (False, True):
            dev_opm.scale_bias = flag
            mt = p.trunk._up(mh.reshape(1, *mh.shape))
            u = dev_opm(mt, None, None)
            got[flag] = p._to_host(u, tuple(ref_u.shape)).float()
            _stat(f"crop{crop} opm scale_bias={flag} (padded {tuple(mt.padded_shape)})",
                  got[flag], ref_u)
            ttnn.deallocate(u)
            ttnn.deallocate(mt)
        dev_opm.scale_bias = False
        # Attribution: the whole difference between the two arms should be the constant
        # per-channel offset `proj_o.bias * (1 - 1/rows)`, broadcast over every (i, j).
        delta = got[False] - got[True]
        pred = (o_bias * (1 - 1.0 / rows)).expand_as(delta)
        _stat(f"crop{crop} (scale_bias off - on) vs predicted bias offset", delta, pred)
        # And the shipped arm's error should be that same offset, up to the bf16 floor.
        _stat(f"crop{crop} shipped opm error vs predicted bias offset",
              got[False] - ref_u, pred)
...
def main() -> None:
    ...
    {1: stage1, 2: stage2, 3: stage3, 4: stage4, 5: stage5, 6: stage6, 7: stage7,
     8: stage8, 9: stage9, 10: stage10, 11: stage11}[args.stage](args)
