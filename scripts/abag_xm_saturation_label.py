#!/usr/bin/env python3
"""AbAg-XM oracle-saturation trailing labeler (state doc §6 Step 4).

Per-sample DockQ ONLY (`--per_sample_only`): every deliverable of this campaign is a
best-of-N / ranked-top-1 statistic over per-sample DockQ, and the pairwise matrix is
quadratic (~34 CPU-h per target at N=1000). Runs host-local over this host's own fold
tree — labels.json lands next to each fold and the analysis pass reads both hosts.

Loop: scan {opendde,protenix,boltz2}/<target[_c<j>]> for complete, unlabeled folds,
label at most --workers concurrently, sleep, repeat. Exits when the CAMPAIGN_DONE
marker exists and a full scan finds nothing left.

  cd $WT && setsid nohup python3 -u scripts/abag_xm_saturation_label.py --workers 3 \
      </dev/null >>~/abag_xm/saturation/logs/label.log 2>&1 &
"""
import argparse, os, json, subprocess, time
from concurrent.futures import ThreadPoolExecutor
from pathlib import Path

WT = Path(__file__).resolve().parent.parent
BASE = Path.home() / "abag_xm" / "saturation"
GT = Path.home() / "abag_xm" / "ground_truth"
DONE_MARKER = BASE / "CAMPAIGN_DONE"
MODEL_DIRS = ("opendde", "protenix", "boltz2")
LABEL_VENV_PY = Path.home() / ".abag_xm_label_venv" / "bin" / "python3"
SHARED_VENV = Path("/home/ttuser/tt-bio-dev/env")  # read-only: DockQ deps live here
THREAD_CAPS = ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS",
               "NUMEXPR_NUM_THREADS", "VECLIB_MAXIMUM_THREADS")
SLEEP_S = 180
LABEL_TIMEOUT_S = 21600


def label_env():
    sp = next(iter(SHARED_VENV.glob("lib/python*/site-packages")), None)
    pp = ":".join(str(x) for x in [sp, WT] if x)
    env = {**os.environ, "PYTHONPATH": pp,
           "PATH": str(Path.home() / ".local" / "bin") + os.pathsep + os.environ.get("PATH", "")}
    env.update({v: "2" for v in THREAD_CAPS})
    return env


def expected_cifs(out_dir):
    return 500 if "_c" in out_dir.name else 1000


def fold_complete(rd, n_exp):
    try:
        data = json.loads((rd / "results.json").read_text())
        rec = data[0] if isinstance(data, list) else data
        if rec.get("status") != "ok":
            return False
        st = rd / "structures"
        return st.is_dir() and len(list(st.glob("*.cif"))) == n_exp
    except Exception:
        return False


def pending_folds():
    todo = []
    for model in MODEL_DIRS:
        mdir = BASE / model
        if not mdir.is_dir():
            continue
        for out_dir in sorted(p for p in mdir.iterdir() if p.is_dir()):
            rds = list(out_dir.glob("*_results_*"))
            if not rds:
                continue
            if (out_dir / "labels.json").exists() or (out_dir / ".label_lock").exists():
                continue
            if fold_complete(rds[0], expected_cifs(out_dir)):
                todo.append((out_dir, rds[0]))
    return todo


def label_one(out_dir, rd):
    target = rd.name.split("results_")[1]
    lock = out_dir / ".label_lock"
    lock.write_text(str(os.getpid()))
    t0 = time.time()
    try:
        cmd = [str(LABEL_VENV_PY), str(WT / "scripts" / "abag_xm_labels.py"), str(rd),
               str(GT / f"{target}.cif"), str(WT / "examples" / "abag_xm" / f"{target}.yaml"),
               "--out", str(out_dir / "labels.json"), "--per_sample_only"]
        try:
            r = subprocess.run(cmd, capture_output=True, text=True, env=label_env(),
                               timeout=LABEL_TIMEOUT_S)
            ok = r.returncode == 0 and (out_dir / "labels.json").exists()
            tail = (r.stderr or r.stdout or "").strip().splitlines()[-1:] or [""]
        except subprocess.TimeoutExpired:
            ok, tail = False, [f"timeout after {LABEL_TIMEOUT_S}s"]
        print(f"{out_dir.parent.name}/{out_dir.name} "
              f"{'ok' if ok else 'FAILED'} {time.time() - t0:.0f}s {tail[0][:160]}", flush=True)
    finally:
        lock.unlink(missing_ok=True)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--workers", type=int, default=3,
                    help="concurrent single-threaded label processes")
    ap.add_argument("--once", action="store_true", help="one scan, then exit")
    a = ap.parse_args()
    print(f"saturation labeler: workers={a.workers} base={BASE}", flush=True)
    while True:
        todo = pending_folds()
        if todo:
            print(f"scan: {len(todo)} pending -> "
                  + ",".join(f"{o.parent.name}/{o.name}" for o, _ in todo), flush=True)
            with ThreadPoolExecutor(max_workers=a.workers) as ex:
                list(ex.map(lambda t: label_one(*t), todo))
        if a.once or (not todo and DONE_MARKER.exists()):
            print("labeler done", flush=True)
            return
        time.sleep(SLEEP_S)


if __name__ == "__main__":
    main()
