# Reference Protenix-v2 NO-MSA control: folds the full 7ROA construct (the a3m
# query row of examples/msa/seq2.a3m) with use_msa=False, proper sampling, then
# Kabsch CA-RMSD vs 7ROA matched by label_seq (pred position == entity label_seq
...
OUT_CIF = os.environ.get('OUT_CIF', '/tmp/protenix_ref_construct_nomsa.cif')
INPUT_JSON = '/tmp/protenix_prot_construct_nomsa.json'
...
print('building v2 model (use_msa=False, no-MSA control)...', flush=True)
m = Protenix(cfg).eval()
...
print('='*60, flush=True)
print('REFERENCE Protenix-v2 NO-MSA control, full 136-res construct', flush=True)
...
print('='*60, flush=True)
print('REF_CONSTRUCT_NOMSA_DONE', flush=True)
