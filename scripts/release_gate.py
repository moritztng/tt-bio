...
def _size_ladder_model_rungs(model: str, want=None) -> tuple:
    """This model's own ladder, ascending, narrowed to ``want`` when one is given.

    ``want`` filters rather than selects, so ``--size-ladder-rungs 256,1088`` resumes rf3 at
    both and every other model at 256 alone instead of failing on a rung its ladder has no
    fixture for.
    """
...
def run_size_ladder(keep: bool, record: bool, baseline_path: Path,
    ...
    models = list(models or SIZE_LADDER_MODELS)
    # None means "each model's whole ladder", which is not the same tuple for every model
    # (SIZE_LADDER_EXTRA_RUNGS), so the narrowing stays a filter applied per model below.
    want = tuple(rungs) if rungs else None
    if not record and want is not None:
        return {"model": "size-ladder", "seconds": 0, "gate": False,
                "card": _size_ladder_card_type(), "rungs": list(want),
