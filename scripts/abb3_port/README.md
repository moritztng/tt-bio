...
|---|---|---|---|
| `reference_gate.py` | is `abodybuilder3_reference.py` upstream's model? | tables 0.0, IPA 1.847e-13, whole model <= 8.7e-14 in float64 | upstream checkout |
...
| `model_gate.py` | is the 8-block model right? | atom14 0.004 A rmsd, gradients cos 0.9995 | card |
...
| `step_gate.py` | what does a COMPLETE step cost? | see `perf/abb3_port/step_gate_complete_qb1c3.txt` | card |
