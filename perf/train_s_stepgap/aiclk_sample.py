...
acc = {}
for _ in range(n):
    out = subprocess.run(["/home/ttuser/.local/bin/tt-smi", "-s"], capture_output=True, text=True).stdout
    d = json.loads(out[out.index("{"):])
    for dev in d["device_info"]:
        bdf = dev["board_info"]["bus_id"]
        acc.setdefault(bdf, []).append(int(dev["telemetry"]["aiclk"]))
    time.sleep(gap)
