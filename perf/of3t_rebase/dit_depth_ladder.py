...
def main() -> int:
    ...
    ap.add_argument("--out", default="perf/of3t_rebase/dit_depth_ladder_043.json")
    a = ap.parse_args()
    t0 = time.time()
    ...
    import openfold3
    import ttnn
    from refrev import reference_revision
    ...
    revision = reference_revision(openfold3, model, sd64)
    print(f"[{time.time()-t0:.0f}s] reference revision {revision['version']} "
          f"missing={revision['load']['missing']} "
          f"unexpected={revision['load']['unexpected']}", flush=True)
    ...
    json.dump({"ladder": rows, "struct": k, "reference_revision": revision,
               "inputs": "real captured activations, DiT truncated to d blocks on BOTH sides",
