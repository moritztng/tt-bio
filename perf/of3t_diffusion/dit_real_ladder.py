#!/usr/bin/env python3
"""Where does our DiT start disagreeing, on IN-DISTRIBUTION inputs?
...
def main() -> int:
    ...
    ap.add_argument("--sizes", default="32,64,96,128,192,256,320,384")
    ap.add_argument("--struct", type=int, default=0)
    ap.add_argument("--out", default="perf/of3t_diffusion/dit_real_ladder.json")
    a = ap.parse_args()
    t0 = time.time()
    ...
    print(f"[{time.time()-t0:.0f}s] both DiTs ready, full n={n_full}", flush=True)
    ...
    for n in [int(x) for x in a.sizes.split(",")]:
        ...
        m = mk.bool()
        rows[n] = {"rel": rel(ours, ref2), "real_tokens": int(m.sum()),
                   "rel_real": rel(ours[m], ref2[m]) if int(m.sum()) else None}
        print(f"[{time.time()-t0:.0f}s] n={n:4d} real={rows[n]['real_tokens']:4d}  "
              f"rel {rows[n]['rel']:.6e}  real-rows {rows[n]['rel_real']}", flush=True)

    json.dump({"ladder": rows, "struct": k, "inputs": "truncated REAL captured activations",
