#!/usr/bin/env python3
"""Is the DiT gap ARITHMETIC or FUNCTIONAL? Compare against their DiT in fp32, not float64.
...
def main() -> int:
    ...
    ap.add_argument("--out", default="perf/of3t_diffusion/dit_dtype_arms2.json")
    ap.add_argument("--ref-dtype", default="float64", dest="ref_dtype")
    ap.add_argument("--our-dtype", default="float32,bfloat16", dest="our_dtype")
    a = ap.parse_args()
    t0 = time.time()
    ...
    OURDT = {"float32": ttnn.float32, "bfloat16": ttnn.bfloat16}
    for dtname in a.ref_dtype.split(","):
        dt = DT[dtname]
        ref_dit.to(dt)
        for d in [int(x) for x in a.depths.split(",")]:
            if d > len(full_blocks):
                continue
            ref_dit.blocks = torch.nn.ModuleList(full_blocks[:d]).to(dt)
            with torch.no_grad(), BM.no_autocast():
                ref = ref_dit(a=aa.reshape(1, 1, n, -1).to(dt), s=ss.reshape(1, 1, n, -1).to(dt),
                              z=zz.reshape(1, 1, n, n, -1).to(dt), mask=mk.reshape(1, 1, n).to(dt),
                              _mask_trans=True, use_high_precision_attention=True)
            ref2 = ref.reshape(n, -1).double()
            for ourname in a.our_dtype.split(","):
                act = OURDT[ourname]
                ft = lambda x, _a=act: ttnn.from_torch(
                    x.float(), layout=ttnn.TILE_LAYOUT, device=dev, dtype=_a)
                ...
                m = mk.bool()
                key = f"ref{dtname}_our{ourname}_d{d}"
                rows[key] = {"ref_dtype": dtname, "our_dtype": ourname, "blocks": d,
                             "rel": rel(ours, ref2), "rel_real": rel(ours[m], ref2[m])}
                print(f"[{time.time()-t0:.0f}s] ref={dtname} ours={ourname:9s} blocks={d:3d}  "
                      f"real-rows {rows[key]['rel_real']:.6e}", flush=True)
        ref_dit.blocks = torch.nn.ModuleList(full_blocks)

    json.dump({"ladder": rows, "struct": k, "inputs": "real captured activations; reference run at BOTH fp32 and float64",
