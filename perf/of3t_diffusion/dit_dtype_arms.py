...
def main() -> int:
    ...
    ap.add_argument("--out", default="perf/of3t_diffusion/dit_dtype_arms.json")
    ap.add_argument("--ref-dtype", default="float32,float64", dest="ref_dtype")
    a = ap.parse_args()
    t0 = time.time()
    ...
    for dtname in a.ref_dtype.split(","):
        ...
        for d in [int(x) for x in a.depths.split(",")]:
            ...
            m = mk.bool()
            rows[f"{dtname}_d{d}"] = {"ref_dtype": dtname, "blocks": d,
                                      "rel": rel(ours, ref2),
                                      "rel_real": rel(ours[m], ref2[m])}
            print(f"[{time.time()-t0:.0f}s] ref={dtname} blocks={d:3d}  "
                  f"rel {rows[f'{dtname}_d{d}']['rel']:.6e}  "
                  f"real-rows {rows[f'{dtname}_d{d}']['rel_real']:.6e}", flush=True)
