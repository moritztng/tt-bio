#!/usr/bin/env python3
"""Is `TT_BIO_FUSE_BIAS_STACKS` safe at 512 aa? The folds that question needs.
...
def main() -> int:
    ...
    assert "TT_BIO_FUSE_BIAS_STACKS" not in os.environ, \
    ...
    work = Path(tempfile.mkdtemp(prefix="b2z2-fusebias-"))
    ...
    state.bind_run("b2z2-fusebias", cfg)
    out["model_load_s"] = round(time.perf_counter() - t0, 3)
    dump()

    def fold(arm, seed, target, keep):
        os.environ["TT_BIO_FUSE_BIAS_STACKS"] = "1" if arm == "on" else "0"
        cfg["seed"] = seed
        try:
