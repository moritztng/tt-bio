"""RFD3 TT ladder: per-step wall by step-count differential, per rung x per batch.
...
OUT = pathlib.Path("perf/dsfix/results/rfd3_tt.jsonl")
OUT.parent.mkdir(parents=True, exist_ok=True)
LADDER = [("R0", [1, 4, 8]), ("R1", [1, 4, 8]), ("R2", [1, 2, 4]),
          ("R3", [1, 2, 4]), ("R4", [1, 2])]
...
for rung, batches in LADDER:
    ...
    print("[ladder] %s featurised L=%d atoms in %.1fs" % (rung, L, time.perf_counter() - t_feat),
          flush=True)
    for b in batches:
        if (rung, b) in done:
            continue
