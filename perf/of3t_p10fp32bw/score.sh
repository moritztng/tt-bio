#!/usr/bin/env bash
# of3t-p10fp32bw: compose a DIFFUSION arm into the graded 3,660-tensor model artifact and read
# the clause off it.
#   score.sh <TAG> <diffusion.pt>
#
# perf/of3t_stackexact/score.sh with one --arm path changed: there it is the trunk that moves
# and the five other scopes are held; here the trunk is held at of3t-p10exact's shipping arm
# (dev_PKGSM.pt, TT_BIO_SOFTMAX_BW_FP32=1) and the DIFFUSION scope moves, because that is the
# scope the step fires the flag in 141 times a rep and the clause graded without it. Same
# pinned references, same section table, same unwrap key, same injection convention. CPU only,
# no card is opened.
set -uo pipefail
...
TRUNK=/home/ttuser/of3t_p10exact/dev_PKGSM.pt
TAG=${1:?usage: score.sh TAG diffusion.pt}; DIFF=${2:?usage: score.sh TAG diffusion.pt}
[ -s "$DIFF" ] || { echo "missing diffusion arm $DIFF"; exit 2; }
[ -s "$TRUNK" ] || { echo "missing trunk arm $TRUNK"; exit 2; }
OUT=perf/of3t_p10fp32bw/MODEL_${TAG}_composed3660_n384.json

OMP_NUM_THREADS=8 nice -n 10 "$PY" perf/of3t_wholemodel/model_scope.py \
  --arm "renorm:diffusion=$DIFF" \
  ...
  --out "$OUT" --sidecar-dir "$O/sidecar_$TAG" 2>&1 | tail -20
rc=${PIPESTATUS[0]}
...
"$PY" perf/of3t_modelframe/clause.py \
  ...
  --rescored "$OUT" --out "perf/of3t_p10fp32bw/CLAUSE_${TAG}.json" | tail -3
"$PY" - "perf/of3t_p10fp32bw/CLAUSE_${TAG}.json" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))["frame_matched"]
print(json.dumps({"tag": sys.argv[1], "CLAUSE": d["CLAUSE"],
                  "trunk_read": d["trunk"]["reading_vs_their_bf16"]}, indent=1))
PY
