...
# `bare2` is `bare` under a second label: an explicit A/A pair, so a keepcache delta is
# scored against this session's own repeatability instead of an inherited floor. It takes
# no branch of its own anywhere below -- every arm test is an equality against a name.
ARMS = ("bare", "bare2", "regions", "cprofile", "sample", "pyspy", "cacheclear",
        "keepcache")
...
def main() -> int:
    ...
    # The holder the capture must be launched under. Checked, not set: the lease file has
    # to carry the running row's identity or the fleet dispatcher stops seeing the task.
    ap.add_argument("--holder", default="worker:c12-host-decomp")
    a = ap.parse_args()
    out = a.out.resolve()
    ...
    try:
        ...
        def one(label, arm, clock, rep):
            ...
            # How many distinct programs the fold leaves cached. A rebuild rebuilds exactly this
            # many, so it converts the A/B delta into a per-program cost and is the sanity check
            # on whether a measured delta is even the right order of magnitude.
            try:
                row["program_cache_entries"] = int(dev.num_program_cache_entries())
            except Exception as e:
                row["program_cache_entries"] = f"unavailable: {e!r}"
            if smp is not None:
                smp.stop.set()
                smp.join(timeout=5)
                gt.off()
        ...
        for rep in range(a.reps):
            ...
            # Rotate the arm order too. With one clock the reversal above does nothing, so the
            # arms would sit in fixed positions inside every rep and a monotone drift across a
            # rep would land entirely on the last arm. Rotating makes each arm visit each slot.
            arm_order = arms[rep % len(arms):] + arms[:rep % len(arms)]
            for clock in order:
