...
# --cifdir replaces them, which is what lets the same scorer read another card's folds without
# mixing two cards into one arm: the byte-identity assertion below is per arm ACROSS sessions, and
# two cards legitimately need not agree byte for byte.
...
def main() -> int:
    ...
    ap.add_argument("--cifdir", type=Path, nargs="+",
                    help="score these fold directories instead of the committed pc sessions.")
