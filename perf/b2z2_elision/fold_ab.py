#!/usr/bin/env python3
"""Boltz-2 512 aa fold A/B of one branch's defaults against the tree they sit on.

`--lever gather`, the default, is the slice+concat atom key gather. Arm `off` pins
`_ATOM_SHIFT_GATHER_OFF`, so the gather stays the one-hot matmul with its four arithmetic-free
programs; arm `on` is the shipped default of this branch. Both arms must write the same CIF byte
for byte, because the gather is a pure index permutation: the bar is a matching sha256, not an
RMSD.

`--lever tierb` is the pair that is not bit-exact, TT_BIO_UNFUSED_SILU and
TT_BIO_DEVICE_CONDITIONING, against the same tree with both off. There the digests are expected to
differ and the run answers only the seconds; the structural delta is scored elsewhere, against a
bar, and neither number can stand in for the other.

The arms are set in process and the diffusion static cache is reset between them, because each
decision is taken once per fold: the gather where `keys_indexing` is still a torch tensor, the
conditioning where it chooses host or device.
...
def main() -> int:
    ...
    ap.add_argument("--lever", choices=("gather", "tierb"), default="gather")
    ...
    pinned = {"TT_BIO_ATOM_SHIFT_GATHER", "TT_BIO_UNFUSED_SILU",
              "TT_BIO_DEVICE_CONDITIONING"} & set(os.environ)
    assert not pinned, f"the arms are set in-process; {sorted(pinned)} would make both arms one arm"
    ...
    def set_arm(arm: str):
        """Every lever this run switches, and nothing else: the rest of the tree is the control."""
        on = arm == "on"
        if args.lever == "gather":
            TT._ATOM_SHIFT_GATHER_OFF = not on
        else:
            TT._UNFUSED_SILU = on
            os.environ["TT_BIO_DEVICE_CONDITIONING"] = "1" if on else "0"
    ...
    OUT["within_arm_identical"] = {a: len(shas[a]) == 1 for a in ("off", "on")}
    dump()
