...
pc=' '.join(f'{k}:{v}' for k,v in (t.get('chain_rmsd') or {}).items())
print(f\"$m $s n={len(r)} chains={list(t['chains'].values())} perchain[{pc}] top={t.get('ca_rmsd')} best={min(v) if v else None} worst={max(v) if v else None} lig(centroid/contact)={lig or '-'}\")"
done
