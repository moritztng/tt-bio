...
def main():
    ...
    try:
        ...
        from perf.c10_generic_identity.observer import Capture, Encoder, unavailable_paths
        from tt_bio.mm_generic import generic_minimal_matmul
        from tt_bio.reblock_permute import reblock_permute
        enc=Encoder(ttnn)
        kd=ttnn.KernelDescriptor(kernel_source="unused.cpp",core_ranges=ttnn.CoreRangeSet([ttnn.CoreRange(ttnn.CoreCoord(0,0),ttnn.CoreCoord(0,0))]),runtime_args=[(ttnn.CoreCoord(0,0),[13,17])], config=ttnn.ComputeConfigDescriptor(math_fidelity=ttnn.MathFidelity.HiFi4))
        native_view=kd.runtime_args
        result["binding_smoke"]={"runtime_view_type":type(native_view).__module__+"."+type(native_view).__name__,"runtime_coordinate_values":list(native_view[0][0]),"descriptor":enc.value(ttnn.ProgramDescriptor(kernels=[kd],cbs=[],semaphores=[]))}
        if result["binding_smoke"]["runtime_coordinate_values"]!=[13,17]: raise RuntimeError("Native RuntimeArgsView mismatch")
        result["binding_smoke"]["gaps"]=list(unavailable_paths(result["binding_smoke"]["descriptor"]))
        save()
        torch.manual_seed(0)
        host_a=[torch.randint(-1,2,(64,64),dtype=torch.int32).to(torch.bfloat16) for _ in range(2)]
        host_b=[torch.randint(-1,2,(64,64),dtype=torch.int32).to(torch.bfloat16) for _ in range(2)]
        host_x=[((torch.arange(1*64*64*32).reshape(1,64,64,32)+i)%127-63).to(torch.bfloat16) for i in range(2)]
        def todev(t): return ttnn.from_torch(t,layout=ttnn.TILE_LAYOUT,device=dev,memory_config=ttnn.DRAM_MEMORY_CONFIG)
        aa,bb,xx=[list(map(todev,v)) for v in (host_a,host_b,host_x)]
        outs=[ttnn.allocate_tensor_on_device(ttnn.Shape([64,64]),ttnn.bfloat16,ttnn.TILE_LAYOUT,dev,ttnn.DRAM_MEMORY_CONFIG) for _ in range(2)]
        cfg=((1,1,1,1,1),(2,2)); ckc=(ttnn.MathFidelity.HiFi4,False,True,False)
        result["controls"]=[]
        result["compute_config"]={"cfg":cfg,"fidelity":"HiFi4","approx":False,"fp32_dest":True}
        for kind in ("matmul","reblock"):
            baseline=[]
            for observed in (False,True):
                case=kind+("_on" if observed else "_off")
                check_live(); fence()
                interval={"case":case,"reps":4,"start_monotonic_ns":time.monotonic_ns()}
                ttnn.graph.begin_graph_capture(ttnn.graph.RunMode.NORMAL)
                returned=[]
                observer=Capture(ttnn,a.out/(case+".jsonl"),config={"case":case,"seed":0}) if observed else nullcontext()
                with observer:
                    for i in range(4):
                        j=i%2
                        z=generic_minimal_matmul(dev,aa[j],bb[j],outs[j],cfg,ckc) if kind=="matmul" else reblock_permute(xx[j],device=dev)
                        returned.append(z)
                ttnn.synchronize_device(dev)
                interval["end_monotonic_ns"]=time.monotonic_ns()
                graph=ttnn.graph.end_graph_capture()
                if isinstance(graph,str): graph=json.loads(graph)
                write_json(a.out/(case+"_graph.json"),graph)
                fence()
                samples=[json.loads(l) for l in (a.out/"clock.jsonl").read_text().splitlines() if l.endswith("}")]
                interval["clock"]=coverage(samples,interval)
                result["intervals"].append(interval); save()
                if not interval["clock"]["pass"]: raise RuntimeError("Clock coverage failed: "+case)
                for i,z in enumerate(returned):
                    actual=ttnn.to_torch(z).to(torch.float64)
                    j=i%2
                    expected=(host_a[j].to(torch.float64)@host_b[j].to(torch.float64)) if kind=="matmul" else host_x[j].to(torch.float64).permute(0,3,1,2)
                    ok=torch.equal(actual,expected)
                    result["controls"].append({"case":case,"index":i,"float64_exact":ok,"max_abs":float((actual-expected).abs().max()),"off_on_exact":torch.equal(actual,baseline[i]) if observed else None,"sha256":hashlib.sha256(actual.contiguous().numpy().tobytes()).hexdigest()})
                    if not observed: baseline.append(actual)
                    if not ok or (observed and not torch.equal(actual,baseline[i])): raise RuntimeError("Control output mismatch")
                if kind=="reblock":
                    for z in returned: ttnn.deallocate(z)
                save()
        for tensor in aa+bb+xx+outs+[f]: ttnn.deallocate(tensor)
