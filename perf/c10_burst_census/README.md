...
Raw data outside named windows was synchronously drained and excluded with a hash/byte ledger. Thus 259,634 host device-operation records are not 259,634 cycle-measured model operations: they also include fences, controls and setup. The retained Pairformer window includes the two-argument path: its native graph contains the `[1,512,384]` single representation and its attention/transition operations. The window metadata does not directly record that wrapper signature; see the [source and operand review](../c10_orchestrator/census_scope_review.json). No full-fold device-cycle closure or instrumentation perturbation measurement exists. Host wall times are diagnostic only. Unclassified fence gaps are not CPU work. Raw wall-clock register extents and per-core durations do not establish a synchronized host conversion or independently measured cross-core offsets.

The [bounded metadata exporter](../c10_export_budget/README.md) replaces automatic timing export and its Python postprocessor. Before another census, verify large-trace resource feasibility, establish full cycle retention and measure instrumentation perturbation. Native matmul and binary operations are the first classes to investigate within the measured scope. Their ranking does not authorize an optimization or predict its saving. Any later accuracy-changing lever still needs independent float64 controls and paired 512/298 structure checks; the 512 bar is 0.60 Å with a 1.84 Å seed floor, and the 298 target retains its own documented bar.
...
The census reducer writes STOP while retaining every validated row. Its successful exit means replay completed, not that the measurement gate passed. `run_census.sh` uses bounded metadata export and derives its worktree and lease-holder name from its own location. The original failed command is retained under `../c10_export_budget/source/`. A new capture still requires the resource and coverage checks described above. Live smoke reproduction is described by `run_smoke.sh` and requires an explicit node-0 grant, normal benchlock and during-interval clock sampling.
