...
def reduce(p):
    ...
    for interval,(left,right) in zip(intervals,fenced(rows,4)):
        ...
        for index,(g,o) in enumerate(zip(graphops,selected)):
            ...
            if calls:
                ...
                fields=call["descriptor"]["fields"]
                if not required_gaps([call]):
                    expected_addresses=[t["address"] for t in tensors[:-1]]
                    for ki,kernel in enumerate(fields["kernels"]):
                        k=kernel["fields"]
                        if is_mm and ki<4:
                            expected_addr=expected_addresses[0 if ki<2 else 1]
                            entries=k["runtime_args"]["entries"]
                            if not entries or any(e["values"][0]!=expected_addr or e["values"][-1]!=expected_addresses[-1] for e in entries):
                                raise ValueError("Runtime address rebinding mismatch")
                        elif not is_mm and ki<2:
                            if k["common_runtime_args"] != [expected_addresses[ki]]:
                                raise ValueError("Common runtime rebinding mismatch")
    ...
    return {"verdict":"STOP" if gaps else "GO","reason":"Missing required runtime identity prevents a census." if gaps else "Live bounded identity, float64 output, multiplicity and address rebinding controls pass; model census remains required.","capture_valid":True,"identity_smoke_pass":not bool(gaps),"raw_header":header,"boot_id":cap["before"]["boot_id"],"other_chip_holder_pids":other,"holder_observations":len(holders),"float64_exact_checks":16,"on_off_exact_checks":8,"binding_smoke":cap["binding_smoke"],"required_identity_gaps":sorted(set(gaps)),"joins":joins,"table":table,"model_invocations_measured":0,"model_coverage_fraction":0,"whole_fold_closure":None,"model_roof":None,"model_floor":None,"model_cycles_above_roof":None,"rank_scope":"raw instrumented spans in these 16 small controls only; not a model class ranking or observer overhead estimate","limits":["No synchronized host/device timebase conversion","No profiler perturbation calibration","No machine-code provenance proof","All program-cache-hit labels false; descriptor hash equality is not a binary cache-hit proof","Missing runtime arguments prevent matmul rebinding validation" if gaps else "Address arguments agree with actual graph operands on all observed calls","Co-tenant node3 recorded; host walls diagnostic only"]}

def main():
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument("--dir",type=Path,default=Path(__file__).parent/"runs/smoke1");ap.add_argument("--require-go",action="store_true");ap.add_argument("--table",type=Path,default=Path(__file__).parent/"TABLE.md");a=ap.parse_args()
    result=reduce(a.dir)
    ...
    lines += ["","Physical reads and issued arithmetic remain unknown. Gaps are not CPU work. The four control envelopes are disjoint; no parent/child sum or invocation extrapolation is used. Zero model invocations were captured. Matmul uses HiFi4 with FP32 accumulation; reblock's source descriptor uses HiFi2. Neither is a throughput roof.","","Verdict: " + result["verdict"] + ". " + result["reason"] + " No whole-fold floor, binding constraint, headroom or campaign ceiling follows from this table."]
    a.table.write_text("\n".join(lines)+"\n")
