#!/usr/bin/env bash
# of3t-stackexact: one model-frame trunk rung on qb2 card 1.
#
#   arm.sh <TAG> <scopes>    scopes: none | softmax | layer_norm | softmax,layer_norm
#
# of3t-modelever's arm.sh with modelarm.py replaced by stackarm.py: same boundary, cotangent,
# lever, arm, crop, env and thread count. An ACCURACY reading: the AICLK sampler and host_quiet
# RECORD the conditions (the writer stamps them, D155/D249), and no timing is quoted.
set -uo pipefail
W=/home/ttuser/.coworker/wt/of3t-stackexact
cd "$W"
O=/home/ttuser/of3t_stackexact
H=$W/perf/of3t_stackexact
mkdir -p "$O"
CARD=1
...
TAG=${1:?usage: arm.sh TAG scopes}; SCOPES=${2:?usage: arm.sh TAG scopes}
...
ENV=(TT_BIO_SOFTMAX_BW_RENORM=1 TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD
     TT_BIO_LEASE_HOLDER=worker:of3t-stackexact OMP_NUM_THREADS=8 PYTHONPATH="$W")
...
d["provenance"] = {
    "host": socket.gethostname(), "row": "of3t-stackexact", "device_involved": True,
