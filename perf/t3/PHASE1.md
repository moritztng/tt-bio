...
| transition_s, all 5 ops @2038/2046/2055/2064/2066 | 484 x 5 | 82.15 total | **39.8** (0.19 % of the pf_stack total) | 4.5 MB DRAM per call | 320 / 264.8 | compute side | K=384 (62.54) and K=1536 (100.13) compute | fc1 16.09 TFLOP/s = 15.5 % of the K=384 compute roof; fc3 19.36 = 16.4 % of the K=1536 compute roof | 11x10; mt=10 tile-rows across 110 cores means **at most 10 of 110 cores can hold an output tile row** | sum | Grouped: the whole s-track Transition is 0.19 % of the pf_stack total. H8: mt=10 is the entire story — 10 tile-rows cannot fill 110 cores no matter the config, so this is latency/occupancy-bound and will never approach a compute roof. Not worth a Phase-3 slot |
...
**Phase 2 must confirm:** (a) H1 — that the premium is output-element-linear and K-independent;
...
op projects c_z=256 down to 16 heads of bias, one output tile column. **Phase 2 must confirm** H5 by
...
(33.4 ms/fold). H6 says the gap is the row reduction pinning kt=8 input tiles in a circular buffer
...
fc1 80.20 us at 110 cores against 77.61 us at 100; fc3 94.65 against 93.36. H3 says mt=300 divides
...
verify that with `torch.equal`, and must check H3 at mt=330 before anyone generalises this to the
