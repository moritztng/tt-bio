#!/usr/bin/env bash
# of3t-stackship (of3t-stackexact score.sh, outputs moved): compose a trunk arm into the graded 3,660-tensor model artifact and read the
...
#
#   score.sh <TAG> <trunk.pt>     -> perf/of3t_stackship/MODEL_<TAG>_composed3660_n384.json
#                                    perf/of3t_stackship/CLAUSE_<TAG>.json
set -uo pipefail
...
OUT=perf/of3t_stackship/MODEL_${TAG}_composed3660_n384.json
...
"$PY" perf/of3t_modelframe/clause.py \
  ...
  --rescored "$OUT" --out "perf/of3t_stackship/CLAUSE_${TAG}.json"
