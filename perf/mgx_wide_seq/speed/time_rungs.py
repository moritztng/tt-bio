...
One JSON line per fold goes to perf/mgx_wide_seq/speed/runs/<model>.jsonl with the commit, host, chip and
...
count against that budget.
    ...
    TT_VISIBLE_DEVICES=<c> TT_BIO_LEASE_CARDS=<c> ... python perf/mgx_wide_seq/speed/time_rungs.py \
...
out = ROOT / "perf" / "mgx_wide_seq" / "speed" / "runs"
...
work = Path(os.environ.get("RELEASE_GATE_SIZE_WORKDIR", ROOT / "perf" / "mgx_wide_seq" / "speed" / f"work-{model}"))
