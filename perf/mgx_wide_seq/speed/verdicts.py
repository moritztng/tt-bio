"""Speed-bar verdict for every timed rung above 1024, from perf/mgx_wide_seq/speed/runs/<model>.jsonl.
...
rung's and takes its sigma inside that fit (route (a)); fewer than three is UNGATED.
    ...
    python perf/mgx_wide_seq/speed/verdicts.py [runs_dir]      # prints one JSON object per model
"""
import functools
import json
import statistics
...
def main(argv):
    runs = Path(argv[1]) if len(argv) > 1 else ROOT / "perf" / "mgx_wide_seq" / "speed" / "runs"
