...
def main() -> int:
    ...
    arms = [a for a in ("base", "ELI", "K2", "DST", "UNION")
            if any(r["arm"] == a for r in timed)]
    singles = [a for a in arms if a not in ("base", "UNION")]
    # K2 and DST are measured beside the same fixture on the same card by
    # `b2z2-bh-compose-landed` (1.00448x and 0.99748x). If they are not arms of THIS run they
    # still belong in the additivity product, labelled imported rather than measured here.
    IMPORTED = {"K2": 1.00448, "DST": 0.99748}
    ...
    ratio = {arm: round(base / per[arm]["median_s"], 5) for arm in arms if arm != "base"}
    # An arm's WORST rep against the base median: the pessimistic reading of the same data.
    worst = {arm: round(base / per[arm]["max_s"], 5) for arm in arms if arm != "base"}
    product = 1.0
    for arm in singles:
        product *= ratio[arm]
    imported_used = {a: r for a, r in IMPORTED.items() if a not in ratio}
    for r in imported_used.values():
        product *= r
    ...
    delta_s = {arm: round(base - per[arm]["median_s"], 3) for arm in arms if arm != "base"}
    ...
    res = {
        ...
        "imported_single_ratios": imported_used,
