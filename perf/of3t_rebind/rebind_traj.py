#!/usr/bin/env python3
"""of3t-rebind: `of3t-modeltraj`s 20-step model-in-the-loop trajectory, re-run on the
SHIPPED default after the optimizer/tape seam was repaired in `tt_bio/autograd.py`.

The only harness change is which arm asks for the workaround: `repin` was the repair and is now
a control. Everything else -- the scope, the partition, the scorer, the reference side -- is
`perf/of3t_modeltraj/modeltraj.py` verbatim, so the two rows sets of numbers are comparable.

ROTOCOL S7 with the MODEL in the loop: 20 optimizer steps driven by the gradient the model
...
def run_ours(blocks, cap, *, steps, warmup, log, brk="none", zero_grad_model=False,
    ...
    `repin` is now a CONTROL rather than a repair. `of3t-modeltraj` had to run every arm but
    `shipped` with it on, because the shipped path was saturated by its own wiring error and a
    break control cannot move a reading already pinned at 1.0. `of3t-rebind` fixed that seam in
    the library (`autograd.Tensor.value` carries the parameter registration onto the handle the
    optimizer writes), so every arm here runs the SHIPPED default and `--arm repin` asks the
    harness to do the now-redundant rebind on top. It must come out bit-identical to `shipped`;
    if it does not, the library fix and the harness workaround are not the same repair.
    """
    import torch
    import ttnn
    ...
    for k in range(1, steps + 1):
        ...
        if repin:
            # REDUNDANT since of3t-rebind, and run as a control for exactly that reason: on a
            # fixed tree `autograd.Tensor.value`s setter already re-keyed the registry when the
            # optimizer wrote the new handle, so this is an idempotent no-op and the arm must
            # score bit-identically to `shipped`.
            for t in params.values():
                ag.parameter(t)
        resolved = 0
...
def main() -> int:
    ...
    out = a.out or f"perf/of3t_rebind/traj_{a.arm}{a.tag}.json"
