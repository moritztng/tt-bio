...
0.07 CA-lDDT, because a model can scatter widely on this fixture while landing at the same quality
every time.

    fold_layout.py --out <json> --cifdir <dir> [--plan base:0,1,2,3;qkv:0,1,2,3]
"""
import argparse
import hashlib
import json
...
def check_arms(arms, T, QKV):
    ...
    Assigning a module attribute nothing reads succeeds silently, so every arm would run the
    same code and score as a null. Check both readers exist before folding anything.
    """
    for arm in arms:
...
def main() -> int:
    ...
    ap.add_argument("--plan", default="base:0,1,2,3;qkv:0,1,2,3")
    ...
    ap.add_argument("--timing-arms", default="base,qkv,base",
    ...
    for f in FLAGS:
        assert f not in os.environ, f"{f} may not be pinned; the arm is set per fold"
    check_arms(list(plan) + args.timing_arms.split(","), T, QKV)
    ...
    work = Path(tempfile.mkdtemp(prefix="b2z2-layout-"))
    ...
    state.bind_run("b2z2-layout", cfg)
...
def timing(args, out, dump, fold, T, ttnn, dev):
    ...
    cif = Path(tempfile.mkdtemp(prefix="b2z2-layout-timing-"))
