...
Written from `out/` rather than by hand. `out/` is gitignored (CIFs, logs, MSA scratch), so this
file is the part of the evidence that survives in the repo, and every field in it is read back
from an artifact the run produced.

Both axes of the coverage bar come out of the run's own `results.json`: `n_tokens` is
`f["asym_id"].reshape(-1).shape[0]` and `msa_depth` is `f["msa_stack"].shape[1]` (worker.py), so
each is the shape the model was handed rather than a count of `>` records in the input a3m. rf3
builds its own msa_stack, so neither is inferred from the fixture.
...
`refusals_in_log` is grepped rather than assumed absent -- rf3 takes caught circular-buffer
throws at every rung and keeps folding, so their presence is a fact about the run and not a
failure. `rss_flat_last_5min` reports whether the process group was still moving at the end,
because a stall presents as a budget timeout and not as a throw.
"""
...
def progress(d: Path) -> dict:
    ...
    rss = [int(r[3]) for r in host if len(r) > 3 and r[3].isdigit()]
    tail = rss[-30:]      # sampled every 10 s, so 30 samples is the last 5 minutes
...
def main() -> int:
    rows = []
    for d in sorted((HERE / "out").glob("rf3_*_dev*")):
        ...
        if res:
            ...
            row |= {"status": r["status"], "runtime_s": r.get("runtime_s"),
                    ...
                    "n_atoms": r.get("n_atoms"), "recycling_steps": r.get("recycling_steps"),
            ...
            if cifs:
                ...
                row |= structure(d, cifs[0], HERE / "inputs" / f"rf3_{rung}.yaml")
        else:
