...
**One lead survives, and it is a code fact rather than a modelled one.** The census measured
`core_grid=CORE_GRID_MAIN` moving the matmul class from 11.88 to 21.38 TFLOP/s — **4.908 s** — but
could not tell whether the engine's own call sites pass it. [`core_grid_sites/`](core_grid_sites/)
answers that by AST: **35 of the 90 `ttnn.linear`/`ttnn.matmul` sites in `tt_bio/tenstorrent.py` are
bare, and 18 of those are on Boltz-2's default path** — `TriangleMultiplication`,
`TriangleAttention`, `OuterProductMean`, `DiffusionTransformer`, `Diffusion`, `AdaLN` and the shared
projection helpers. So the 4.908 s is not purely a replay artifact. It is **not a priced lever**
either: a bare site gets ttnn's default grid, which may already suit that shape, and this is exactly
the shape of a one-size-tuning defect, so it must be A/B'd per site.
...
| | |
|---|---|
| [`wedge_telemetry/`](wedge_telemetry/) | **A wedge shows up in device telemetry ~60 s later, in two counters qb2 already logs.** While folding, `slv_rd_data_word_sent0` runs 0.3–4.4 M words/s and `mst_rd` varies 100 %; while wedged, `slv_rd` is **0 in every sample** and `mst_rd` is pinned to a constant (**0.07 %** spread over 12 samples) — a host poll loop against a stopped chip. Power separates too but overlaps. CPU% separates neither. Scored on one incident; the second wedge read zero telemetry and could not be scored. |
| [`wedge_triage/`](wedge_triage/) | The 768 aa wedge checked against the known Blackhole sub-tile-last-axis hang class, as that memory's standing instruction asks. **Not matched, not excluded.** Only one recorded shape has a non-tile last-axis extent (a 48-wide per-head permute, above a tile where the class is below). And the counter-evidence is strong: **`1x768x512`, the shape the second sighting names, runs 9,600x per 512 aa fold across 74 clean folds — ~0.7M executions, no hang** — so shape alone is not the trigger. New information for that memory's open item. Next step needs a chip: a 768 aa census. |
...
| [`percall_residual/`](percall_residual/) | **The matmul gap is flat per call, not proportional to work.** Across a **1152x** range in FLOPs/call the residual (modelled minus roofline) spans only 4.3x, median **22.4 us** — a rate deficit would have tracked the FLOP range. That is **2.071 s of a modelled 3.141 s, 66 %**, and 13x the device's 1.70 us launch floor. **Or pc's harness added it**: the modelled per-call times come from pc's 130-core firmware, and nothing here separates those. Pre-registers a two-branch prediction that differs by >3x on the achieved rate. |
| [`matmul_ceiling/`](matmul_ceiling/) | **Bandwidth does not explain the fold's matmul rate.** The recorded shapes carry enough arithmetic intensity for a structural ceiling of **78.2 TFLOP/s** against a modelled **19.88** today — a 3.9x gap roofline does not account for — and the target's 35.49 is only **45 %** of that ceiling. So the headroom is not excluded by the hardware, and what binds is implementation (occupancy, tile quantisation, K-loop), not DRAM. The 16-head pair matmuls (AI 101) really are DRAM-bound near 43 TFLOP/s; the 768-family linears straddle the 247 balance. **Robust to the roofs**: across every roof pair the campaign has measured the ceiling moves only 78.2–84.0, the gap stays 3.9–4.2x and the target stays 42–45 % of it. |
| [`node1_anchor/`](node1_anchor/) | **Every number of record is on node 0, which is quarantined off the PCIe bus.** All future measurement is on node 1, so converting anything new against the 14.8813 s baseline is a cross-chip step. `c10-fixed-cost` left a **0.3273 s (2.2 %)** node-0/node-1 gap unattributable because that reading changed chip *and* firmware together — **node 1 now runs 19.15.0.0, the same bundle**, so the control is clean and costs **one warm fold**. |
| [`core_grid_sites/`](core_grid_sites/) | **35 of the engine's 90 matmul call sites pass no `core_grid`; 18 are on Boltz-2's hot path.** Answers by AST the one code question `c10-fold-census` left open when it measured `core_grid=CORE_GRID_MAIN` worth **4.908 s** on its own replay. So the difference is not purely a replay artifact — the engine calls these ops both ways. Not a priced lever: a bare site gets ttnn's default, and this is a one-size-tuning shape, so A/B per site. |
