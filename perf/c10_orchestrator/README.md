...
| [`matmul_ceiling/`](matmul_ceiling/) | **Bandwidth does not explain the fold's matmul rate.** The recorded shapes carry enough arithmetic intensity for a structural ceiling of **78.2 TFLOP/s** against a modelled **19.88** today — a 3.9x gap roofline does not account for — and the target's 35.49 is only **45 %** of that ceiling. So the headroom is not excluded by the hardware, and what binds is implementation (occupancy, tile quantisation, K-loop), not DRAM. The 16-head pair matmuls (AI 101) really are DRAM-bound near 43 TFLOP/s; the 768-family linears straddle the 247 balance. |
