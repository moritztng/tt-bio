#!/usr/bin/env python3
"""Interleaved fold A/B of the fp32-softmax free-core-count plan at a size the rectangle serves.

S1 (shipped) only touches sizes where the tuned 8x8 rectangle cannot divide any affordable block,
so `s1_xmodel_ab.py` scored it at 704 padded tokens. S2 is the other half: the free core count
offered at EVERY size, under the constraint that the taller block must keep the batched-matmul
program config the block it replaces had. That means it only moves sizes the rectangle DOES serve --
512, 576, 640, 768, 1024 -- and 704 is exactly where it changes nothing. So this harness is the same
instrument at a different `--aa`.

Arm A is today's default, arm B is S2 (`TT_BIO_FP32_SOFTMAX_L1_FLOAT_CORES`). Both arms are flipped
in process, so they share one loaded checkpoint and one warm device and interleave inside one sweep;
`--arms AA` runs the same instrument as an A/A control. Bit-exactness is a digest match, not a
tolerance: only the memory config of the fp32 score copy moves.

    TT_VISIBLE_DEVICES=1 TT_BIO_LEASE_CARDS=1 python3 perf/fp32softmax/s2_xmodel_ab.py \
        --model openfold3 --aa 512 --arms ABABAB --out /tmp/s2_of3_512.json
"""
...
def main() -> None:
    ...
    ap.add_argument("--arms", default="ABABAB",
                    help="fold order, A = shipped 8x8 plan, B = S2. One cold pair is discarded.")
    ...
    work = Path(tempfile.mkdtemp(prefix="s2ab-%s-" % args.model))
