...
uptime > $S/p5/a2.loadavg
BL=/home/ttuser/.coworker/scripts/benchlock.sh
export BENCHLOCK_WAIT_S=600
$BL worker:relion-acc-backend -- /usr/bin/time -f "%e %M" -o $S/p5/a2.time \
  mpirun -n 3 -x PYTHONPATH=/home/ttuser/.coworker/wt/relion-acc-backend -x TT_RELION_BACKEND=ttnn \
    $S/relion/build-tt/bin/relion_refine_mpi --o $S/p5/a2_run \
    --continue $S/Tutorial5.0/Refine3D/job019/run_it012_optimiser.star --auto_iter_max 13 \
    --cpu --j 6 --pool 6 --dont_combine_weights_via_disc --preread_images > $S/p5/a2.log 2>&1
echo "a2 rc=$?" > $S/p5/rc
uptime > $S/p5/a2.loadavg_end
