# Copy of wk/mgx-instrument perf/sizegate/mgx/hold.py, which is not on main yet; drop it once that merges.
...
took chips 7 and 9 from this row's openbind and rf3 chains between folds.
...
holder = os.environ.get("TT_BIO_LEASE_HOLDER", "worker:mgx-diffusion")
    ...
    try:
        ...
        if meta.get("released") or not alive(meta.get("pid") or 0):
            new = {"host": lease_host(), "card": card, "holder": holder, "pid": chain,
                   ...
                   "note": "held between folds by a mgx-diffusion chain"}
            os.ftruncate(fd, 0)
