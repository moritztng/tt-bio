#!/usr/bin/env bash
# of3t-readverbs device arms. qb1 (tt-quietbox), p150a Blackhole.
#   arm.sh <TAG> <64|384> <pin|""> <blocks|"">
#
# AICLK sampled every 4 s DURING the run, min/median/max with n. A number without a clock is
# not a measurement on this hardware.
set -uo pipefail
W=/home/ttuser/.coworker/wt/of3t-readverbs
cd "$W"
O=/home/ttuser/of3t_readverbs
mkdir -p "$O"
...
OUT=$O/grads_${TAG}.pt
COT=$O/cot_${TAG}.pt
SIDE=$W/perf/of3t_readverbs/SIDE_${TAG}.json
...
RV_PIN="$PIN" RV_BLOCKS="$BLOCKS" RV_SIDECAR="$SIDE" \
RV_ERRBLOCKS="${RV_ERRBLOCKS:-44,4,0}" RV_CAP="${RV_CAP:-2}" \
TT_BIO_SOFTMAX_BW_RENORM=1 \
TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD TT_BIO_LEASE_HOLDER=worker:of3t-readverbs \
OMP_NUM_THREADS=8 \
timeout ${RV_TIMEOUT:-5400} python3 perf/of3t_readverbs/arm.py --lever none --cot-out "$COT" \
  --boundary "$BND" --cap-last "$R/block47_boundary.pt" --out "$OUT" \
  --report "$W/perf/of3t_readverbs/DEV_${TAG}.json" --arm flipped --crop "$CROP" 2>&1 \
...
if [ "${RV_KEEP_GRADS:-0}" != "1" ]; then rm -f "$OUT"; fi
ls -la "$COT" "$OUT" 2>/dev/null
exit $rc
