...
ap.add_argument("--mem", type=str, default="l1", choices=("l1", "dram"),
                help="where the activation and the output live. The production call sites are "
                     "DRAM interleaved on both; the L1 default is what the WH row swept.")
ap.add_argument("--blocks", type=str, default="",
                help="semicolon-separated M,K,N,sh,sw list; empty = the full 16-config sweep.")
a = ap.parse_args()
...
BLOCKS = [(4, 4, 1, 4, 1), (2, 4, 1, 2, 1), (8, 4, 1, 4, 1), (16, 4, 1, 4, 1), (32, 4, 1, 4, 1),
          ...
          (4, 2, 2, 2, 2), (8, 2, 2, 2, 2)]
if a.blocks:
    BLOCKS = [tuple(int(v) for v in b.split(",")) for b in a.blocks.split(";")]
...
def native_for(block, grid):
    ...
    def f():
        ttnn.deallocate(ttnn.experimental.minimal_matmul(
            x, w, compute_kernel_config=ckc, memory_config=MEM, config=mmc))
    return f
            ...
            try:
                if kind == "generic":
                    ...
                    t = ttnn.experimental.minimal_matmul(
                        x, w, compute_kernel_config=ckc, memory_config=MEM,
