...
# After `get_device`, which widens `COMPUTE_GRID_MAIN` to the device's actual compute grid; a
# name bound at import time is the stale declaration.
...
print(f"N={N}, grid {gx}x{gy}, chip {os.environ.get('TT_VISIBLE_DEVICES', '?')}, "
      f"ttnn {version('ttnn')}, HiFi4 + fp32_dest_acc + packer_l1_acc")
