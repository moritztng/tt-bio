"""The full Boltz-2 512 aa cell fold with the atom-axis window shard WIRED IN, on a 1xN mesh.

Adapted from `perf/b2z2_dualchip/mesh_fold.py` rather than written fresh: that row already
measured the mesh tax and the allocator-rung trap with this harness, and its lesson is that an
op-level bit-exactness suite cannot certify a shard -- only an end-to-end CIF digest can. The
digest is what this file is for.

  single  the ordinary one-chip path. The control, and the fold ratio's denominator.
  mesh    a 1xN MeshDevice, every tensor REPLICATED, shard OFF. Isolates the mesh tax, and is the
          arm the shard's own ratio is taken against.
  shard   the same 1xN mesh with TT_BIO_ATOM_WINDOW_SHARD=1. Must write the SAME CIF as `single`.
...
MODE = sys.argv[1] if len(sys.argv) > 1 else "mesh"   # single | mesh | shard
MESH_N = int(os.environ.get("FOLD_MESH_N", "2"))
...
if MODE in ("mesh", "shard") and MESH_N > 1 and not os.environ.get("TT_MESH_GRAPH_DESC_PATH"):
    # ttnn ships descriptors for the systems it knows and nothing for a line of N chips cut out
    # of a 32-chip galaxy; `perf/b2z2_shardscale/meshdesc.py` generates one, and the path is read
...
state.bind_run("b2z2-atom-shard-wire", cfg)
...
OUT["shard_flag"] = bool(T._ATOM_WINDOW_SHARD)
assert OUT["shard_flag"] == (MODE == "shard"), "the shard flag does not match the mode"
