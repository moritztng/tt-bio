...
res = tb.measure("protenix-v2", a.repeat, REPO / f".msa_s512_{a.size}", Path(a.out),
                 fix / f"cdk2x2_{a.size}.yaml", fix / f"cdk2x2_{a.size}.a3m", f"{a.size} aa")

served, declined = rp.STATS
rej = {f"{k[0]}|{list(k[1])}": v for k, v in rp.REJECTS.items()}
import tt_bio.tenstorrent as _T
extra = {"size": a.size, "l1_headroom": _T._TRANSPOSE_L1_HEADROOM,
         "pt_row_major": _T._PT_ROW_MAJOR, "l1_n_max": rp.L1_N_MAX, "l1_n_min": rp.L1_N_MIN, "reblock_served": served, "reblock_declined": declined, "reblock_rejects": rej,
