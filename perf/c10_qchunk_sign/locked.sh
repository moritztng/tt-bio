...
mkfifo "$FIFO"
python3 perf/c10_qchunk_sign/ambient.py "$OUT/ambient.jsonl" < "$FIFO" &
AMBIENT=$!
exec 9>"$FIFO"
finish() {
...
for SIZE in 512 298; do
  TT_VISIBLE_DEVICES=0 TT_BIO_LEASE_CARDS=0 TT_BIO_LEASE_HOLDER=worker:c10-qchunk-sign \
    python3 perf/c10_qchunk_sign/capture.py --size "$SIZE" --out "$OUT/$SIZE"
done
