...
FLAGS = {"hoist": ("TT_BIO_DIT_COND_HOIST", "_B2_DIT_COND_HOIST"),
         ...
         # c14-matmul-ceiling: a fitted in0_block_w for the DiT's four square token projections,
         # which ttnn derives as 1 over a 24-tile K.
         "bw": ("TT_BIO_MM_SHORT_M_BW", "_MM_SHORT_M_BW")}
...
ARMS = {"base": (False, False, False, False), "silu": (False, True, False, False),
        "hoist": (True, False, False, False), "both": (True, True, False, False),
        "eltwise": (False, False, True, False), "all3": (True, True, True, False),
        # c14-matmul-ceiling's arm, alone and on top of the stack c13 is landing.
        "bw": (False, False, False, True), "both_bw": (True, True, False, True),
        ...
        "hoist_elt": (True, False, True, False), "silu_elt": (False, True, True, False),
        "default": None}
...
def main() -> int:
    ...
    import tt_bio.eltwise_fusion as EF
    # On a checkout without the eltwise lever merged there is no `mask_add` to wrap. That is fine
    # for an arm set that does not use it, and fatal only if one does -- `check()` already asserts
    # the attribute for those arms, so the counter is simply left at 0 here rather than importing
    # a dependency the measured arms do not touch. (A hard `T.mask_add` lookup here is what made
    # this harness unusable on main.)
    _mask_add = getattr(T, "mask_add", None)

    if _mask_add is not None:
    ...
    def fold(arm, rep, pos):
        ...
        if want is not None:
            ...
            setattr(T, "_MM_SHORT_M_BW", want[3])
        ...
        return {"arm": arm, "rep": rep, "pos": pos, "cold": rep < 0,
                ...
                "bw": bool(getattr(T, "_MM_SHORT_M_BW", False)),
                "bw_taken": int(T.MM_SHORT_M_STATS[0]),
                "bw_declined": int(T.MM_SHORT_M_STATS[1]),
