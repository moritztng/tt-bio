#!/usr/bin/env bash
# of3t-denoise: one arm of our full step on qb2 card 2 (p300c), replaying fullstep64's rollout draws.
#
#   devarm.sh <TAG> on|off [BATCH]    (DEVSTEP_EXTRA="--denoise ..." passes through)
#     -> perf/of3t_denoise/DEV_<TAG>.json, /home/ttuser/of3t_denoise/grad_<TAG>.pt
#
# of3t-inproj's devarm.sh with the card, worktree and row changed. The writer stamps host, board,
# card and host_quiet into the artifact (D155/D249); AICLK is trainfwd_run's DURING sampler.
set -uo pipefail
W=/home/ttuser/.coworker/wt/of3t-denoise
...
BARG=(); [ -n "${3:-}" ] && BARG=(--batch "$3")
DRAWS=/home/ttuser/of3t_fullstep64/draws.pt
...
ENV=(TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD
     TT_BIO_LEASE_HOLDER=worker:of3t-denoise OMP_NUM_THREADS=8 PYTHONPATH="$W")
...
echo "$QUIET"
env "${ENV[@]}" timeout 7200 python3 perf/of3t_denoise/devstep.py --exact "$EXACT" \
  --draws "$DRAWS" --grad-out "$S/grad_${TAG}.pt" "${BARG[@]}" --out "$OUT" ${DEVSTEP_EXTRA:-} 2>&1 \
...
with open(g, "rb") as f:
    for b in iter(lambda: f.read(1 << 22), b""):
        h.update(b)
d["provenance"] = {"host": socket.gethostname(), "row": "of3t-denoise", "card": int(card),
                   ...
                   "exit": int(rc), "grad_dump_sha256": h.hexdigest()}
