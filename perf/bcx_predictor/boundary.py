...
mo, zo = dev.stack(dev.up(m), dev.up(z), 0, 48, ckpt=False)
dev.sync()
...
out = {"msa_in_shape": list(cap["msa_in"].shape), "pair_in_shape": list(cap["pair_in"].shape),
       ...
       "stamp": A.stamp(3)}
(HERE / "boundary_check.json").write_text(json.dumps(out, indent=1, default=str))
