...
STEPS_PIN=${STEPS_PIN:-12}
STEPS_DRAW=${STEPS_DRAW:-20}
# PROVISION=0 re-runs the arms on a box that is already set up. The dataset is 1.8 GB
# and the install is a few minutes; neither needs repeating to re-take a measurement.
...
if [ "$PROVISION" = 1 ]; then
  ...
  echo "=== PROVISION=0: reusing the box as provisioned ==="
  cd "$ROOT/openfold-3"
