#!/usr/bin/env bash
# of3t-f64route: score a trunk arm IN FRAME on qb1, with of3t-frame384's own scorer.
#
# Identical invocation to of3t-trunkceiling/score_qb1.sh except for the arm's board: these arms
# run on qb1's p150a, not qb2's p300c, which is why CTRL_B (trunkceiling's CEIL_B lever set) is
# scored beside ROUTE_HF. The references are the same pair, on the host that built them (D189:
# the bf16 denominator is host-dependent at the percent level, the float64 one is not).
#
#   score.sh <TAG> <arm.pt> <card>
# CPU only. No card is opened.
set -uo pipefail
D=/home/ttuser/of3t_frame384
M=/home/ttuser/.coworker/wt/of3t-f64route/perf/of3t_f64route
PY=/home/ttuser/tt-bio-dev/env/bin/python
export PYTHONPATH=$D/pkg

TAG=$1; ARM=$2; CARD=${3:-2}
OMP_NUM_THREADS=8 nice -n 10 "$PY" $D/pkg/of3t_frame384/frame384.py \
  ...
  --arm-built-on "qb1 (tt-quietbox) card $CARD, p150a Blackhole -- NOT the p300c the 1.0293953378 and 0.5547455957585244 arms were built on; CTRL_B is the same-board control" \
...
m = d["MATCHED"]
BAR = 0.5268825372815341           # the in-frame A26 bar (D218); 0.4361680548 is withdrawn
SHIPPED = 1.029395337772341
CEIL_HF = 0.5547455957585244
...
print(f"  A26 bar {BAR}  -> {b / BAR!r}x the bar, {'PASSES' if b <= BAR else 'MISSES'}")
print(f"  shipped {SHIPPED} -> this arm is {SHIPPED / b!r}x better")
print(f"  trunkceiling CEIL_HF {CEIL_HF} -> ratio {b / CEIL_HF!r}")
