...
for i in 1 2; do
  ...
  sh perf/of3_mm/run_fold.sh ab5_base_$i 0 > perf/of3_mm/logs/ab5_base_$i.log 2>&1
  git checkout -q HEAD -- $F
  sh perf/of3_mm/run_fold.sh ab5_fix_$i 0 > perf/of3_mm/logs/ab5_fix_$i.log 2>&1
done
git checkout -q HEAD -- $F
echo AB_DONE
