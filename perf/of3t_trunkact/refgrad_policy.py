...
def main() -> int:
    ...
    ap.add_argument("--policy", default="f64", choices=("f64", "f32", "bf16auto", "bf16full"))
    ...
    dt = {"f32": torch.float32, "bf16auto": torch.float32,
          "bf16full": torch.bfloat16}.get(a.policy, torch.float64)
    ...
    rep = {"what": __doc__.strip().splitlines()[0], "tree": a.tree,
           ...
           "dtype_policy": {
               ...
               "bf16full": "every parameter and every activation bfloat16, autocast OFF, so the "
                           "normalisations and the softmax run at bf16 too -- the policy a bf16 "
                           "device kernel implements. of3t-trunkact added this arm; it is NOT a "
                           "claim about what upstream runs, it is the other end of A27's axis",
           }[a.policy],
