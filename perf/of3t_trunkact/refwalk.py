#!/usr/bin/env python3
"""Upstream 0.4.3's 48-block pairformer stack at padded N=384, in float64 AND in bf16 autocast,
walked block by block in ONE process, scoring our device dump at every block.

Three arms, one function by construction (one boundary, one checkpoint, one interpreter):
...
def main() -> int:
    ...
    rep = {"what": __doc__.strip().splitlines()[0],
           ...
           "dims": dims, "load_f64": load64, "load_f32": load32,
           ...
           "abs_err_first_difference": {
               ...
               for nm in list(arms) + ["bf16"] for leaf in ("s", "z")
