...
def arm_C():
    a = lin(w1)
    b = lin(w2)
    ttnn.multiply_(a, b, input_tensor_a_activations=[ttnn.UnaryOpType.SILU])
    ttnn.deallocate(b)
    return a
...
try:
    res["C_inplace_act"] = bench("C  linear + multiply_(a_act=SILU)", arm_C)
except Exception as e:
    print("C rejected:", str(e)[:120])
...
json.dump(res, open("perf/attn_block/silu_fuse2_qb2c1.json", "w"), indent=1)
