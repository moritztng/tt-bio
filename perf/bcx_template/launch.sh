#!/bin/bash
# bcx-template runs on qb2 card 0, in the environment bcx-seam and bcx-template used.
#   launch.sh <out-name> <script.py> <script args...>
# The script gets --out perf/bcx_template/runs/<out-name>. DUMP=1 adds the optimised-HLO dump
# perf/bcx_seam/hostmap.py needs.
set -u
WT=$(cd "$(dirname "$0")/../.." && pwd)
cd "$WT" || exit 1
NAME=$1; SCRIPT=$2; shift 2
OUT=$WT/perf/bcx_template/runs/$NAME
mkdir -p "$OUT"
export TT_VISIBLE_DEVICES=0 TT_BIO_LEASE_CARDS=0 TT_BIO_LEASE_HOLDER=worker:bcx-template
