...
COLS = {
    ...
    "trisc0_wait_front": "DEVICE COMPUTE CB WAIT FRONT [ns]",
    "trisc2_reserve_back": "DEVICE COMPUTE CB RESERVE BACK [ns]",
...
SUMS = [k for k in COLS if k.startswith(("trisc0_", "trisc2_", "br_", "nc_"))]
...
def aggregate(rows, op_filter=None, min_ns=0):
    ...
    for o in sel:
        f = frac(o)
        if max(f["trisc0_wait_front"], f["dm_noc_read"]) > 1.05:
            over += 1
        for k, v in f.items():
...
def main():
    ...
    for op, g in list(res["op_codes"].items()) + [("ALL", res["ALL"])]:
        ...
        print(f"  compute    wait-front {fr['trisc0_wait_front']:6.1%}   reserve-back {fr['trisc2_reserve_back']:6.1%}")
        for r, name in (("br", "BRISC "), ("nc", "NCRISC")):
            print(f"  {name}     noc-read {fr[f'{r}_noc_read']:6.1%}  noc-write {fr[f'{r}_noc_write']:6.1%}  "
                  ...
                  f"unclassified {fr[f'{r}_unaccounted']:6.1%}")
