...
Composed 2026-09-19 20:57:41Z from `origin/main` at `72596df4f`.
Head `10bac80c0`, **326 commits ahead**,
carrying **14 of 14 rows**.
...
| `of3t-diffusion` | `3129a846c` | `perf/of3t_orchestrator/audit_evidence.py`,`perf/of3t_orchestrator/compose_verify.sh` `perf/of3t_orchestrator/COMPOSITION.md`,`tests/test_tape_embedding.py` `tt_bio/autograd.py`,`tt_bio/taped_ttnn.py` `tt_bio/tenstorrent.py` |

**Shape of the diff**, because 300-odd files is three very different piles:

| pile | files | lines | what it is |
|---|---|---|---|
| **engine** | 19 under `tt_bio/`, none vendored | **1430 insertions(+) 137 deletions(-)** | the part that changes behaviour |
| vendored upstream | 33 under `tt_bio/_vendor/openfold3/` | 15198 insertions(+) 18 deletions(-) | their pipeline, carried in |
| campaign artifacts | 342 under `perf/of3t_*/` | the bulk | measurements and instruments |

Total  411 files changed, 331201 insertions(+), 166 deletions(-).
