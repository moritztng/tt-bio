...
### 3z. PRECONDITION — both sides must be on the SAME BOUNDARY, and it is CHECKED, not asserted

**Nothing in §3 is evidence unless our arm and the reference were produced on the same boundary:
the same batch, the same entry activations, and the same incoming cotangent.** A per-parameter
comparison across two boundaries is not a weak measurement of our gradient, it is a measurement of
something else, and it reads like a precision defect.

This protocol had no BOUNDARY rule until pass 378, and the campaign paid for the omission three
times (**D214**, **D218**, **D237**) on the same section. The separate obligation that would have
caught it earlier — *score the best possible artifact, ask whether a faithful reproduction can
reach the bar* — was already required by **A31** at pass 319 and went unasked of this clause for
fifty-nine passes. The last of them: the graded model artifact
scored 2,736 `pairformer_stack` tensors against a reference computed on a different boundary. The
two boundaries' own **float64** references disagreed at **1.8416532191 at cos 0.36514** with no
device op on either side, **82.93 %** of the failing term was that mismatch, and a **bit-exactly
perfect** trunk still read **3.1624x** the bar. The clause could be neither passed nor failed.

**The rule.** Every arm in a §3 comparison records the boundary that produced it — batch id, the
digest of the entry activations, the digest of the incoming cotangent — and any artifact
comparing two arms asserts those three agree. A comparison whose sides disagree on any of them is
reported as a boundary mismatch and **not** as an accuracy reading.

**The cheap detector, for when provenance is unavailable or disputed.** A frame-matched arm
differs from its reference by ROUNDING, so it stays nearly parallel to it and keeps its magnitude.
Two different problems do neither. So for every section, report

    mass_weighted_cos vs the float64 reference        frame-matched: ~1
    mass_weighted_norm_ratio                          frame-matched: ~1

and treat a section that is an outlier on BOTH as boundary-suspect until its provenance is shown.
Measured on the artifact the gate reads: ten sections at **cos >= 0.9945882130143565** and norm
ratio **0.9291-1.0211**, against `pairformer_stack` at **cos 0.1796088253016317** and norm ratio
**1.9890**. The separation is three orders of magnitude wider than the thing it has to
distinguish, which is why a cheap test is sufficient. Note the detector is **arm-scoped**: on the
older shipped arm `diffusion_transformer` also reads cos 0.4187, so run it on the arm you are
about to quote and not on a neighbouring one.

**A corollary that cost a pass on its own.** A ratio measured in one frame may not be carried into
another, even as a ratio. `x_ours / x_theirs` on boundary P is not `x_ours / x_theirs` on boundary
Q when P and Q compute different gradients, and an "aim" defined on P is not the bar a clause on Q
applies. D218 chose the in-frame pair as the target while the gate kept reading the model-frame
pair; both pairs were internally frame-clean and they disagreed **2.31x**.
...
**A34 — 2026-09-22, raised by `of3t-orchestrator` from D237, §3 gains a new PRECONDITION §3z.
Numbers existed and two targets are withdrawn.** The protocol never required the two sides of a
§3 comparison to be on the same boundary, and every guard the campaign built checked our arm
rather than the comparison. D237 found the consequence: the clause that is the campaign's only
open charter condition was scoring 2,736 trunk tensors across two boundaries whose own float64
references differ at 1.8416532191 at cos 0.36514, with no device op on either side. 82.93 % of
the failing term was that mismatch and a bit-exactly perfect trunk still read 3.1624x the bar.
§3z now makes boundary provenance a recorded, asserted precondition and supplies a cos/norm-ratio
detector for when provenance is unavailable or disputed.

**What is NOT new here, and saying so is the point.** §3z's boundary rule is new. The check that
*found* the problem is not: **A31 already required it at pass 319** — *"a bar is fixed before the
numbers, and it must also be checked against the SOURCE of the thing being reproduced before the
numbers... `break_control` proves a clause can report MET on a synthetic artifact; that is
reachability in the synthetic world, and it is not the same question as whether a faithful
reproduction can reach it. Ask the second one explicitly."* Nobody asked it of the GRADIENTS
clause for **fifty-nine passes**, 319 to 378, while six rows measured against it. So this is a
compliance failure against an existing obligation and not a gap in the protocol, and the durable
fix is the one A31 already prescribed: **score the best possible artifact before writing or
believing a bar.** `perf/of3t_orchestrator/framegate/score_the_best_possible_artifact.py` does it
for this clause in one CPU-second and should be run for every condition.

Affected results, stated rather than quietly dropped: the trunk's **in-frame** aim of 1.9537x and
the derived *"95.01 % of the required factor, 5.25 % left"* are **withdrawn as targets** — they
are on the local boundary, not the one the clause applies. The clause's own frame-clean
requirement is 4.5173x on the contaminated artifact, and the honest figure awaits
`of3t-modelframe`'s frame-matched arm. Nothing about the other ten sections changes: they were
measured frame-matched and §3z's detector confirms it independently.

**A35 — 2026-09-22, raised by `of3t-orchestrator` from D237 UPDATE 2, §3 unchanged, one more
target withdrawn.** **A lever reading is not a target until its forward and its backward are the
same function.** `CEIL_HF`/`VERB_HF` reads trunk 0.5547455957585244 and reproduces to ratio 1.0 —
and on the 912 untaped blocks of the checkpointed forward it takes each block's output from the
device softmax and its Jacobian from the float64 one, because `autograd.checkpoint` runs its first
forward under `no_grad` and `taped_ttnn` dispatches to a verb only on a taped Tensor. Forward and
backward disagree: not shippable, and not the gradient of anything. I projected the charter clause
on it at pass 377. The honest lever is `ROUTE_HF` at 0.7734340172378431, 7442 of 7442 blocks with
both the same softmax, and the projection moves from **0.8986x PASSING to 1.0674x FAILING**. Any
protocol claim quoting a lever now names its forward/backward consistency; **"reproduces exactly"
is not that check** — the hybrid reproduced exactly, scored better, and was wrong.
...
**A37 — 2026-09-22, raised by `of3t-orchestrator` from R130, §9 gains a boundary on what this
campaign may publish as a target. A number already existed; this amendment moves no tolerance.**

Twice in two passes the orchestrator published a projected clause reading as the thing rows were
to aim at, and twice it was wrong. Pass 378 projected 0.8986x PASSING from a lever whose forward
and backward were different functions (R129). Pass 378 corrected it to 1.0674x FAILING by
carrying an in-frame multiple across a frame change; `of3t-modelframe` measured 1.7814x, 1.3295x
outside the pre-registered 15 % band (R130). The cause of the second is stated in the brief that
commissioned it: the model boundary carries 3.12x less trunk gradient mass, so the relative error
there could move either way, which is the definition of a quantity that has to be measured.

**The rule. A projection may be published as a PREDICTION, never as a TARGET, and it must carry
its own falsifier and a named frame.** Concretely, in any of3t document:

- A projected reading is written with the word *projected* or *predicted*, the frame it was
  computed in, and the reading that kills it. `of3t-modelframe`'s brief did all three and that is
  why the projection died loudly at 1.3295x instead of being quietly carried. Copy it.
- **No row's bar, allowance or done-check may be derived from a projection.** Bars come from the
  reference's own properties (A26 is the model) or from §3's fixed tolerances. This is the clause
  that would have prevented the damage: a projection is cheap to be wrong about, a bar is not.
- **A ratio measured in one frame may not be divided into a threshold in another** (D214, D218,
  already law); A37 adds that it may not be *multiplied* into one either, which is the form both
  of these took.

**What this does not say.** Projecting is useful and should continue — the 1.0674x projection is
...
## A44 — 2026-09-24, pass 432. A SECTION WHOSE LABEL IS A FUNCTION OF THE PREDICTION IS GRADED ON THE MEAN OVER PRE-REGISTERED DRAWS. THE 3x CLAUSE DOES NOT MOVE.

`of3t-paez` (STOP, `perf/of3t_paez/LABEL_NULL.json`, `GRAD_SPLIT.json`) located D267 in the pae
LABEL, which bins `pred_xyz` at 0.5 A (`tt_bio/train/losses.py:323`). One denoise draw's
label-carried gradient is one sample from a wide distribution: iid structure noise at the
device's residual RMS carries 0.034 +/- 0.023 (24 seeds) and the device reads 0.066 (+1.4 sd);
at bf16's RMS it carries 0.068 +/- 0.034 and bf16 reads 0.024 (-1.3 sd). The device's structure
is closer to float64 than bf16's (0.035 A against 0.052 A RMS). A single-step ratio on this term
measures the reference's luck.

**THE RULE.** For every section, the GRADIENTS-SECTIONS ratio is `mean_k rel(ours, k) /
mean_k rel(bf16, k)` over K = 6 denoise draws: draw 0 is CF384's own draw (bf16's lucky one,
kept, which makes the bar harder), draws 1-5 use noise seeds 1-5, fixed here before any of them
exists. The float64 reference is re-run per draw; each side is graded against float64 at the
SAME draw. The bar stays 3x. The rule applies to every section, not only pae, so no term is
special-cased after its result. The five GRADIENTS fields must hold on EVERY draw, which is
stricter than today's one.

**WHY THIS IS NOT A TOLERANCE CHOSEN AFTER THE RESULT.** The threshold is unchanged; the
estimator of "upstream bf16's own rel" goes from n=1 to n=6, and draw 0 is kept although it is
the draw that made the bar low. It is the flattering direction for pae and the report says so:
the single-draw figure (6.07x) is published beside the mean, and if the mean still exceeds 3x
D267 stays open as a defect. `a-ratio-is-not-a-defect-until-the-references-own-ratio-is-measured`.

## A45 — 2026-09-25, pass 443. THE MASS FIELD IS GRADED PER TENSOR ON THE SIX-DRAW MEAN, THE SAME ESTIMATOR A44 GAVE SECTIONS. THE 0.95 DOES NOT MOVE.

A44 moved sections to the six-draw mean because one draw's bf16 rel measures the reference's luck,
and then kept `mass_at_or_better_than_bf16` per draw, per tensor, binary. That was inconsistent:
the field is the same single-draw bf16 comparison at a finer grain, where luck is larger.
Measured (`perf/of3t_orchestrator/DRAWMASS.json`, `01182ed2e`; the instrument reproduces all six
published per-draw figures to four places): the 24 `diffusion_transformer.blocks.N.attention_pair_bias.layer_norm_a.layer_norm_s.weight`
tensors hold 46 % of the float64 mass, read 0.164 against bf16's 1.172 on the mean (7.1x
better), and on each draw a DIFFERENT 1 to 3 of them lose. bf16's own rel on blocks.10 spans
0.17 to 1.36 across draws. Draws 3 (0.836) and 5 (0.691) fail on those near-ties.

**THE RULE.** Per tensor, ours is at or better than bf16 when `mean_k rel(ours,k) <= mean_k
rel(bf16,k)` over A44's six draws; each tensor weighs its mean normalised float64 mass. The field
holds at >= 0.95. The per-draw figures stay published beside it, and a tensor that loses on 5 or 6
of 6 draws is listed as a standing loser and triaged as a defect whatever the field reads.

**WHY THIS IS NOT A TOLERANCE CHOSEN AFTER THE RESULT, AND WHERE IT IS.** The threshold is
unchanged and the estimator is A44's, applied where A44 should have applied it. It is written
after draws 3 and 5 were read, and it is the flattering direction for them; that is stated here and
in the verdict. What keeps it from being a free pass: a tensor that is systematically worse loses
on the mean just as it does per draw, and the rule finds one (`aux_heads.distogram.linear.weight`,
1.44x on all six draws, 0.86 %, D270). Result: **0.9913**.

## A46 — 2026-09-25, backward sprint. A BACKWARD KERNEL'S WIN IS GRADED ON THE VJP, PRICED AGAINST THE ROOF THAT BINDS, AND HELD AGAINST INFERENCE.

The campaign's subject changed on 2026-09-25 from whether the gradients are correct to how fast
they are computed. This document's first duty is unchanged: the rule is written before the numbers
exist. A46 is that rule for a fused backward kernel. It **adds** conditions; it moves no bar in
§3-§7 and it relaxes nothing.

1. **The VJP is what is graded, never the forward.** A fused backward is accepted on its
   vector-Jacobian product against a float64 reference that is itself validated by central finite
   differences, reported as a per-tensor worst case located by parameter path. A forward check is
   structurally blind here and this fleet has the measurement: a bfp8 arm read forward cosine
   0.99999 and backward Inf/NaN on the same op. Instrument A (§3) and its bars apply unchanged.

2. **Every claimed saving names the roof that binds the part it saves.** State the part's measured
   arithmetic intensity and its measured machine balance, and price the saving against the roof
   that actually binds — not the one that flatters. This campaign spent 18 passes on a phantom 4.4x
   because a DRAM-bound op was priced against a compute roof measured on a dense 4096 cube. A
   saving with no named binding roof is not a result, and "no headroom here" is a result.

3. **The axis, the clock and the board are part of the number.** Seconds off the recorded 466.70 s
   crop-384 step; arms interleaved on one board; AICLK sampled DURING the arm, not before;
   `host_quiet.py` green; board class named, because qb1 p150a and qb2 p300c run the same 512 aa
   fold 17.39 s against 14.59 s at the same 1350 MHz. A number without a clock is not a measurement.

4. **Inference is held. This is a hard stop, not a bar to be traded.** Moritz, 2026-09-21: *"make
   sure regular inference is not changed... i dont want to see regression in inference."* Every
   kernel in this sprint is engine-level, and the attention and normalisation sites are shared by
   every model in tt-bio. A backward whose change can be reached from an inference fold owes an A/B
   against an A/A floor on every model that executes the site, with byte-identical digests reported
   first, before it is proposed as a default. If speed or fidelity can only be reached through a
   change inference would feel, the row says so and stops; that is Moritz's decision, not a row's.

5. **Adapt before you author, and cite what you took.** tt-metal's training library already ships
   eight backward kernels (`tt-train/sources/ttml/metal/ops/`). A row that writes one of them again
   must say what it tried and why it did not fit. A row that took from FlashAttention's backward
   recomputation, DeepSpeed's `DS4Sci_EvoformerAttention` or XLA's fusion over the gradient graph
   cites the file or the paper. "Took inspiration" with nothing named does not satisfy this clause.

6. **Gates are batched, and a stack is approved as a stack.** Land several levers, then one strong
   accuracy gate over the batch: the fp32 CPU diffusion reference is not reproducible run to run
   (7.17 A against 11.04 A on the same design), so consulting it per micro-step produces noise
   rather than safety. Stack perturbations are strongly sub-additive, so a stack is approved whole
   or not at all and never by summing its members' individual readings.

**What A46 does not license.** It does not license a speedup that does less of the model's own
work — fewer steps, fewer recycles, a skipped term — which remains a hard stop whatever it measures.
It does not license tensor parallelism. And it does not license quoting 5x chip-to-chip: the
hardware ratio is ~8.5x compute and ~11x bandwidth, so ~6-7x of the measured 58-67x is software and
that is the whole of what this sprint can win.
