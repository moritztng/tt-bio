...
LEDGER: `~/.coworker/state/of3t/LEDGER.md`, **R1-R199 and K1-K19**. **This field is an INDEX, not
...
ROWS: **one hundred thirty-six dispatched, one hundred thirty-four concluded** (counts restamped from disk by `perf/of3t_orchestrator/stamp_row_counts.py` at the next compose, which is the only thing that may edit them). **Live at pass 424: `of3t-denoise` (DEFERRED to 23:20Z; arms DN384R on qb2 card 1, DN384RB on card 3), `of3t-pwaslice` (qb2 card 2 once DN384G exits; D262, dispatched pass 424).** Concluded this pass: `of3t-inproj` GO (232/232 in-projection leaves carry a gradient, inference op trace identical) and `of3t-bcastaudit` GO (0 D258/D259 sites in 1,479,561 wrapped calls over six models; D259 is campaign-internal). History: `state/of3t/PASSLOG.md`.

SEQUENCE: **pass 424.** The final trunk+diffusion score is `of3t-denoise`'s DN384R/DN384RB on b770aee71 (D261 fixed), ~23:20Z; DN384G/GB (b22e9a6bf, embedder untrained) are provisional and give the cross-card A/A of that code. The reference's A40 passes (6.972e-10 at h=1e-6). `of3t-pwaslice` fixes D262 in parallel and proves every other gradient bit-identical to DN64R, so its result stacks onto the DN384R score without a re-run: it only adds a backward edge into 9 leaves that had none (8e-7 of mass). Compose at pass 424 needed two new resolvers: main's MGX PairformerLayer now passes `add_to_input=True`, which the composed Transition refuses together with a mask, and of3t-msafwd's fp32 pair residual cannot use it at all; the layer takes main's block when plain (every inference call) and the row form when taped-wide or masked. The composed training path through main's new form has not run on a device. Held: the SLZ arm on both boundaries (D252). History: `state/of3t/PASSLOG.md`.
...
GAP: **Pass 424.** (0) Closed: D259/D258 reach no inference fold (`of3t-bcastaudit`, 0 of 1,479,561 calls). (1) D262: 9 MSA-module PWA leaves (`pwa.{m,g,o}_weight`) train as constants because a raw `ttnn.Tensor.__getitem__` bypasses the tape; `of3t-pwaslice` owns it. (2) The 384-token score of D254, D255 (resolved mask), D256 (84 late-registered diffusion weights), D257 (diffusion module ungraded), D260 and D261 on DN384R/RB against the validated float64 reference, with the cross-card A/A; `of3t-denoise`. D254 is fixed at 64 (`of3t-inproj` GO) and composed at pass 424. (3) Both arms sit above upstream bf16 on the full step (global 0.146 vs 0.100 at 384 before the fixes; MSA-side 2-3x bf16), D250's bf16 pair residual, Moritz's call, owed an Angstrom reading. (4) D252: exact LayerNorm chosen on a pad-dominated boundary; SLZ arm on both boundaries still held. (5) Confidence-head pae/pde/plddt: the float64 reference has no such head (1e-7 of mass), and plddt does not fire because its labels are absent from the batch; path coverage for plddt is unread. (6) Open question from DN64: pae 4.1104 (b770aee71) vs 4.1200 (b22e9a6bf) with trunk terms identical; the 384 cross-card A/A decides whether it is card-dependent. Standing: 2.0150 % unread mass; crop 640 is a single-card capacity wall. Earlier GAP text: PASSLOG.
...
**Counts** (`stamp_row_counts.py`): `state/concluded` holds **one hundred thirty-six** of3t files, so **one hundred thirty-four rows have concluded**.
**Two hundred sixty-two defects filed**, **98 UNFIXED** (5
