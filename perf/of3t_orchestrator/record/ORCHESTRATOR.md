...
VERDICT: PARTIAL, re-stamped pass 355, 2026-09-22 — **still working, which is what PARTIAL means.**
**One hundred three** of3t rows concluded, **two hundred eight defects** filed, **75 UNFIXED**, counted
...
**What fails — and GRADIENTS misses two of its three clauses, not three.**
    ...
    TRAJECTORY  scope 88.8350 % against 89.2106 %, REPOINTED pass 355 to the widest capture's
                own span -- and `scope.coupled` is required and unemitted, so two clauses miss
    COVERAGE    MET -- 11 of 11 paths fire on a real end-to-end step, 8 of 8 loss terms,
                inference byte-identical verified twice

**What is verified.** The update rule's four state-free factors are exact or at 1e-06 under an
injected drive. **Outside the
...
**The trunk FAILS frame-matched at the width we report (D191), and it is the campaign's only large
accuracy object.** At 384 ours **0.8354121633** against upstream's own bf16 floor **0.3739383921**
is **2.2341x**, **1.586x outside** the in-frame A26 bar 0.5268825373; at 64 the same scorer reads
1.0251x, and the floor moves 1.0000022x where our arm moves **2.1795x**. Growth 0.5510 mw², denominator width-invariant to
3.20e-15. **LOCALISED pass 352**: a **STEP created in the backward of blocks 45
and 44**, not a per-block factor — rung 44 reads **2.201x** against the model-scope 2.1795x, so all
of it is present four blocks in, decaying monotonically thereafter. **100 % of it is ours** (the
float64 reference is bit-identical across the two widths at all 49 rungs). Softmax is refuted as
the carrier by decomposition: **6.3 % better per firing** at 384, the pooled 1.7797x being
reweighting. `of3t-blk4544` owns naming the op.
The **93.80 % on LayerNorm affine is the CARRIER**, its excess width-INVARIANT while the whole
upstream group grows together; the leaf's own wrongness (**7.3729 at cos -0.0057**) is a separate
object. **D187 is LOCATED and smaller than filed**: `diffusion_transformer` reads **1.3501x** in
A26's own form, not 2.019x, which was a floor ratio — and a precision floor, upstream's own bf16
missing the per-tensor bar on 369 of 456 against our 354.
...
default-off, which is why GRADIENTS' coverage reads 97.98499 %.

**Merge story:** one branch, `wk/of3t`, rebuilt from `origin/main` every compose. Main holds none
of it and will not without Moritz.
