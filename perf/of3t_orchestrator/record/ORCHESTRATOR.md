...
ROWS: **one hundred twenty-one dispatched, one hundred nineteen concluded** (counts restamped from disk by `perf/of3t_orchestrator/stamp_row_counts.py` at the next compose, which is the only thing that may edit them). **One live: `of3t-frameself` (qb2). `of3t-cotcoh` concluded at 20:39. None dispatched at pass 382 — the critical path is already owned and adding a row to it would be two agents on one question.**
...
can grade it.**
    ...
    aim       **The pair track, not the single track (R143).** `of3t-cotcoh`, measuring
              like-for-like from one entry, reads the single-track cotangent at **1.0583x**
              upstream's own bf16 and the pair-track cotangent at **4.9964x**. The campaign has
              been working the single track. This is the first reading that says where the
              remaining error lives rather than how big it is, and it is an in-frame quotient so
              it survives D242. **D240 is dead** (R144): 4,983 of 5,569 taped tensors take a
              bf16 cotangent whatever their fan-out, and the downcast is load-bearing — the
              break control NaNs 33 of 48 blocks.
...
**Pass 382, two findings and a shipped fix.** The reference ran the trunk **checkpointed one
block at a time** (`blocks_per_ckpt: 1`, the `train` preset) while every `ref_grad.py` replay runs
a bare loop — a named candidate for D242 with a break control cheaper than the live row's own next
experiment, delivered to its brief (R141). D242 is a **regression, not an oversight**: the control
already existed as `of3t-conditioning`'s COTANGENT_COMPLETE, and `capture_model_frame.py` argued in
a comment that the check was impossible and shipped a provenance witness instead (R142). And
`of3t-cotcoh` concluded: **the aim is the PAIR track at 4.9964x, not the single track at 1.0583x**,
the first reading saying where the error lives rather than how big it is (R143).

**D243, a user-facing defect fixed this pass with no card.** `tt-bio finetune`'s fit planner was
