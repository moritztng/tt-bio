...
hundred five concluded, three live; `state/concluded` holds **one hundred seven** of3t files. **Two hundred fifteen defects filed**, **80 UNFIXED** (5
...
that loop at k = 20 and never above **1.3677x**. Outside the pairformer trunk the gradient is at upstream's own accuracy, and the honest
same-reference reading is stronger than the one this field used to carry (D215, pass 360 — the
old "0.945x to 2.176x" divided ours-vs-their-bf16 by theirs-vs-float64, two references). With
both sides against float64, **five of ten sections are strictly BETTER than upstream's own bf16
training step** — `aux_heads` 0.010x (0.0023 against their 0.2394, a factor of **100**),
`diffusion_conditioning` 0.126x on 36.95 % of the mass, `layer_norm_a` 0.255x, `atom_attn_dec`
0.395x, `msa_module` 0.499x — two are at parity, and the spread is **0.010x to 2.019x** outside
the trunk. Pooled over those nine with the arm's own `ref_sq` we read 0.0834300442 against their
0.0754253407, **1.1061x**, because `diffusion_transformer` at 43.62 % of the mass outweighs the
five wins. Against the clause's own reference the nine read **0.1006694647** against the
0.1473526833 bar — **0.683x, passing on their own.**
