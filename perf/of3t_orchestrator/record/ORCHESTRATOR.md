...
ROWS: **one hundred thirty-nine dispatched, one hundred thirty-eight concluded** (counts restamped from disk by `perf/of3t_orchestrator/stamp_row_counts.py` at the next compose, which is the only thing that may edit them). **Live at pass 426: `of3t-go384` (qb2 card 3), the GRADIENTS reading GO is decided on.** Concluded: `of3t-ieatom` GO (D263 and D264 fixed, CM64F bit-identical to PW64F, inference digest unchanged), `of3t-composed64` STOP (found D264). History: `state/of3t/PASSLOG.md`.

SEQUENCE: **pass 426.** `of3t-go384` runs one 384-wide step on the composed engine (tt_bio/ byte-identical to ieatom's scored 06b91c5ce) and scores it against ref384 float64; its bar is pre-registered in CHARTER_EVIDENCE GRADIENTS. After it: rerun charter_evidence.py, then the GO read. GO is also held by the 8 USER-FACING defects (D10/D24/D184 merges owed through land-standing, D210 release, D32/D55/D205/D250 card work); those are the next dispatch once go384 lands. Held: the SLZ arm on both boundaries (D252). History: `state/of3t/PASSLOG.md`.
...
GAP: **Pass 426.** (1) GRADIENTS charter condition: repointed pre-registered to the whole-step score `perf/of3t_go384/SCORE_GO384.json`; the arm has not run (`of3t-go384`). The superseded trunk-frame reading was 0.2207 against A26 0.1521. (2) Eight USER-FACING defects unfixed (D10, D24, D32, D55, D184, D205, D210, D250), see `perf/of3t_orchestrator/userfacing/CLOSURE_PLAN.json`; D264's main-side line cb2edd1d6 is owed to main. (3) D252: exact LayerNorm chosen on a pad-dominated boundary; SLZ arm held. (4) Confidence head pae/pde/plddt: the float64 reference has no such head (1e-7 of mass); plddt does not fire. (5) Only `__getitem__` was audited among ttnn.Tensor methods that could take a weight off the tape. (6) DN64R vs DN64G pae 4.1104 vs 4.1200, a code-version difference at weight 1e-4, unexplained. Standing: crop 640 is a single-card capacity wall. Earlier GAP text: PASSLOG.
