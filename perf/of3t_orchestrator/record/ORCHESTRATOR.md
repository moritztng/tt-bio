...
ROWS: **one hundred thirty-six dispatched, one hundred thirty-five concluded** (counts restamped from disk by `perf/of3t_orchestrator/stamp_row_counts.py` at the next compose, which is the only thing that may edit them). **Live at pass 425: `of3t-denoise` (launched 22:31 CEST to score DN384R/DN384RB, both arms exited 0 on qb2 at 20:04Z/20:30Z).** Concluded this pass: `of3t-pwaslice` GO (D262: the 9 PWA leaves carry gradient, better than bf16 against float64; the other 3365 tensors and the loss bit-identical; inference fold digest identical). History: `state/of3t/PASSLOG.md`.

SEQUENCE: **pass 425.** The final trunk+diffusion score is `of3t-denoise`'s DN384R/DN384RB on b770aee71 against the A40-validated float64 reference (6.972e-10 at h=1e-6); both arms are on disk and the row is scoring them now. `of3t-pwaslice` is composed (wk/of3t -> a700e9563) and its result stacks onto the DN384R score without a re-run: it only adds a backward edge into 9 leaves that had none (8e-7 of mass). GO then owes one 64-token taped arm on composed `wk/of3t` (a700e9563) scored against the 64 reference, because main's MGX PairformerLayer form has not run a training step on a device; dispatched once denoise reports. Held: the SLZ arm on both boundaries (D252). History: `state/of3t/PASSLOG.md`.
...
**Counts** (`stamp_row_counts.py`): `state/concluded` holds **one hundred thirty-seven** of3t files, so **one hundred thirty-five rows have concluded**.
