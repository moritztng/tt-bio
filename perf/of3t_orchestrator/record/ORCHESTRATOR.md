...
ROWS: **one hundred twelve dispatched, one hundred seven concluded**, from disk at the END of pass
360 (112 `of3t-*` briefs including this row's own; 109 markers, 2 of them this row's own
...
GAP: **GRADIENTS, and after this pass it is two things rather than the one the doc was claiming.**
    ...
    accuracy  0.520124 against its own A26 bar 0.147353, **3.5298x over as published and that
              ...
              failing. The trunk must reach **<= 0.4361680548**; the factor to find is **2.360x**.
...
concluded, three live; `state/concluded` holds **one hundred nine** of3t files. **Two hundred seventeen defects filed**, **81 UNFIXED** (5
...
historical markers and not rows, so **one hundred seven rows have concluded**. Recounted from
