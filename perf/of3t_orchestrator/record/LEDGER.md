...
- **R215** (pass 455, `of3t-exactscope`, NO-GO) the exact softmax **alone** costs **637.33 s of the step -- 25.25x the noexact step by itself** (fwd 195.76, bwd 441.57) [**total CORRECTED at pass 493 from a transcribed 612.09**: the components sum to 637.33, `PRICE.json`'s `arms.softmax.step_s` reads 637.33, and only 637.33 gives the 25.25x this entry already quoted -- 612.09/25.24 is 24.25x. The wrong total sat in the ledger and in `COST.md` from pass 455 and was one command from reaching a user-facing doc], against a noexact step of **25.24 s**, three arms on pc card 0 p150a as the box's sole device tenant, crop 384, 1 trunk cycle, taped, AICLK 1350 DURING. **So the layer norm is the SMALLER half of the exactness and the hoped-for narrowing would have bought little even had it cleared the bar** -- it fails at 1.3038x regardless. Two derived facts worth carrying: the softmax path is very nearly the whole of the exactness cost, so work is sized against it first; and `of3t-bwattrib` read **39.11 s** for the identical noexact arm under load against this row's quiet **25.24 s**, a **1.55x contention penalty** that bounds every contended figure taken on that box. Each arm asserts its scope from the mechanism on both legs (`exact_softmax_installed()` while the tape is open, plus the exact verbs' counters differenced per leg), which is what excludes the flattering failure mode of an arm that silently kept both ops and would have read as "softmax alone is nearly free".
...
- **K46** (pass 489, orchestrator — **my own state doc had TWO fields of the same name, and the gate read only one of them**) **I created a duplicate `GAP:` last pass by inserting a new field instead of replacing the old one, and neither I nor the DONE_CHECK would have noticed.** The doc carried `GAP:` at 951 B (pass 488's) followed by `GAP:` at 5,011 B (pass 457's). The gate's pattern is `^GAP:` and takes the **first** match, so it validated the new one while the old sat below carrying superseded content — *"there is still no full training step ... the reason is a memory defect"* (`of3t-stepqb2` has been running one since 21:00:37) and *"`AdamW.__init__` +6.00 GiB"* (a direct census puts it at **7.105**, R229). **A reader scrolling the doc and a gate matching a regex would have come away with different states of the campaign**, which is worse than either being stale. Found by listing the field sizes rather than by reading the doc — `re.finditer(r'^([A-Z][A-Z-]+):')` over the file prints the duplicate immediately, and reading top-to-bottom does not, because the second `GAP:` looks exactly like a section you have already read. **The general form: a document with named fields needs its field list checked, not just its prose.** One `grep -c '^GAP:'` is the whole audit. Merged into a single current field, 34,961 -> 31,931 B. **Same family as the `COST.md` restructure at pass 471** — that one had three overlapping sections on host memory; this one had two sections with the *same name*, which is the failure mode one step worse. Both came from appending under time pressure instead of replacing, and both were invisible until something enumerated the structure.

- **K47** (pass 493, orchestrator — **a number I had carried for 38 passes was wrong, and its own entry contained the disproof**) **R215's `612.09 s` was a transcription error; the figure is `637.33 s`.** The entry read *"612.09 s of the step — 25.25x the noexact step by itself (fwd 195.76, bwd 441.57)"*. **195.76 + 441.57 = 637.33.** `perf/of3t_exactscope/PRICE.json` records `arms.softmax.step_s = 637.33` and `arms.noexact.step_s = 25.24`, and **only 637.33 yields the 25.25x the entry itself quotes** — 612.09/25.24 is 24.25x. So the components, the ratio and the artifact all said 637.33 while the total said something else, and it propagated into `COST.md` and survived every pass that re-read both. **It was caught because I was about to write it into `docs/training.md` and checked the cited artifact first.** That is the whole lesson: the discipline that catches a stale number is not re-reading your own document, which I had done many times, but **reconciling it against the source at the moment you publish it** — a reader of the ledger sees a total flanked by its own components and does not add them. **Cheap general check: when an entry carries a total AND its parts AND a ratio, verify all three agree before quoting any one of them.** Three quantities, one degree of freedom; a disagreement is free to detect and invisible to skim.
- **K48** (pass 494, orchestrator — **`plan()` was refusing an OpenFold3 crop the engine can run, and the GAP item I had been re-publishing claimed the opposite defect**) **The OpenFold3 training crop wall moved 512 -> 576 on `origin/main` on 2026-09-23, and neither the shipped planner nor the docs noticed.** `of3t-cropwall`'s concat-heads split (`e558bfb06`, "Split the concat-heads gradient on dh, not on a dim of extent H") is an **ancestor of main**, and it reshapes exactly the allocation the 576 refusal named — *contiguity inside `ttnn::concat`, 6,671,522,304 B still free, short by 77,930,560 B per bank*. Post-fix **576 COMPLETES**, at two separate main-ancestor commits (`split_576_post.json` at `63b51514d`, `split_576_fix2_card3.json` at `e558bfb06`): backward DRAM high-water **30,230,471,680 B of the card's 34,225,520,128 B, 88.3 %**, qb2 p300c card 3, **median 1350 MHz polled DURING**. Meanwhile `tt_bio/train/dryrun.py:84` still carried `576: (None, 77_930_560, ...)` and `LARGEST_MEASURED_TO_FIT` still capped openfold3 at 512, so **`plan(model="openfold3", tokens=576)` returned `refused` on a documented command** — the same class of defect that per-model table was built to stop (D243), arriving the other way round: not a neighbour's wall borrowed, but this model's own wall outliving its cause. **Fixed on `wk/of3t-docsfix` at `1537aa409`** (release-gated, not merged): 544 and 576 drop out of `FORWARD_OOM_BY_MODEL` — absence means `UNMEASURED`, this module's own first-class answer — and the fit cap goes to 576, so both now answer `UNMEASURED` **with** *"the memory fits up to 576 aa, so this crop is expected to run"*, fully sourced. Verified by **exercising `plan()` before and after**, not by reading the literals: 544 and 576 go `refused -> UNMEASURED`, **640 stays `refused`** and 512 is unchanged. 640 was re-measured post-fix and still refuses, so that wall is not the concat one and its entry stays with the re-confirmation cited; 544 is unmeasured post-fix and is now claimed in neither direction, since it sits below a crop that runs. **Two things in my own record were wrong when written, and I had re-published both for 28 passes.** K38 said *"`git grep` over `README.md` and `docs/` on `origin/main` finds no crop or token limit stated anywhere"* — **false**: the limit landed 2026-09-22 (pass 382, `2ecd28c03`) as a bolded bullet inside `## plan(): what is measured, and where it says UNMEASURED`, and it was **better than what I would have written**, naming all four refusing sizes and distinguishing the two refusal mechanisms. K38 also said D248's fix was *"release-gated and unmerged, so it has not raised the limit on main"* — `merge-base --is-ancestor` says **merged**. **Why the grep missed it, and this is the reusable part: the capability sentence is written in the domain's own unit** — *"512 aa runs and is the largest that does"* — **not in the concept word I searched for** (`crop`, `limit`, `max`). A pattern built from the words I would use finds the *discussion* lines and misses the *statement*. **A negative grep result is evidence about your pattern, not about the codebase; before publishing "the docs never say X", open the section where X would live and read it.** **And a near-miss worth more than the finding.** Every one of these artifacts opens with `"doc": "Who owns the bytes at the backward's DRAM high-water, at a crop where it COMPLETES."` — a **static docstring the script writes unconditionally**, pre-fix and post-fix, on runs that completed and runs that died. I read it on the 640 file and briefly concluded 640 now ran too. The outcome is in the **body** (`FATAL ... Out of Memory`), and one grep for the failure language across all fifteen `split_*` artifacts separated them in a single command: 512 and 576 clean, **640 refusing at every post-fix commit**. **An artifact's self-description is its author's intent, not its result — grade it by its result fields.** Same family as K47 (verify the cited artifact before quoting it) and as `a-flat-metric-across-varied-inputs-is-not-measuring-them`. **What this leaves open:** 544 post-fix is one rung of a size ladder, and whether the *shipped training step* (not the `split` high-water probe) completes at 576 is a stronger claim than anything measured here — both are card jobs, deliberately not dispatched while `of3t-stepqb2` holds pc card 0 and the campaign's last EXIT item is mid-flight. Evidence: `perf/of3t_cropwall/out/split_{512,576,640}_*.json`, `perf/of3t_crop768/out/split_*.json`, `git merge-base --is-ancestor origin/wk/of3t-cropwall origin/main`, `tt_bio/train/dryrun.py` before/after under `importlib`.

- **K49** (pass 494, orchestrator — **my own lease-release rule, applied literally, would have taken a live row's card**) **A lease records the pid of the DEVICE RUN, not of the row that owns it, so between device runs it reads pid-dead AND node-free while the row is alive and about to re-open the card.** Observed on `state/leases/pc-card0.json`: `{"holder": "worker:of3t-stepqb2", "pid": 294984}` with **294984 dead** and `fuser /dev/tenstorrent/0` **free** — both of K34's release conditions satisfied — while `of3t-stepqb2`'s worker (187622) and its agent (199437, one turn, 22 min in) were **both alive**, the agent having spawned 294984 itself and outlived it. **K34's rule as I wrote it is "release only after asserting pid-dead AND device-node-free", and that is not sufficient; the missing conjunct is AND THE HOLDER ROW IS NOT ALIVE.** The `holder` field is the durable identity and the `pid` field is a per-run detail, so a reaper keyed on pid-liveness strips a live row's grant during **every gap between its device runs** — and the gap is not rare, it is most of a host-bound row's wall time. This is K43's distinction arriving from the third direction: there a *cleared cardblock* meant safe-to-touch rather than available-to-take, here a *dead pid plus a free node* means between-runs rather than abandoned. K34 was written to fix the opposite error (I left a genuinely stale lease and caged `of3t-xsplit` for 45 minutes), and the over-correction it invites is worse than the error it fixed, because caging a row wastes time while stripping a live row's lease corrupts a run. **Check `holder` before `pid`. A stale-looking lease whose named row is alive is not stale.** No action taken on `pc-card0.json`, which is correct and is the finding. Evidence: `ps -o pid,ppid -p 187622,199432` (parent/child, so not a duplicate launch either), `kill -0 294984`, `fuser /dev/tenstorrent/0`.

- **K50** (pass 495, orchestrator — **applying K48's risk test to every other measured refusal in the planner, and getting a clean negative**) **Protenix-v2's 384 and 512 aa refusals are NOT stale, and the arithmetic that says so is cheaper than a measurement.** K48's lesson is that a measured refusal is only true of the code it was measured on, so the obvious next question is whether the planner's *other* entries have outlived their causes. They are at risk on the face of it: the protenix-v2 numbers were measured **2026-09-18** (`1b39b18aa`, train-r5) and the table was last touched **09-22**, while **five** trimul/chunking commits landed on main on **09-23 and 09-24**, several of them explicitly making a refused block narrow rather than fail — `2a6362cb7` *"a refused block above SEQ_LEN_MORE_CHUNKING halves instead of ending the fold"*, `1f338604e` *"a refused sample chunk narrows instead of crashing"*, `6d4bc23ee` and `25fbc26e0` on the trimul norm row-block, and `72ea47311` moving an L1 mask to DRAM. **Every one of them is out of range, and the gate constants settle it without opening a card.** `SEQ_LEN_MORE_CHUNKING = 1536` on Blackhole, so the narrowing path is entered only above 1536 tokens and cannot reach 384 or 512; `TRIMUL_IN_NORM_ROWBLOCK_BYTES = 3 GiB` **per tensor**, and protenix-v2's trimul in-projection at 384 aa is ~37.7 MB, three orders of magnitude below it; `1f338604e` is the diffusion sampler, not the trunk forward under checkpointing; and `72ea47311` moves pressure **into** DRAM, which if it changes a DRAM refusal at all makes it more likely, not less. **So the entries stand, and the useful general form is: a refusal below every relevant threshold cannot be affected by a change that only fires above them — read the gate constants before booking a card.** That is the cheap half of K48's test, and it is available on any host. **What the check did surface, and it is a lead rather than a defect.** The planner's own comment says production's trimul chunking *"is built and the training path simply does not get it"*, and **that is still true** — `TRIANGLE_MULT_CHUNK_SIZE = 32` is an always-on chunk with no size gate, none of the five merges wired the taped path to it, and the description has been accurate and unfixed since 09-18. So protenix-v2's wall has a **named, built, unused mechanism** sitting in production code, which is K48's shape with the fix never landed rather than landed and unnoticed. **Whether it bears on OF3's own 640 refusal is the campaign-relevant question and it is NOT answered here**: the chunking decision at `tenstorrent.py:1199-1228` is priced against an **L1** budget (`TRIANGLE_MULT_L1_CHUNK_BUDGET`, chunk tensors `[batch, chunk, seq, seq]`), while both refusals under discussion are **DRAM** — OF3's 640 dies *"card full, 23,710,208 B free device-wide"*. Chunking the channel dim would shrink intermediates that may or may not be DRAM-resident at that shape, and **reading further can only choose the measurement, not replace it** (`a-workaround-in-an-instrument-is-an-unfiled-bug-report-about-production`). Recorded as the next lever candidate behind `of3t-stepqb2`, deliberately not dispatched: pc card 0 is held by the campaign's last EXIT item and all four qb2 cards are leased at loadavg 14.57/16. Evidence: `tt_bio/tenstorrent.py:25,181,212,1199-1228`, `tt_bio/train/dryrun.py:60-70`, `git log --since=2026-09-10 origin/main -- tt_bio/` filtered to trimul/chunk.

- **R231** (pass 496, `of3t-stepqb2` still live, arbitrated from its PUSHED commit `056bfe5f0` — a banked commit is not a live artifact, which is the K36 line) **The exactness-ON forward half is MEASURED at 375.908 s, the first exactness-ON arm in this campaign to carry its own DURING-sampled clock — and the same commit contradicts the numerator under the campaign's published GPU gap by 11.7x.** Three results, in the order they matter. **(1) The clock gap R224 recorded is closed.** R224 said *"no clean second exists for any OF3T exactness-ON arm"* — all four prior runs died before the harness writes `env.aiclk_during`. This one records **min 1343 / median 1350 MHz, n>250, phase-tagged, read off the sysfs class node** on pc card 0 (p150a, Gen4 x8), on `wk/of3t-optorder`'s tree merged into the row's branch. Every number below is therefore a number by this campaign's own clock rule rather than an artifact. **(2) The forward half is measured, not bounded, and it confirms the bound.** `375.908 s` = untaped trunk prefix 3.255 + taped cycle 301.16 + diffusion 65.415 + losses 6.078, and **the components sum exactly**. R224's `<= 386 s` bound holds and was tight — the measurement lands at **97.4 %** of it, which is a good outcome for a bound derived from two clock stamps and a subtraction. **The ratio it carries is a different story: 14.705x, not `<= 65.2x`.** The old figure divided the bound by a **5.919 s** forward taken on `451ed56f4`; the row re-took the denominator on its own board and tree and got a cold OFF forward of **25.563 s**. `COST.md` now carries `375.908 s against 25.563 s -> 14.705x`. **(3) And the finding that outranks both: the published 58-67x has a numerator nothing on the current tree reproduces.** The row measured the no-exactness step at **39.886 s steady (n=3, spread 0.865 s), 77.532 s cold**, and flagged that this *"disagrees with 466.702 s by 11.7x, which is more than staleness explains -- flagged, not diagnosed here."* **It is more than a row-local curiosity, because 466.702 s is the numerator of the GPU gap `COST.md` publishes**, and it came off `451ed56f4`, whose own commit subject reads *"a steady training step is **507.02 s**"*. **The caveat that row of the table used to carry was the wrong caveat**: *"on the pre-exactness tree, so it prices a configuration whose gradients do not clear the bar"* is an **accuracy** objection, and both 466.702 s and 39.886 s are **no-exactness** arms — so the accuracy framing implicitly conceded the seconds were sound while the seconds are exactly what is in doubt. A reader would have discounted the number for the wrong reason and kept trusting its magnitude. **The table now says DO NOT RE-PUBLISH UNTIL RE-DERIVED and names both problems separately.** **Direction check, applied deliberately:** the error runs the way that flatters Tenstorrent — a gap of ~5x rather than 58-67x — so it gets more scrutiny, not less, and I have **not** restated the headline. Three live explanations and I cannot separate them from here: **scope** (`of3t-stepfloor`'s "steady training step" may not be the same span as OFFSTEP), **cold vs steady** (the row's own cold figure is 77.532 s, and a cold-with-compile 466.702 s would explain most of the ratio), or **a genuine order-of-magnitude gain on the no-exactness path**, which would be a large positive result that must be named rather than buried. **The row has been briefed to BOUND it, not chase it** — "it is at least partly scope, and here is the term" closes it, and it was told explicitly to stop and say so rather than spend the card, with the ON full step still ranked first as the campaign's last EXIT item. Evidence: `056bfe5f0` on `wk/of3t-stepqb2`, `git log -1 451ed56f4`, `state/of3t/COST.md` rows 57-60.
