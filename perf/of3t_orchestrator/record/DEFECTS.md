...
### D254. Every triangle-multiplication in-projection is a registered leaf that receives no gradient. **UNFIXED, gradient-correctness (training path)** — found by `of3t-fullstep64` at pass 420 (F3). The fused in-projection chunks (`tt_bio/tenstorrent.py` `_gp_cache` / `_gp_gout_cache`; upstream `linear_{a,b}_{p,g}`) are walked and registered, and `.grad` is None after backward in every arm at every width 64-384: 473 tensors, 9.70e-4 of float64 mass, scored as zero. Those weights would never train. Whether dx through the projection is also dropped is untested. Assigned to `of3t-inproj`.

### D255. The resolved objective averages over pad tokens. **UNFIXED, campaign-internal (training path)** — found by `of3t-fullstep64` at pass 420 (F2). `tt_bio/train/objectives.py:269` calls `losses.resolved(logits, coord_mask)` without an atom mask: value f64 3.691, OFF 5.898, 3.619 with pad features +3, confidence rel 1.087 vs 0.944 under that perturbation. It is the confidence head's gap to bf16. Assigned to `of3t-inproj`.
