...
| **the backward, MEASURED on a real taped arm** | **663.3 s** — 432.15 s inside ttnn verbs (65.15 %), 231.15 s outside (34.85 %); crossings `to_torch` **178.41** + `from_torch` **139.13** = **317.54 s = 73.5 % of verb time**; `matmul` **0.85 s** over 4,672 calls at 675 GB/s | `of3t-stepqb2`, `bwprof.py --arm base`, crop 384, pc p150a Gen4 x8, **AICLK 1350 MHz DURING (n=299)**, loadavg 2.59 rising to 5.14 so absolutes are upper bounds. The artifact stamps its own `env.commit` and `exact_training_ops`. **This is the authoritative decomposition** (`perf/of3t_stepqb2/out/hist_384_base.json`, on `wk/of3t-bwd`) |
| ~~backward, both exact ops, 570.196 s of 705.78 s (80.79 %)~~ **DENOMINATOR WITHDRAWN** | the exact ops read **385.41 s by closure (58.1 %)** on the measured arm: `host_f64_softmax…bw` 259.12 s over 111 fires, `_v_exact_layer_norm…bw` 126.29 s over 658 | **`of3t-xcost` never measured a backward.** `705.78` is a hardcoded literal at `layoutprice.py:143-144` used as a denominator, beside per-call crossing prices its own JSON labels `banked_`, and its `env` block records no `commit`. Its host-price measurements stand; every share DERIVED from that denominator does not. The two disagree by more than a tree: crossings **251.07 s banked against 317.54 s measured (1.265x)** while the backward itself moves the other way, **705.78 against 663.3 (0.94x)**, so the crossing share goes **35.6 % → 47.9 %**. Reconciliation is `of3t-xroof`'s |
...
| **real transfer — ROOF CLAIM WITHDRAWN** | **165.99 s of a 168.51 s residual** | ATTRIBUTED (`of3t-xsplit`, GO): only **2.54 s** is queue drain, `from_torch`'s is **zero by measurement**, and that part stands. **The roof does not (R236).** xsplit measured the board's achievable host-DMA rate in the same process at **1.265 GB/s** device→host and **2.013** host→device and called the transfer irreducible; `of3t-stepqb2` then measured a real exactness-ON backward's crossings at **2.15 and 2.77 GB/s — 1.70x and 1.38x those rates**. A roof you exceed by 70 % is not a roof. The rows may be measuring different work (xsplit's leg includes the host untilize), so neither is impeached — but **this time is back on the board as possibly-engineering**, and it is the sprint's largest remaining upside. Settled by both rates over one shape on one layout path in one process |
...
engineering. **Updated 2026-09-25 19:5xZ: the pc half of this has been fixed and the fix is
measurable in the fleet's own log.**
