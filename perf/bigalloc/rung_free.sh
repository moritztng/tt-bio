#!/bin/bash
# rung_free.sh MODEL RUNG|FIXTURE.yaml [OUT.jsonl [PREDICT ARGS...]]: wait for a whglx chip whose lease a dead holder
...
pick() {
  ...
  for f in *.json; do
    ...
    python3 - "$f" <<'P' && flock -n "$f" true || continue
import json, os, sys
...
P
    # A released lease stays released until the fold writes its own: claim the chip meanwhile, so two
    # launches of this script never pick the same one.
    find "/tmp/mgxb-claim-$n" -maxdepth 0 -mmin +10 -exec rmdir {} + 2>/dev/null
    mkdir "/tmp/mgxb-claim-$n" 2>/dev/null && { echo "$n"; return 0; }
  done
  return 1
}
...
case $rung in
  *.yaml) exec ~/env/bin/python perf/whceil/ladder.py --model "$model" --device "$c" --rungs "$rung" \
            ...
            --env TT_BIO_LEASE_HOLDER=worker:mgx-bigalloc -- --host_threads 2 "${@:4}" ;;
esac
