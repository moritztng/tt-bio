...
floor (6.51 s/fold), and showed ttnn trace closes only 0.15 s of the 2.7 s gap. So the gap is not
op dispatch. This harness brackets the named host regions of one traced sampler loop and prices
each one, then prices the instrument itself with an uninstrumented traced fold in the same
process.
...
def main():
    ...
    res = {"size": a.size, "trace_region": T.trace_region_size(),
           "grid": list(T.COMPUTE_GRID_MAIN), "runs": []}

    def run(label, instrument, traced):
        W.clear()
        ON["v"] = instrument
        sm._diffusion_trace = traced
        t0 = time.perf_counter()
        fold_s, m = one_fold()
        ON["v"] = False
        rec = {"label": label, "instrumented": instrument, "traced": traced,
        ...
        print(label, json.dumps(rec.get("split_ms", {})), "fold %.2fs" % fold_s, flush=True)
        return fold_s

    run("cold", False, True)
    run("traced_bare_1", False, True)
    run("traced_instrumented", True, True)
    run("traced_bare_2", False, True)
    run("eager_instrumented", True, False)
    run("eager_bare", False, False)

    bare = [r["fold_s"] for r in res["runs"] if r["label"].startswith("traced_bare")]
    inst = [r["fold_s"] for r in res["runs"] if r["label"] == "traced_instrumented"]
    if bare and inst:
        res["instrument_cost_s"] = round(inst[0] - sum(bare) / len(bare), 3)
    a.out.write_text(json.dumps(res, indent=1))
    print(json.dumps({k: v for k, v in res.items() if k != "runs"}, indent=1), flush=True)
    T.cleanup()
