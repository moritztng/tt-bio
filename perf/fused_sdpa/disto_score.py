...
ARMS = ["def", "hifi"]          # overridable with --arms; index 0 is the SHIPPED arm
DMIN, DMAX = 4.0, 30.0
LONG_RANGE = 24
CONTACT = 8.0
BLOCK = 10
...
def main():
    ...
    ap.add_argument("--arms", default=None,
                    help="comma pair of arm subdirectory names, SHIPPED first. Defaults to "
                         "def,hifi. The calibration steps below all key off arm 0, so the "
                         "order is load-bearing, not cosmetic.")
    a = ap.parse_args()
    if a.arms:
        global ARMS
        ARMS = [x for x in a.arms.split(",") if x]
        assert len(ARMS) == 2, f"--arms takes exactly two names, got {ARMS}"
    ...
    seeds = [int(p.name.split("seed")[1]) for p in folds[ARMS[0]]]
    assert seeds == [int(p.name.split("seed")[1]) for p in folds[ARMS[1]]], "arm seeds differ"
    ...
    for arm in ARMS:
        for p in folds[arm]:
            ...
            E[arm].append(e)
            P[arm].append(pr)
    n_tok = E[ARMS[0]][0].shape[0]
    cif = next(folds[ARMS[0]][0].glob("*.cif"))
    fold_ca = ca_map(cif)
    ...
    n_bins = P[ARMS[0]][0].shape[-1]
    print(f"tokens {n_tok}  bins {n_bins}  cif {cif.name}")

    report = {"size": a.size, "seeds": seeds, "n_tokens": n_tok, "n_bins": n_bins,
              "sampling_steps": json.loads((d / ARMS[0] / "fold.json").read_text())["sampling_steps"],
              "diffusion_samples": json.loads(
                  (d / ARMS[0] / "fold.json").read_text())["diffusion_samples"],
              "segments": {}}
    ...
    for label, pairs in GT_SEGMENTS[a.size].items():
        ...
        if abs(np.mean(r0)) < 0.8:
            ...
            report["segments"][label] = {
                ...
                "rho_margin_mean_DIRECTION_ONLY": round(
                    float(np.mean(rho[ARMS[1]]) - np.mean(r0)), 6),
                "verdict": "VOID",
        ...
        margin_per_seed = [rho[ARMS[1]][k] - rho[ARMS[0]][k] for k in range(len(seeds))]
        ...
        for t in range(NBOOT):
            ...
            mk = [spearman(Efull[ARMS[1]][k][si, sj], dt)
                  - spearman(Efull[ARMS[0]][k][si, sj], dt) for k in range(len(seeds))]
            boot[t] = np.mean(mk)
