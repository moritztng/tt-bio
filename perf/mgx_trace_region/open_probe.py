"""One pinned open at a given trace region, step by step, so a stall names its step.
    ...
    TT_VISIBLE_DEVICES=<c> TT_BIO_LEASE_CARDS=<c> TT_BIO_LEASE_HOLDER=worker:mgx-trace-region \
        timeout -s INT 60 python perf/mgx_trace_region/open_probe.py <region_bytes> [n_adds]
...
synchronize `_assert_local_dispatch` runs, then one capture + replay of n_adds (default 1) adds.
A region smaller than the capture shows what an overflowing capture does: raise, or hang.
"""
import json
import sys
import time
...
n_adds = int(sys.argv[2]) if len(sys.argv) > 2 else 1
...
if region:
    ...
    ys = [ttnn.add(x, y) for _ in range(n_adds)]
    try:
        ttnn.end_trace_capture(dev, tid, cq_id=0)
    except Exception as e:
        say("capture_raised", error=str(e).splitlines()[0][:300])
        ttnn.synchronize_device(dev)
        z = ttnn.to_torch(ttnn.add(x, x))
        say("eager_after_raise_ok", value=float(z.sum()))
