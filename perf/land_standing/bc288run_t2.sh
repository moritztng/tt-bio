...
OUT=$WT/perf/land_standing/out/bc288b
...
run () {
  ...
  env TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD TT_BIO_LEASE_HOLDER=worker:land-standing \
      ...
      "$PY" -m pytest -v -p no:cacheprovider -p ttstats_plugin -s -k test_the_extra_msa_swap_completes_a_backward_on_card tests/test_bindcraft2_hw.py \
