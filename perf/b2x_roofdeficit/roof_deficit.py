...
def main():
    ...
    rem = dev - inst_s
    opt = dev - floor_s - rem
    mid = dev - floor_s - rem * (floor_s / inst_s)
    print("Ceiling of the byte-deletion axis. It is a range, because the uninstrumented remainder")
    print(f"({rem:.3f} s, {100 * rem / dev:.0f}% of the fold) has never been attributed and could be")
    print("either deletable bytes or more deficit:")
    print(f"  remainder is all deficit          {dev - floor_s:7.3f} s = {dev / (dev - floor_s):.3f}x")
    print(f"  remainder has the same deficit %  {mid:7.3f} s = {dev / mid:.3f}x")
