...
Arms per shape, all on synthetic DRAM-interleaved operands of the exact padded shape:
    ...
    grid        + core_grid=CORE_GRID_MAIN (11x10). ttnn routes this to
...
def run(row, dev, ckc, roofs, cores, l1_budget):
    ...
    pred = naive_config(mt, kt, nt, m_pad, n_pad, 13, 10, ta, tb, tinterm, l1_budget)
    ...
    add("grid" + ("" if grid_fits else "(NO-OP: cbs do not fit, ttnn falls back)"),
        mm(core_grid=CORE_GRID_MAIN), batch if grid_fits else None)
...
def main():
    ...
    cores = CORE_GRID_MAIN.x * CORE_GRID_MAIN.y
    ...
    l1_budget = 1464 * 1024
    print(f"# grid {CORE_GRID_MAIN.x}x{CORE_GRID_MAIN.y} = {cores} cores, "
