...
model was handed rather than a count of `>` records in the input a3m. Neither is inferred: the
OF3 family folds the resolved alignment whole, and `_parse_a3m_to_msa` dedups by sequence, so
the depth in the file and the depth in the tensor are different numbers.
...
still moving at the end, because OpenFold3's L1 refusals inside the diffusion transformer are
caught and retried, so a stall presents as a budget timeout and not as a throw.

Adapted from perf/obcov/collect.py with the rung prefix and model changed.
"""
...
def clock(d: Path, dev: int) -> dict:
    ...
    col = [int(r[UMD_TO_NODE[dev] + 1]) for r in rows
           if len(r) > UMD_TO_NODE[dev] + 1 and r[UMD_TO_NODE[dev] + 1].isdigit()]
...
def progress(d: Path) -> dict:
    ...
    hf = d / "host.log"
    host = [line.split() for line in hf.read_text().splitlines() if line.strip()] if hf.exists() else []
    grp = d / "grouprss.log"
...
def main() -> int:
    rows = []
    for d in sorted((HERE / "out").glob("of3_*_dev*")):
        ...
        if res:
            ...
            if cifs:
                ...
                row |= structure(d, cifs[0], HERE / "inputs" / f"of3_{rung}.yaml")
        else:
    ...
    for r in rows:
        print(r["rung"], "dev", r["umd_device"], r.get("aiclk_median_mhz"), "MHz",
