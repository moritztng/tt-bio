...
def write_pxdesign(d, gpu, data):
    ...
    ref_txt = (
        ...
        f"H200, and that wall is not this cell: the rest of it is the ProteinMPNN and AF2-IG "
        f"filtering subprocesses ({mpnn:.1f} s of ProteinMPNN against "
        f"{h200['stages_s']['mpnn_s']:.1f} s, {statistics.median(j['split']['af2ig_s'] for j in reps):.1f} s "
        f"of AF2-IG against {h200['split_s']['af2ig_s']:.1f} s), which the CLI does not run at all. "
        ...
        f"{e['gpu']}, driver {e['nvidia_smi'].split(',')[1].strip()}, compute capability "
        f"{'.'.join(str(x) for x in e['gpu_capability'])} against the H200's "
        f"{'.'.join(str(x) for x in ref['stack']['gpu_capability'])}. Peak allocated VRAM "
        f"{r0['peak_vram_alloc_GiB']:.3f} GiB against the h200 cell's "
        f"{h200['peak_vram_alloc_GiB']:.3f} GiB. Stack byte-matched to the h200 cell's: torch "
