#!/usr/bin/env python3
"""of3t-apbleaf: `perf/of3t_trunkopclass/classarm.py` with the LayerNorm capture NARROWED to
the one leaf this row owns, so it can be taken at every block instead of at three.

One change, marked APBLEAF below. classarm.py captures every LayerNorm in a captured block, and
the pair track's norms carry an x of [1, 384, 384, 128] -- 75 MB per site in fp32, 3.6 GB over
48 blocks, on a host with 24 GB free. The capture here fires only for `pre_norm_s`, whose x is
[1, 384, 384], so all 48 blocks cost about 85 MB and the counterfactual can be built over the
whole 96-tensor leaf set rather than over a sample of it.

Nothing else is touched: the arm, the levers, the fused-five placement, the config spy and the
scoring path are classarm.py's, which are of3t-trunkg043's, so `--lever none` must still come
back bit-identical to the banked trunk gradient.

Original docstring follows.

of3t-trunkopclass: `perf/of3t_bwdaccum/dev_cot.py` with the levers CLASS-SCOPED, plus a
runtime call census per op class.

Unchanged from that file except where marked: the arm, the fused-five placement, the config spy
and the whole scoring path stay the ones that produced the banked trunk gradient, so
`--lever none --class-scope ALL` must come back BIT-IDENTICAL to it. That equality is this row's
instrument control and it is checked before any arm is read.
...
CAP_LEAF = "pre_norm_s"   # APBLEAF: upstream attn_pair_bias.layer_norm_a

LEVERS = ("none", "prod_fp32", "sum_fp32", "xhat_fp32", "dxcfg", "dx_fp32", "all", "lofi",
          "softmax_fp32")

# of3t-trunkopclass. The op-class map is `perf/of3t_trunkopclass/PREDICTION.md`'s.
#
# TWO namespaces, and conflating them is the trap this row nearly walked into. The CENSUS is
# grouped by UPSTREAM's parameter names, because that is the space the comparison is made in
# (PROTOCOL 3a). A lever on the device sees OUR names. They are not the same strings, and the
# one that matters most does not even live in the same module: our `pre_norm_s.weight` is
# upstream's `attn_pair_bias.layer_norm_a.weight` (`tt_bio/protenix_weights.py:87-88`), the leaf
# `of3t-trunkact` measured as carrying 56.1174 % of the trunk's gradient error mass -- and it is
# constructed in `PairformerLayer`, OUTSIDE the `AttentionPairBias` module. A selector keyed on
# the module on the call stack reaches every other APB site and misses that one. Measured, not
# assumed: the first census run resolved it to OUTSIDE, 48 of 48 blocks.
CLASS_OF_OUR_LEAF = (
    ("triangle_multiplication_start.", "TRI_MUL"), ("triangle_multiplication_end.", "TRI_MUL"),
    ("triangle_attention_start.", "TRI_ATT"), ("triangle_attention_end.", "TRI_ATT"),
    ("transition_z.", "TRANS"), ("transition_s.", "TRANS"),
    ("attention_pair_bias.", "APB"),
    ("pre_norm_s_", "APB"), ("pre_norm_s.", "APB"),
)
CLASS_OF_THEIR_LEAF = (
    ("pair_stack.tri_mul_in.", "TRI_MUL"), ("pair_stack.tri_mul_out.", "TRI_MUL"),
    ("pair_stack.tri_att_start.", "TRI_ATT"), ("pair_stack.tri_att_end.", "TRI_ATT"),
    ("pair_stack.pair_transition.", "TRANS"), ("single_transition.", "TRANS"),
    ("attn_pair_bias.", "APB"),
)
CLASSES = ("TRI_MUL", "TRI_ATT", "TRANS", "APB")


def class_of_path(path):
    """A device or upstream parameter path -> op class, or None if it is outside the trunk."""
    if path is None:
        return None
    p = path.split(".")
    for i, tok in enumerate(p):
        if tok == "blocks" and i + 1 < len(p) and p[i + 1].isdigit():
            leaf = ".".join(p[i + 2:])
            break
    else:
        return None
    for pfx, cls in CLASS_OF_OUR_LEAF + CLASS_OF_THEIR_LEAF:
...
def main() -> int:
    ...
    ap.add_argument("--class-scope", default="ALL", choices=("ALL",) + CLASSES,
                    help="apply --lever ONLY to sites in this op class; every other class runs "
                         "exactly the shipped path. ALL is the unscoped lever, and with "
                         "--lever none it is the shipped arm either way.")
    ap.add_argument("--census-out", default="", help="where the runtime call census lands")
    ...
    # ---- 2a. the RUNTIME call census, and the class context a parameterless site needs ----
    # Counted by wrapping the module objects the stack actually calls, not read off the source:
    # a percentage attached to a code-read route has already survived to a brief in this
    # campaign with zero calls behind it.
    CTX = []
    CENSUS = {c: {"forward_calls": 0, "ln_backward": 0, "softmax_backward": 0,
                  "lever_applied_ln": 0, "lever_applied_softmax": 0} for c in CLASSES}
    CENSUS["OUTSIDE"] = {"forward_calls": 0, "ln_backward": 0, "softmax_backward": 0,
                         "lever_applied_ln": 0, "lever_applied_softmax": 0}
    # Coverage is MEASURED off a census, never asserted from a selector's name. The per-class
    # totals alone cannot tell a correct assignment from one the CTX fallback rescued, so every
    # LayerNorm backward records the parameter path it resolved -- or that it resolved none.
    from collections import Counter as _Counter
    LNSITES = _Counter()
    SMSITES = _Counter()

    def _instrument(mod_cls, cls_name):
        real = mod_cls.__call__

        def wrapped(self, *ar, **kw):
            CENSUS[cls_name]["forward_calls"] += 1
            CTX.append(cls_name)
            try:
                return real(self, *ar, **kw)
            finally:
                CTX.pop()
        mod_cls.__call__ = wrapped

    for _mc, _cn in ((T.TriangleMultiplication, "TRI_MUL"), (T.TriangleAttention, "TRI_ATT"),
                     (T.Transition, "TRANS"), (T.AttentionPairBias, "APB")):
        _instrument(_mc, _cn)

    SCOPE = a.class_scope

    def in_scope(cls):
        return SCOPE == "ALL" or cls == SCOPE
    ...
    def _taped_layer_norm(shipped, args, kwargs):
        ...
        # The class is decided at FORWARD time and closed over, because the backward runs long
        # after the module call that owns this site has returned. Parameter path first, module
        # context only as a fallback -- the path is what the census groups by, so keying the
        # lever off anything else would let the arm and the census disagree.
        _fwd_path = _name_of(gamma.value) if gamma is not None else None
        _fwd_ctx = CTX[-1] if CTX else None
        site_cls = class_of_path(_fwd_path)
        _by_path = site_cls is not None
        if site_cls is None:
            site_cls = _fwd_ctx
        site_key = site_cls if site_cls in CENSUS else "OUTSIDE"
        site_on = in_scope(site_cls)

        def make():
            def bw(g):
                LN["bw"] += 1
                CENSUS[site_key]["ln_backward"] += 1
                LNSITES["%s|%s|%s" % (site_key, "path" if _by_path else "ctx",
                                      _fwd_path or ("UNRESOLVED:" + str(_fwd_ctx)))] += 1
                if site_on and lev != "none":
                    CENSUS[site_key]["lever_applied_ln"] += 1
                xv = x.value
                if site_on and on("xhat_fp32"):
                ...
                # APBLEAF: the block filter AND the leaf filter. `CAP_LEAF` is our device name
                # for upstream's `attn_pair_bias.layer_norm_a`; see the namespace note above.
                want = (path is not None
                        and CAP_LEAF in path
                        and int(path.split(".")[1]) in cap_blocks) if path else False
                dw = db = None
                if gamma is not None and gamma.requires_grad:
                    LN["dw"] += 1
                    if site_on and on("prod_fp32"):
                    ...
                    dw = _sum_leading(prod, gamma.value.shape,
                                      site_on and on("sum_fp32") or site_on and on("prod_fp32"))
                    gamma.add_grad(dw)
                if beta is not None and beta.requires_grad:
                    db = _sum_leading(g, beta.value.shape, site_on and on("sum_fp32"))
                    beta.add_grad(db)
                if want:
                    CAPTURED.append({
                        "gamma_path": path,
                ...
                if x.requires_grad:
                    ...
                    if site_on and on("dx_fp32"):
                        ...
                        kcfg = {"compute_kernel_config": bwcfg} if site_on and on("dxcfg") else {}
                        if kcfg:
                            LN["dxcfg_applied"] += 2
    ...
    def _v_softmax_counted(shipped, args, kwargs):
        """Shipped softmax, counted. Without this the census would only see the levered arm and
        a class's softmax count would depend on which arm was running."""
        ctx = CTX[-1] if CTX else None
        key = ctx if ctx in CENSUS else "OUTSIDE"
        out = _real_sm(shipped, args, kwargs)
        node = getattr(out, "node", None)
        if node is not None and getattr(node, "fn", None) is not None:
            orig_fn = node.fn

            def counted(g, orig_fn=orig_fn, key=key, ctx=ctx):
                CENSUS[key]["softmax_backward"] += 1
                SMSITES["%s|%s" % (key, ctx)] += 1
                return orig_fn(g)
            node.fn = counted
        return out
    ...
    def _v_softmax_fp32(shipped, args, kwargs):
        ...
        key = ctx if ctx in CENSUS else "OUTSIDE"
        if not in_scope(ctx):
            return _v_softmax_counted(shipped, args, kwargs)   # other classes run shipped
        x = ag._wrap(args[0])
        ...
        def make():
            def bw(g):
                SM["fired"] += 1
                CENSUS[key]["softmax_backward"] += 1
                CENSUS[key]["lever_applied_softmax"] += 1
                y = box[0]
                g32 = ttnn.typecast(g, ttnn.float32)
    ...
    for v in ("softmax", "softmax_in_place"):
        if v in tt._VERBS:
            # `all` does NOT include softmax_fp32, and that is deliberate rather than an
            # oversight inherited from dev_cot.py. `_v_softmax_fp32` is a hand-written softmax
            # backward that never calls `ag.softmax_bw_inner`, so it silently drops
            # TT_BIO_SOFTMAX_BW_RENORM -- D116's fix and the SHIPPED default
            # (`tt_bio/autograd.py:87`). Folding it into `all` does not raise the arm's accuracy,
            # it removes a correction the shipped arm has: measured at APB, 0.5362249041 ->
            # 11.2182337263 against upstream's own bf16. A lever that makes a reading 21x worse
            # is a defect in the lever.
            tt._VERBS[v] = (_v_softmax_fp32 if lev == "softmax_fp32"
                            else _v_softmax_counted)
    ...
    census = {"lever": lev, "class_scope": SCOPE,
              "softmax_backward_firings": SM["fired"],
              "layer_norm_backward_reach": LN,
              "per_class": CENSUS,
              "ln_backward_by_resolved_path": dict(sorted(LNSITES.items())),
              "ln_sites_resolved_by_path": sum(v for k, v in LNSITES.items()
                                               if k.split("|")[1] == "path"),
              "ln_sites_resolved_by_context_only": sum(v for k, v in LNSITES.items()
                                                       if k.split("|")[1] == "ctx"),
              "softmax_backward_by_context": dict(sorted(SMSITES.items())),
              "seconds": round(time.perf_counter() - t0, 1)}
    if a.census_out:
        json.dump(census, open(a.census_out, "w"), indent=2)
    print("CLASS_CENSUS " + json.dumps(census))
    return rc
