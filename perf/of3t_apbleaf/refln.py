#!/usr/bin/env python3
"""of3t-apbleaf: `perf/of3t_trunkg043/ref_grad.py` with the reference's own LayerNorm operands
captured at `attn_pair_bias.layer_norm_a` in every block.

Two additive changes, marked APBLEAF. `--ln-out` registers a forward hook on each block's
`attn_pair_bias.layer_norm_a` which snapshots that module's INPUT and attaches a tensor hook to
its OUTPUT, so the backward hands back the cotangent that arrived there. Nothing on the
arithmetic path moves, and the control for that is not an argument: the 96 parameter gradients
this run produces must be BIT-IDENTICAL to the banked `ref_f64_n384.pt`.

`--checkpoint` recomputes each block in the backward, so the forward hook fires twice per site
and the recompute's copy is the one whose output sits in the executed graph. Both writes carry
the same value because an eval()-mode block with no dropout recomputes deterministically, and
the run reports how many times each site fired so "twice" can be told from "once".
...
def main() -> int:
    ...
    ap.add_argument("--ln-out", default="",
                    help="capture attn_pair_bias.layer_norm_a's input and output cotangent in "
                         "every block, in float64, and write them here")
    ...
    # APBLEAF: the capture. A forward hook per block, and a tensor hook on the output so the
    # cotangent that ARRIVED at this site is the one recorded rather than one recomputed from
    # a neighbour. `fires` distinguishes the checkpoint recompute's second call from a miss.
    LNCAP, LNFIRE = {}, {}
    if a.ln_out:
        def _mk(i):
            def _fwd(mod, inp, out):
                LNFIRE[i] = LNFIRE.get(i, 0) + 1
                if not (torch.is_grad_enabled() and out.requires_grad):
                    return
                xs = inp[0].detach().to(torch.float64).clone()

                def _gh(gr, i=i, xs=xs):
                    LNCAP[i] = {"x": xs, "g": gr.detach().to(torch.float64).clone()}
                out.register_hook(_gh)
            return _fwd
        for i, m in enumerate(mods):
            m.get_submodule("attn_pair_bias.layer_norm_a").register_forward_hook(_mk(i))
    ...
    # APBLEAF: write the capture, with the two parameter gradients of the same site beside it
    # so a consumer never has to pair two files by index.
    if a.ln_out:
        sites = {}
        for i in range(a.blocks):
            ...
            if e is None:
                continue
            base = f"{PRE}{i}.attn_pair_bias.layer_norm_a."
            sites[i] = {"block": i, "param_prefix": base,
                        "x": e["x"], "g": e["g"],
                        "gamma": sd[base + "weight"].to(torch.float64).clone(),
                        "beta": sd[base + "bias"].to(torch.float64).clone(),
                        "dW_ref": grads.get(base + "weight"),
                        "db_ref": grads.get(base + "bias"),
                        "forward_hook_fires": LNFIRE.get(i, 0)}
        torch.save({"sites": sites, "policy": a.policy, "crop": a.crop,
                    "blocks": a.blocks, "tree": a.tree}, a.ln_out)
        print(json.dumps({"ln_out": a.ln_out, "sites": len(sites),
                          "fires": sorted(set(LNFIRE.values())),
                          "missing": [i for i in range(a.blocks) if i not in sites]}),
              flush=True)
