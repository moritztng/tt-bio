...
#
# JOBS lines: "<tag> <tree> <model> <fixture> [predict args...]"; a job is claimed by appending its tag to
...
# Cards 1 and 24-27 carry the live app and a co-tenant and are never taken; LANE_SKIP adds more.
# LANE_HOLDER names the lease holder (default worker:mgx-accuracy).
set -u
JOBS=$(realpath "$1"); OUT=$(realpath -m "$2")
LEASES=/home/agent/leases; HOST=j10glx02; HOLDER=${LANE_HOLDER:-worker:mgx-accuracy}
SKIP=" 1 24 25 26 27 ${LANE_SKIP:-} "
...
lease() {  # card released(null|epoch); writes only under the card's free flock, releases only our own record
  flock -n "$LEASES/$HOST-card$1.json" python3 - "$LEASES/$HOST-card$1.json" "$1" "$2" $$ "$HOLDER" <<'EOF'
import json, sys, time
p, card, rel, pid, holder = sys.argv[1:]
try:
    cur = json.load(open(p))
except Exception:
    cur = {}
if rel != "null" and cur.get("holder") not in (None, holder):
    sys.exit(0)  # another worker took the card after our fold ended
json.dump({"host": "j10glx02", "card": card, "holder": holder, "pid": int(pid),
...
while job=$(claim); do
  read -r tag tree model fx args <<<"$job"
  exec 8>>"$OUT/.cardlock"
  flock 8
  ...
  (cd "$tree" && PYTHONPATH="$tree" $PY perf/whceil/ladder.py --model "$model" --device "$card" \
     ...
     --out "$OUT/$tag.jsonl" --out-root "$OUT/runs/$tag" --timeout "${LANE_TIMEOUT:-5400}" -- --host_threads 2 $args) \
...
EOF
  ...
  [ $? = 3 ] && [ "${tag%+++}" = "$tag" ] && echo "$tag+ $tree $model $fx $args" >> "$JOBS"
done
