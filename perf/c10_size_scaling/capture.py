"""One warmed device context per ladder size, two clock arms INTERLEAVED inside it.

c10-fixed-cost's capture, unchanged except for the ladder it walks: the timer boundary is still
c10-bare-baseline's (synchronize, monotonic, the complete `predict_one` including featurization and
CIF writing, synchronize, monotonic), FORCE_AICLK is still re-sent and re-verified before every
label, and a 1350 and an 800 MHz fold still sit next to each other in one process, one device
context and one warm cache. Changing the boundary would silently break comparability with every
number this campaign is priced against, which is why it is copied rather than rewritten.

Two clocks per size is not a luxury here. A ladder measured at ONE clock cannot separate the fold
into a term that scales with AICLK and a term that does not, so it cannot say anything about work
at all -- and the whole question is how the WORK term grows with the target.
"""
...
def main():
    ...
    try:
        ...
        for key,expected in [('TT_VISIBLE_DEVICES','0'),('TT_BIO_LEASE_CARDS','0'),('TT_BIO_LEASE_HOLDER','worker:c10-size-scaling')]:
