#!/usr/bin/env bash
# of3t-inproj (from of3t-fullstep64): one arm of our full step on qb2 card 1, replaying the float64 run's draws.
#
...
OUT=$W/perf/of3t_inproj/DEV_${TAG}.json
WARG=(); [ "${3:-}" = weights ] && WARG=(--weights-out "$S/weights_walked.pt")
DRAWS=${5:-/home/ttuser/of3t_fullstep64/draws.pt}; [ -n "${4:-}" ] && WARG+=(--batch "$4")
...
ENV=(TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD
     TT_BIO_LEASE_HOLDER=worker:of3t-inproj OMP_NUM_THREADS=8 PYTHONPATH="$W")
...
with open(g, "rb") as f:
    for b in iter(lambda: f.read(1 << 22), b""):
        h.update(b)
d["provenance"] = {"host": socket.gethostname(), "row": "of3t-inproj", "card": int(card),
