...
# The card grant of this row is node 1, not node 0. Everything that names a node reads it from
# one place so a mis-set grant fails loudly instead of validating the wrong chip.
NODE = os.environ.get("C10_NODE", "1")
DEV = f"/dev/tenstorrent/{NODE}"
AICLK = f"/sys/class/tenstorrent/tenstorrent!{NODE}/tt_aiclk"
