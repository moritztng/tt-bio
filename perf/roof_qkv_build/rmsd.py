...
Copied from `perf/roof_msa_ladder/rmsd.py` and repointed at this row's CIF directories; the
readings and the Kabsch are unchanged so the two rows' Angstrom figures are comparable.
...
def main() -> int:
    ...
    for size in ("512", "298"):
        files = {}
        for d in (Path("perf/roof_qkv_build/cif298"), Path("perf/roof_qkv_build/cif512")):
            s = "s1"
    ...
    Path("perf/roof_qkv_build/rmsd.json").write_text(json.dumps(out, indent=2) + "\n")
    return 0
