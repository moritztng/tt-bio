...
def _plan_main(args):
    ...
    total = sum(r["in_situ_s"] for r in rows)
    print(f"\nin situ {total:.5f} s over {sum(r['calls_per_fold'] for r in rows)} programs, "
          f"1350 MHz -> {total * 1350e6 / 1e6:.1f} Mcycles")
    (OUT / "plan.json").write_text(json.dumps({"all_guards_pass": bool(ok), "grid": [11, 10],
                                               "signatures": rows}, indent=1) + "\n")
...
def _device_main(args):
    ...
    try:
        ...
        rows = [_arms(dev, s, args.reps) for s in SIGS]
