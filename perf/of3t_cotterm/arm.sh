#!/usr/bin/env bash
# of3t-cotterm: one trunk backward with the APB boundary captured at every block.
# qb1 (tt-quietbox), card 1 -- this row's grant. AICLK sampled every 4 s DURING the arm.
#   arm.sh <c64|n384> <TAG>
set -uo pipefail
...
APB=$O/apb_${TAG}_${SCOPEARG}.pt
REP=$W/perf/of3t_cotterm/DEV_${TAG}_${SCOPEARG}.json
CEN=$O/callcensus_${TAG}_${SCOPEARG}.json
...
S=$(date +%s)
echo "=== $TAG $SCOPEARG start $(date -u +%FT%TZ) host=$(hostname) card=$CARD ==="
