#!/usr/bin/env bash
# of3t-frameself step 1: differentiate the REAL model.pairformer_stack on the captured pair, in
# the same process as the capture that produced it.
#
...
echo "=== selftest start $(date -u +%FT%TZ) host $(hostname) threads $THREADS ==="
nice -n 5 "$PY" perf/of3t_modelframe/capture_model_frame.py \
  ...
  --selftest --selftest-compare /home/ttuser/of3t_twoside/ctrl_f64.pt \
  --out-dir "$O" --threads "$THREADS" --tag selftest_n384 2>&1 \
  | grep -vE "UserWarning|warnings.warn|^  from openfold3|Consider using tensor.detach"
echo "=== selftest exit ${PIPESTATUS[0]} $(date -u +%FT%TZ) ==="
ls -la "$O"
