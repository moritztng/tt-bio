...
def leg(tag, root):
    ...
    # The engine stamps time of day with no date, so a leg that crosses midnight hands this
    # loop a step backwards. Walk the stamps onto a monotonic timeline first (the same trap
    # af2ig's collector hit at c5aea1343) instead of recording a negative quiet window.
    stamps = [datetime.strptime(m, "%H:%M:%S") for m in re.findall(r"^(\d\d:\d\d:\d\d)  \[", log, re.M)]
    mono, day = [], 0
    for i, t in enumerate(stamps):
        if i and t < stamps[i - 1]:
            day += 1
        mono.append(t + timedelta(days=day))
    row["max_quiet_s"] = max((int((b - a_).total_seconds()) for a_, b in zip(mono, mono[1:])), default=0)
    return row
