#!/usr/bin/env bash
# of3t-cotcoh pass 2 arm.  arm2.sh <card> <lever>
# Card fanout: the grant is widened on THIS command only, to the card named, and the open stays
# pinned. The sibling cards were read idle at 800 MHz and 29-31 W before launch.
set -uo pipefail
...
mkdir -p "$O"
CARD=${1:?usage: arm2.sh <card> <lever>}
LEV=${2:?usage: arm2.sh <card> <lever>}
...
echo "=== $LEV start $(date -u +%FT%TZ) host $(hostname) card $CARD board $BOARD ==="
CLK=$O/aiclk_${LEV}.txt
: > "$CLK"
( while true; do
    "$SMI" -s 2>/dev/null \
...
timeout 5400 python3 perf/of3t_cotcoh/arm2.py --lever "$LEV" \
  ...
  --out "$O/sqnorms_${LEV}.pt" \
  --report perf/of3t_cotcoh/DEVARM2_${LEV}.json --arm flipped --crop 0 2>&1 \
  | grep -E '^\{|SYS_PATH|TRUNK|SCORE ERROR|OUR GRADIENT|discovery|Traceback|rror|FAILED|placed' | tail -30
rc=${PIPESTATUS[0]}
E=$(date +%s)
kill "$SAMPLER" 2>/dev/null
echo "=== $LEV exit $rc elapsed $((E-S))s $(date -u +%FT%TZ) ==="
...
python3 - perf/of3t_cotcoh/ARM2_${LEV}.json "$CLK" "$CARD" "$BOARD" "$rc" <<'PY'
...
d["provenance"] = {"host": socket.gethostname(), "card": int(card), "board_class": board,
                   "aiclk_mhz_during_the_run": ({"n": len(s), "min": s[0],
                                                ...
                                                if s else "NO SAMPLES"), "exit": int(rc)}
