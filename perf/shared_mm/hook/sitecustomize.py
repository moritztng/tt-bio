...
Enable by putting this directory first on PYTHONPATH and setting SHARED_MM_CENSUS to an output
directory. Each process writes census_<pid>.json at exit; the fold child is the big one.
"""
import os, sys

_OUT = os.environ.get("SHARED_MM_CENSUS")
...
if _OUT:
    ...
    # Driver hooks: the census must cover exactly one warm fold, not the cold compile fold.
    import builtins as _b
    def _census_reset():
        _counter.clear()
    _b._census_reset = _census_reset
    _b._census_counter = _counter
