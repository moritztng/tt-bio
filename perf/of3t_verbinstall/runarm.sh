...
if ! env TT_VISIBLE_DEVICES=1 TT_BIO_LEASE_CARDS=1 TT_BIO_LEASE_HOLDER=worker:of3t-verbinstall OMP_NUM_THREADS=8 PYTHONPATH="$W" \
     timeout 120 python3 perf/of3t_verbinstall/cardcheck.py 1; then
  echo "=== $TAG ABORTED: card 1 failed the bounded dispatch check. Reset it (tt-smi -r 1) ==="
...
env "${ENVS[@]}" \
  ...
  | tee "$O/raw_${TAG}.log" | grep --line-buffered -E '^\{|^CENSUS|^EXACT_SOFTMAX|OUR GRADIENT|discovery|Traceback|rror|FAILED|placed|config ' | tail -30
rc=${PIPESTATUS[0]}
E=$(date +%s)
kill "$SAMPLER" 2>/dev/null
