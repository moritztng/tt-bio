#!/bin/bash
# The qb2 leg of the triatt q-split (PM._Q_SPLIT) A/B at 768 and 1024 aa.
#
...
#
# Per-leg `timeout`: attempt 1 (13:31Z) hung the device for 33 min inside the cold `on` fold at
# 1024 and had to be SIGKILLed + `tt-smi -r 3`. An unbounded leg cannot be allowed to sit on
# benchlock like that again, and a 1024 leg of 6 folds needs ~1100 s of real work.
...
timeout 2400 $E perf/other512/fold_ab_multi.py --model boltz2 --sizes 1024 \
   --arms on,on,on,qsplit,qsplit --out perf/b2sizes/qsplit_ab_1024_qb2.json
echo "1024 RC=$? at $(date -Is)"

echo "=== 768 aa $(date -Is) ==="
timeout 1800 $E perf/other512/fold_ab_multi.py --model boltz2 --sizes 768 \
   --arms on,on,on,qsplit --out perf/b2sizes/qsplit_ab_768_qb2.json
echo "768 RC=$? at $(date -Is)"
echo "=== done $(date -Is) ==="
