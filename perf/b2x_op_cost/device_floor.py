    ...
    if "grab" in phases:
        for cname in targets:
            if cname == "PairformerLayer":
                ...
                # Same hazard as Transition, but NOT the same filter. AttentionPairBias never
                # takes the pair tensor as its first argument -- z enters as a bias. At 512 aa its
                # first arg is 1x512x768 (4800 calls, the diffusion transformer), 1x140x32x128
                # (1200, the atom transformer) or 1x512x384 (264), all far under 4M elements, so a
                # volume filter copied from Transition matches nothing and the class is dropped
                # from the table instead of corrected. Match the token track by rank and channel
                # width: 3-D first arg with the wide channel count.
                arm(cname, lambda o, ar: bool(ar) and hasattr(ar[0], "shape")
                    and len(ar[0].shape) == 3 and int(ar[0].shape[-1]) > 512)
            else:
