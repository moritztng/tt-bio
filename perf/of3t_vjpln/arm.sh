#!/usr/bin/env bash
# of3t-vjpln device arms. qb1 (tt-quietbox), p150a Blackhole.
...
set -uo pipefail
W=/home/ttuser/.coworker/wt/of3t-vjpln
cd "$W"
O=/home/ttuser/of3t_vjpln
mkdir -p "$O"
...
OUT=$O/grads_${TAG}.pt
COT=$O/cot_${TAG}.pt
SIDE=$W/perf/of3t_vjpln/SIDE_${TAG}.json
...
VJP_PIN="$PIN" VJP_BLOCKS="$BLOCKS" VJP_SIDECAR="$SIDE" \
TT_BIO_SOFTMAX_BW_RENORM=1 \
TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=2,$CARD TT_BIO_LEASE_HOLDER=worker:of3t-vjpln \
OMP_NUM_THREADS=8 \
timeout 5400 python3 perf/of3t_vjpln/arm.py --lever none --cot-out "$COT" \
  --boundary "$BND" --cap-last "$R/block47_boundary.pt" --out "$OUT" \
  --report "$W/perf/of3t_vjpln/DEV_${TAG}.json" --arm flipped --crop "$CROP" 2>&1 \
