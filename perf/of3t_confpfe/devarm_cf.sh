#!/usr/bin/env bash
# of3t-confpfe (fixed tree, outputs under cf/): go384's 384-wide denoise arm of the OF3 training step on qb2 card 0, from a given tree.
#
#   devarm.sh <TAG> <TREE> [--weights-out W.pt]
#     -> perf/of3t_confpfe/DEV_<TAG>.json, /home/ttuser/of3t_confpfe/grad_<TAG>.pt
#
...
set -uo pipefail
W=/home/ttuser/.coworker/wt/of3t-confpfe
S=/home/ttuser/of3t_confpfe/cf
CARD=${CARD:-0}
SMI=/home/ttuser/.local/bin/tt-smi
TAG=${1:?usage: devarm.sh TAG TREE}; TREE=${2:?usage: devarm.sh TAG TREE}; shift 2
OUT=$W/perf/of3t_confpfe/cf/DEV_${TAG}.json
...
ENV=(TT_VISIBLE_DEVICES=$CARD TT_BIO_LEASE_CARDS=$CARD
     TT_BIO_LEASE_HOLDER=worker:of3t-confpfe OMP_NUM_THREADS=8 PYTHONPATH="$TREE")
echo "=== $TAG start $(date -u +%FT%TZ) $(hostname) card $CARD $BOARD tree $TREE $(git rev-parse --short HEAD) ==="
env "${ENV[@]}" timeout 7200 python3 "$W/perf/of3t_confpfe/devstep.py" --repr-out "$S/repr_${TAG}.pt" --denoise --exact on \
...
d["provenance"] = {"host": socket.gethostname(), "row": "of3t-confpfe", "card": int(card),
