...
W=/home/ttuser/.coworker/wt/of3t-confpfe; S=/home/ttuser/of3t_confpfe; P=$W/perf/of3t_confpfe; L=$S/chain.log
R=$S/ref384c; CK=/home/ttuser/of3-weights/of3-p2-155k.pt
cd $W
(while true; do echo "$(date -u +%T) $(/home/ttuser/.local/bin/tt-smi -s 2>/dev/null | python3 -c "import sys,json;d=json.load(sys.stdin);print(d['device_info'][0]['telemetry']['aiclk'].strip())")"; sleep 5; done) > $S/aiclk_chain.txt 2>&1 &
M=$!
bash $P/devarm.sh CF384 $W --weights-out $S/weights_walked_CF384.pt >> $L 2>&1
rc=$?
kill $M
