...
def eligible(xa, xb, wa, wb):
    ...
    if tuple(xa.shape) != tuple(xa.padded_shape):
        # The output is allocated at `xa.padded_shape`, so a target whose token count is not a
        # multiple of the 32-row tile gets a `z_update` one padded row-block too wide and the
        # residual `add_` throws. protenix-v2 at 298 aa died here on main; 320 and 512 pass
        # because they are 10 and 16 whole tiles. A parity sweep whose shapes are generated by
        # stepping tiles cannot see this, which is why it shipped.
        return "pad_shape"
    return None
