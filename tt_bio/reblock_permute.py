...
def _cache_key_gated(x, out, device, reader_ct, writer_ct):
    ...
    return (
        device.id(),
        bool(GATE_DST_RESIDENT), int(WORK_CB_DEPTH),
...
def _build_gated(x, out, device, reader_ct, writer_ct, fidelity, fp32_acc):
    ...
    cbs = [cb(P_CB, WORK_CB_DEPTH), cb(G_CB, WORK_CB_DEPTH), cb(SIG_CB, WORK_CB_DEPTH),
           cb(OUT_CB, GROUP_TILES * 2), cb(STAGE_CB, WORK_CB_DEPTH)]
    if not GATE_DST_RESIDENT:
        ...
        # Depth comes from WORK_CB_DEPTH like the rest. Taking either side of this merge whole
        # deletes the other row's lever: cb-depth's hunk reintroduces MUL_CB unconditionally and
        # the DST fusion then reports enabled while doing nothing.
        cbs.insert(3, cb(MUL_CB, WORK_CB_DEPTH))
