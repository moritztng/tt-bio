...
from tt_bio._vendor.openfold3.core.config.config_utils import (
    _convert_molecule_type,
    _ensure_list,
)
from tt_bio._vendor.openfold3.core.data.io.dataset_cache import read_datacache, write_datacache_to_json
from tt_bio._vendor.openfold3.core.data.io.s3 import open_local_or_s3
from tt_bio._vendor.openfold3.core.data.io.sequence.template import (
    A3mParser,
    CifDirectParser,
    TemplateData,
...
from tt_bio._vendor.openfold3.core.data.io.structure.atom_array import (
    read_atomarray_from_npz,
    write_atomarray_to_npz,
)
from tt_bio._vendor.openfold3.core.data.io.structure.cif import _load_ciffile, parse_mmcif
from tt_bio._vendor.openfold3.core.data.primitives.caches.format import DatasetCache
...
from tt_bio._vendor.openfold3.core.data.primitives.sequence.hash import get_sequence_hash
from tt_bio._vendor.openfold3.core.data.primitives.sequence.template import (
...
from tt_bio._vendor.openfold3.core.data.primitives.structure.component import BiotiteCCDWrapper
from tt_bio._vendor.openfold3.core.data.primitives.structure.metadata import (
    get_asym_id_to_canonical_seq_dict,
    get_cif_block,
    get_release_date,
)
from tt_bio._vendor.openfold3.core.data.primitives.structure.template import clean_template_atom_array
    ...
    def validate_inputs(self) -> "TemplatePreprocessorInputInference":
        ...
        if self.template_cif_chain_ids is not None:
            if self.template_cif_paths is None:
                raise ValueError(
                    "'template_cif_chain_ids' requires 'template_cif_paths'"
                )
            if len(self.template_cif_chain_ids) != len(self.template_cif_paths):
                raise ValueError(
                    f"Length mismatch: {len(self.template_cif_paths)} CIF files "
                    f"but {len(self.template_cif_chain_ids)} chain IDs"
                )
    ...
    def _parse_templates_from_cif_files(
        ...
        for idx, cif_path in enumerate(input_data.template_cif_paths):
            try:
                ...
                specified_chain_id = None
                if input_data.template_cif_chain_ids is not None:
                    specified_chain_id = input_data.template_cif_chain_ids[idx]
                ...
                if cif_templates:
                    ...
                    template = template._replace(index=idx, cif_path=cif_path)
                    templates[idx] = template

                    if self.create_logs:
                        worker_logger.info(
                            f"Selected chain {template.chain_id} from {entry_id} "
                            f"(seq_id={template.seq_id:.3f}, "
                            f"q_cov={template.q_cov:.3f})"
                        )

            except Exception as e:
                if self.create_logs:
                    worker_logger.warning(f"Failed to process CIF file {cif_path}: {e}")
                continue
        ...
        if templates:
            ...
            for t in templates_list[:5]:
                if self.create_logs:
                    worker_logger.info(
                        f"  - {t.entry_id}_{t.chain_id}: "
                        f"seq_id={t.seq_id:.3f}, q_cov={t.q_cov:.3f}"
                    )
    ...
    def _preprocess_templates_for_query(
        ...
        if isinstance(input_data, TemplatePreprocessorInputInference):
            if input_data.aln_path is not None:
                if self.create_logs:
                    worker_logger.info(
                        f"Parsing template alignment {input_data.aln_path}..."
                    )
            elif input_data.template_cif_paths is not None:  # noqa: SIM102
                if self.create_logs:
                    worker_logger.info(
                        f"Processing {len(input_data.template_cif_paths)} CIF files "
                        "in CIF-direct mode..."
