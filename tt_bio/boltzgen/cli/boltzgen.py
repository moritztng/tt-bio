...
Pipeline tasks live alongside this file:
    * Predict — tt_bio/boltzgen/task/predict/predict.py
    * Analyze — tt_bio/boltzgen/task/analyze/analyze.py
    * Filter  — tt_bio/boltzgen/task/filter/filter.py
"""

# Select the Tenstorrent device(s) BEFORE importing anything under
# ``tt_bio.boltzgen`` — its package __init__ pulls in the ttnn adapters,
# and ttnn reads TT_VISIBLE_DEVICES at import time. Mirrors how tt-bio's
...
def add_configure_arguments(
    ...
    p.add_argument(
        ...
        "stream (lossless; collapses per-step host dispatch). Opt-in; reserves "
        "0.2-0.3 GB of device memory.",
    )
...
def run_command(args: argparse.Namespace) -> None:
    """Run the binder-design pipeline end to end.
        ...
        $ tt-bio gen run design.yaml --output out --device_ids 0,1,2,3
    ...
    # --devices N picks the first N (same detection as `tt-bio predict`).
...
def execute_command(args: argparse.Namespace, *, headless: bool = False) -> None:
    ...
    * `--reuse` : Skip recomputation for existing results.
    ...
    # in-process filter pass. Mirrors tt-bio predict's worker setup.
