...
from tt_bio.boltzgen.data.featurizer import (
    res_from_atom14,
    res_from_atom37,
    res_all_gly,
)
from tt_bio.boltzgen.data.write_mmcif import to_mmcif
from tt_bio.boltzgen.data.write_pdb import to_pdb
        ...
        for n in range(n_samples):
            ...
            try:
                ...
                if sample_id is not None:
                    ...
                    stem = str(batch["id"][0])
                    # multiplicity (number of design samples per input spec)
                    # comes from the batch — the original code reached for
                    # ``trainer.datamodule.cfg.multiplicity`` which is gone
                    # with Lightning. The batch's coords first dim carries the
                    # same count.
                    multiplicity = (
                        batch["multiplicity"][0].item()
                        if "multiplicity" in batch
                        else 1
                    )
