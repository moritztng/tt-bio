...
//
// The mcast matmul's output write, factored out so the in0 RISC can issue it instead of the in1
// RISC.  The body is the writer from reader_bmm_tile_layout_in1_receiver_writer_padding.cpp
// lifted verbatim: same subblock loops, same barrier per subblock, same padding handling, same
// compile time arg values in the same order.  Only the thread that runs it changes.
//
...
inline MatmulOutWriterArgs matmul_out_writer_args(uint32_t& idx) {
    MatmulOutWriterArgs a;
    a.enabled = get_arg_val<uint32_t>(idx++);
    a.out_tensor_addr = get_arg_val<uint32_t>(idx++);
    a.out_tensor_start_tile_id = get_arg_val<uint32_t>(idx++);
    a.out_num_nonzero_subblocks_h = get_arg_val<uint32_t>(idx++);
    a.out_last_num_nonzero_subblocks_h = get_arg_val<uint32_t>(idx++);
    a.out_last_subblock_h = get_arg_val<uint32_t>(idx++);
    a.padded_block_tiles_h_skip = get_arg_val<uint32_t>(idx++);
    a.out_num_nonzero_subblocks_w = get_arg_val<uint32_t>(idx++);
    a.out_last_num_nonzero_subblocks_w = get_arg_val<uint32_t>(idx++);
    a.out_last_subblock_w = get_arg_val<uint32_t>(idx++);
    a.padded_subblock_tiles_addr_skip = get_arg_val<uint32_t>(idx++);
    a.padded_block_tiles_w_skip = get_arg_val<uint32_t>(idx++);
    a.last_num_blocks_h_dim = get_arg_val<uint32_t>(idx++);
    a.last_num_blocks_w_dim = get_arg_val<uint32_t>(idx++);
    return a;
}
...
template <
    uint32_t CTA,
    uint32_t cb_id_out0,
    uint32_t num_blocks_h_dim,
    uint32_t num_blocks_w_dim,
    typename Accessor>
inline void matmul_write_out_block(
    ...
    for (uint32_t sbh = 0; sbh < out_num_nonzero_subblocks_h_; ++sbh) {
        ...
        if (bw == num_blocks_w_dim_ - 1) {
            cb_wait_front(cb_id_out0, wargs.padded_block_tiles_w_skip);
            cb_pop_front(cb_id_out0, wargs.padded_block_tiles_w_skip);
        }
        out_tensor_sbh_start_tile_id += out_tensor_next_subblock_stride_h;
    }
    // Pop row(s) of fully padded subblocks
    if (bh == num_blocks_h_dim_ - 1) {
        cb_wait_front(cb_id_out0, wargs.padded_block_tiles_h_skip);
        cb_pop_front(cb_id_out0, wargs.padded_block_tiles_h_skip);
    }
}
