...
class OF3Trunk(Module):
    ...
    def __init__(self, state_dict, compute_kernel_config, num_cycles: int = 4):
        ...
        # reference Attention), and the two kernels under one PairformerLayer need opposite
        # flags to deliver that. `AttentionPairBias` adds the bias inside its own score scale,
        # so unscaled-at-the-reference means pre-baked by sqrt(d) here: scale_pair_bias=True.
        # `TriangleAttention` divides the bake back out before its add, so the same convention
        # is tri_att_scale_pair_bias=False. One shared False held the token bias at
        # 1/sqrt(24) = 0.204 of reference in all 48 blocks of every fold we served.
