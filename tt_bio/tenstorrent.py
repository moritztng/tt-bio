...
@lru_cache(maxsize=None)
def _l1_resident_matmul_config(
    m_tiles: int, k_tiles: int, n_tiles: int, elem_bytes: int, full_k: bool
) -> ttnn.MatmulMultiCoreReuseMultiCast1DProgramConfig | None:
    """Program config for a tall narrow projection whose result can stay in L1, or None.

    ttnn's auto config for a tall narrow matmul sets in0_block_w = div_up(k_tiles, num_cores),
    which collapses to 1 whenever k_tiles < num_cores — every Pairformer-trunk projection at
    C_Z <= 384. The K reduction then runs as k_tiles pack-accumulate rounds instead of one, and
    the result is written to DRAM only for the next op to read it back. Keeping the result in L1
    removes the round-trip: the tri-attention qkv chain (projection + head-split + SDPA) went
    0.519 ms -> 0.264 ms at 128 tokens (state/tt-bio-qkv-op-utilization-rootcause.md).

    full_k picks the K blocking, and with it what the config is bit-exact against: True spans
    the whole of K in one block, the accumulation order minimal_matmul uses; False keeps
    in0_block_w=1, the blocking the auto config already runs in this shape class, so a
    ttnn.linear core_grid call can move its result to L1 without changing a bit.

    Guards, all hard:

    - the shape must be tall narrow (m_tiles >= 8*n_tiles) with k_tiles < num_cores — outside
      that class the auto config is already correct, and in0_block_w=1 would not match it;
    - the CBs plus the L1-resident output tensor must fit per-core L1 (the output CB grows with
      per_core_M, so larger sequences stop fitting and fall back);
    - the result must fit aggregate L1 with room to spare, since the rest of the block is still
      allocating.
    """
    gx, gy = COMPUTE_GRID_MAIN
    num_cores = gx * gy
...
@lru_cache(maxsize=None)
def _l1_resident_row_block(rows: int, cols: int, k_tiles: int, n_tiles: int, elem_bytes: int) -> int:
    """Rows of the pair tensor per block so a projection's result stays in L1, or 0.

    The whole-tensor L1 residency dies with sequence length: at 128 tokens the qkv result
    is 25 MB and fits, at 320 tokens it is 157 MB and no per-core budget admits it, so the
    projection writes it to DRAM for nlp_create_qkv_heads to read straight back. The result
    is a row block of the pair tensor though, and every op between the projection and the
    output projection is row-local or row-batched, so splitting the pair tensor into row
...
def _tri_att_row_block(x: ttnn.Tensor, w: ttnn.Tensor, dtype) -> int:
    """_l1_resident_row_block for a concrete tri-attention projection, or 0.

    0 when the whole tensor already fits (blocking would only add dispatches) or when the
    shapes are outside the guard's class."""
    if dtype != ttnn.bfloat16:
        return 0
    try:
...
def _configure_active_compute_grid(device: ttnn.Device) -> None:
    ...
    _l1_resident_matmul_config.cache_clear()
    _l1_resident_row_block.cache_clear()
...
def _trimul_out_proj(
    ...
    # Both results are consumed by the multiply_ that follows, so the L1-resident config
    # applies where the guard admits it; where it does not, _l1_resident_linear is the same
    # ttnn.linear(core_grid=CORE_GRID_MAIN) into DRAM this used to call directly.
    # full_k=False keeps the auto config's K blocking, so it is bit-exact either way.
    return _l1_resident_linear(x, weight, _dtype(), ckc)
...
class TriangleMultiplication(Module):
    ...
    def __call__(self, x: ttnn.Tensor, mask: ttnn.Tensor | None = None) -> ttnn.Tensor:
        ...
        # destination allocated once. L1 (small-L) path only; applicable() checks the chunk
        # geometry and dtype. It was measured against a running concat, which this file no
        # longer does on either path, so its gain here is smaller than the branch reported.
        out_fused = (
            _TRIMUL_OUT_FUSED
            and not large_seq
            and not _TRIMUL_RAW_CHANNEL_MOVES
...
class TriangleAttention(Module):
    ...
    def __call__(self, x: ttnn.Tensor, attn_mask: ttnn.Tensor | None = None) -> ttnn.Tensor:
        ...
        # Row-block for L1 residency, not for memory safety: the projections' results only
        # fit L1 one row block at a time past ~128 tokens (_l1_resident_row_block). 0 when
        # the whole tensor already fits, or when nothing fits and blocking would not help.
        # It rides the whole-tensor prologue below, NOT the memory-safety chunk path: that
        # path re-norms the pair tensor a second time to keep it off the device, which costs
        # more than the residency wins (measured 0.91-0.98x at 320 tokens).
        l1_rows = 0 if (need_chunk or self.affinity) else _tri_att_row_block(
            x, self.qkv_weight, _dtype())
        if need_chunk:
        ...
        def attend(qkv_in, bias):
            ...
            # (see _l1_resident_matmul_config) reading it back out to DRAM here would hand back
            # the whole win, and the chunked path's qkv is in DRAM so this is a no-op for it.
            q, k, v = ttnn.experimental.nlp_create_qkv_heads(
        ...
        def gate_and_project(o_in: ttnn.Tensor, g_in: ttnn.Tensor, l1: bool = False) -> ttnn.Tensor:
            o_in = ttnn.multiply_(o_in, g_in, input_tensor_b_activations=[ttnn.UnaryOpType.SIGMOID])
            ttnn.deallocate(g_in)
            # full_k=False: the auto config already runs this projection's K in in0_block_w=1
        ...
        def proj_l1(x_in: ttnn.Tensor, w: ttnn.Tensor) -> ttnn.Tensor:
            # minimal_matmul accumulates the whole K in one block, so the full-K L1
            # config is bit-exact against it (_l1_resident_matmul_config).
            cfg = _l1_resident_linear_config(x_in, w, _dtype())
            if cfg is not None:
                return ttnn.linear(
                    x_in,
                    w,
        ...
        if need_chunk:
            ...
            for s in range(0, S, chunk):
                ...
                qkv_chunk = proj_l1(x_chunk, self.qkv_weight)
                g_chunk = proj_l1(x_chunk, self.g_weight)
                ttnn.deallocate(x_chunk)
                if self.affinity:
            ...
            # Row-blocked so the projections keep their results in L1 (see
            # _l1_resident_row_block). The layer_norm and the triangle bias above are the
            # whole-tensor ones, so only the projection -> head split -> SDPA -> output
            # chain is blocked, and every op in it is row-local or row-batched: bit-exact
            # against the whole-tensor chain, verified with torch.equal at 320 and 384
            # tokens. Measured on the chain alone at 320 tokens: 1.24x.
            ...
            for start in range(0, S, l1_rows):
                ...
                o_blk = attend(qkv_blk, triangle_bias)
                ttnn.deallocate(qkv_blk)
                parts.append(gate_and_project(o_blk, g_blk))
            ttnn.deallocate(x)
            ttnn.deallocate(triangle_bias)
            x = ttnn.concat(parts, dim=0)
            for part in parts:
            ...
            qkv = proj_l1(x, self.qkv_weight)
            g = proj_l1(x, self.g_weight)
            ttnn.deallocate(x)
            if attn_mask is not None:
                triangle_bias = ttnn.add(triangle_bias, attn_mask)
            ...
            ttnn.deallocate(qkv)
            ttnn.deallocate(triangle_bias)
            x = gate_and_project(o, g, l1=True)
        if self.ending:
            x = ttnn.permute(x, (1, 0, 2), memory_config=_transpose_memory_config(x))
