...
def _accurate_softmax(x, compute_kernel_config=None, fp32: bool = True):
    ...
    nothing, and neither does the ttnn version: 0.67.4 (the pin), 0.68.0 and 0.75.0 all
    return rowsum_min 0.9612371693410751 and rel_rms 0.019012 on the same input, so this
    is not a regression waiting on an upstream fix
    (perf/xmsoftmax/results/kernel_v06*.json, kernel_v0750.json).
