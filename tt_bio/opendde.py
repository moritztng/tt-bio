...
class OpenDDE:
    ...
    def expand_and_refine(self, ifd, s_inputs_res, s_res, z_res, *,
        ...
        # holds 172 device tensors before that call and 268 after, and the 96 new ones are all
        # `_gp_cache`: 12 fused input-weight chunks per trimul, 2 trimuls per block, 4 blocks,
        # ~9.4 MB. Nothing is mutated (changed=0). The first call BUILDS that cache while it
        # computes, and no later call does.
        #
        ...
        bias = ttnn.reshape(attn_bias, (1, 1, Ns, Ns)) if extra_attn_bias else None
