...
#: ``[fused, composed]`` per fusion. These three were the only shipped levers in the tree with
#: no counter, so `scripts/lever_census.py` could not tell a fusion that fires from one whose
#: operands never satisfy it -- and ``scale_add`` declines on a dtype, which is a property of
#: the call site and not of the flag. Counting is free and the count is the only way to read
