...
$PY -m pytest models/demos/llama3_1_8b_p150/demo/simple_text_demo.py::test_demo_text -k "ci-token-matching and performance" -s > "$WT/logs/pcc_direct3.log" 2>&1
echo "stepB3 rc=$? $(date -u)" >> $ST
echo "=== STEP C3 perf as-shipped $(date -u) ==="
$PY -m pytest models/demos/llama3_1_8b_p150/tests/e2e/test_main_perf.py -s > "$WT/logs/perf_asis3.log" 2>&1
echo "stepC3 rc=$? $(date -u)" >> $ST
...
$PY -m pytest models/demos/llama3_1_8b_p150/tests/e2e/test_main_perf_fixed_probe.py -s > "$WT/logs/perf_fixed3.log" 2>&1
echo "stepD3 rc=$? $(date -u)" >> $ST
rm -f models/demos/llama3_1_8b_p150/tests/e2e/test_main_perf_fixed_probe.py
echo "DEVICE TESTS3 COMPLETE $(date -u)" >> $ST
